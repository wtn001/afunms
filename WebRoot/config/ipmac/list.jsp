...
<%
	...
	if (macbaselist != null && macbaselist.size() > 0) {
		for (int i = 0; i < macbaselist.size(); i++) {
			...
			macbaseHash.put(ipmacbase.getMac() + ":"
					+ ipmacbase.getRelateipaddr(), ipmacbase);
		}
	}
	dao.close();
%>
																...
																<tr>
																	<td class="body-data-title" style="text-align: right;">
																		<a href="<%=rootPath%>/ipmac.do?action=downloadipmacreport"
																			...
																			border="0">导出EXCEL</a>&nbsp;&nbsp;&nbsp;&nbsp;
																		<a href="<%=rootPath%>/ipmac.do?action=downloadipmacreportall"
																			target="_blank"><img name="selDay1" alt='导出EXCEL'
																			style="CURSOR: hand"
																			src="<%=rootPath%>/resource/image/export_excel.gif" width=18
														...
														<td>
															<table>
																...
																<%
																	...
																	String ipaddres="--";
																	String mac="--";
																	if(vo.getRelateipaddr()!=null&&!vo.getRelateipaddr().equalsIgnoreCase("null"))ipaddr=vo.getRelateipaddr();
																	if(vo.getIpaddress()!=null&&!vo.getIpaddress().equalsIgnoreCase("null"))ipaddres=vo.getIpaddress();
																	if(vo.getMac()!=null&&!vo.getMac().equalsIgnoreCase("null"))mac=vo.getMac();
																...
																<tr <%=onmouseoverstyle%>>
																	...
																	<td align="center" class="body-data-list"><%=ipaddr%></td>
																	<td align="center" class="body-data-list"><%=vo.getIfindex()%></td>
																	<td align="center" class="body-data-list"><%=ipaddres%></td>
																	<td align="center" class="body-data-list"><%=mac%></td>
																	<%
																		int baseflag = 0;
		...
		<script language="JavaScript" type="text/javascript">
			...
			{  
				    ...
				    Ext.get("process").on("click",function(){
						...
						Ext.MessageBox.wait('数据同步中，请稍后.. ');  
						mainForm.action = "<%=rootPath%>/ipmac.do?action=sychronizeData";
	  					mainForm.submit(); 
					 });    
			});
		</script>
