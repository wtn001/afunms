...
<%@page import="com.afunms.config.model.IpConfig"%>
	...
	<head>
		...
		<script type="text/javascript">
			var curpage= <%=jp.getCurrentPage()%>;
 			...
 			var delAction = "<%=rootPath%>/macconfig.do?action=delete1";
		...
		<script language="JavaScript">
			...
			*/
			...
			function toAdd()
			{
				mainForm.action = "<%=rootPath%>/macconfig.do?action=ready_add1";
				mainForm.submit();
			}
			
			function edit()
			{
				mainForm.action = "<%=rootPath%>/macconfig.do?action=ready_edit1&id="+node;
				mainForm.submit();
			}
			...
			function search(condition){
			    ...
			    mainForm.action = "<%=rootPath%>/macconfig.do?action=list1&jp=1&condition="+condition;
				mainForm.submit();
			}
															...
															<table>
																<tr>
														    		...
														    		<td class="body-data-title" style="text-align: left;">
									    								...
									    								&nbsp;&nbsp;
									    								<input type="button" value="IP查询"  onclick='search("IP")'>
														    		</td>
														    		<td class="body-data-title" style="text-align: left;">
														    			&nbsp;&nbsp;&nbsp;
		        									...
		        									<tr>
		        										<td>
		        											<table>
		        												<tr>
		        													...
		        													<td align="center" class="body-data-title">IP描述</td>
		        									    ...
		        									    if(list!=null&& list.size()>0){
		        									        for(int i = 0 ; i < list.size() ; i++){
		        									            ...
		        									            String districtName = "";
		        									            if(hashtable!=null && hashtable.containsKey(ipconfig.getId())){
		        									            	districtName = ((DistrictConfig)hashtable.get(ipconfig.getId())).getName();
		        									            }
		        									            %>
		        									            <tr <%=onmouseoverstyle%>>
		        													<td align="center" class="body-data-list"><INPUT type="checkbox" value="<%=ipconfig.getId()%>" name="checkbox" onclick=""><%=i+jp.getStartRow()%></td>
		        													<td align="center" class="body-data-list"><%=ipconfig.getIpaddress()%></td>
		        													<td align="center" class="body-data-list"><%=districtName%></td>
		        													<td align="center" class="body-data-list"><%=ipconfig.getIpdesc()%></td>
		        													<td align="center" class="body-data-list">
																		<img title="右键查看操作" src="<%=rootPath%>/resource/image/status.gif"
																			border="0" width=15 oncontextmenu=showMenu('2','<%=ipconfig.getId()%>','','1')>
																	</td>
					        									</tr>
