...
function downloadLog(){
   ...
   mainForm.action="<%=rootPath%>/slacmd.do?action=downloadLog";
	mainForm.submit();
}
function loadFile(){
window.open("<%=rootPath%>/slacmd.do?action=loadFile","oneping", "height=500, width= 800, top=100, left=100,scrollbars=yes");		
}
function saveFile(){
var commands=$("#commands").text();
reg = new RegExp("(\r)","g");
var value=commands.replace(reg,";;") 
window.open("<%=rootPath%>/slacmd.do?action=saveFile&&commands="+value,"oneping", "height=400, width= 500, top=300, left=100,scrollbars=yes");
				}
</script>
