  ...
  function toAdd()
  {
     ...
     mainForm.submit();
	  ...
	  if(confirm('是否确定删除这条记录?')) {
		mainForm.action = "<%=rootPath%>/knowledge.do?action=delete";
		mainForm.submit();
		}
	}
														...
														<td>
															<table>
																...
																<tr <%=onmouseoverstyle%>>
																	...
																	<td align="center" class="body-data-list">
																		<a href="#"
																			onclick="window.open('<%=rootPath%>/config/knowledges/show.jsp?attachfiles=<%=attachfiles%>&category=<%=category%>&entity=<%=entity%>&subentity=<%=subentity%>')"><%=vo.getAttachfiles()%></a>
																	</td>
																	<td align="center" class="body-data-list"><%=vo.getBak()%></td>
																	<td align="center" class="body-data-list">
																		<a
																			href="<%=rootPath%>/knowledge.do?action=ready_edit&id=<%=vo.getId()%>"><img
