...
<%
	...
	os0="selected";
	}else if(machine.getServerType().equalsIgnoreCase("AIX")){
	os1="selected";
	}else if(machine.getServerType().equalsIgnoreCase("HP UNIX")){
	os2="selected";
	}else if(machine.getServerType().equalsIgnoreCase("SUN Solaris")){
	os3="selected";
	}else if(machine.getServerType().equalsIgnoreCase("Linux")){
	os4="selected";
	}else if(machine.getServerType().equalsIgnoreCase("AS400")){
	os5="selected";
	}else{
	os="selected";
	}
...
//-- nielin modify at 2010-01-04 end ----------------
	function loadFile(obj){
				window.open("/afunms/serverUpAndDown.do?action=loadFile&scriptType="+obj,"oneping", "height=500, width= 800, top=100, left=100,scrollbars=yes");		
			}
																	...
																	<td>
																		...
																		<table border="0" id="table1" cellpadding="0"
																			...
																			<tr style="background-color: #ECECEC;">
																				...
																				<TD nowrap width="40%" colspan=3>&nbsp;
																					<select   name="serverType"  class="formStyle">
																						<option value='windows' <%=os0 %>>Windows</option>
																						<option value='aix' <%=os1 %>>AIX</option>
																						<option value='unix' <%=os2 %>>HP UNIX</option>
																						<option value='solarix' <%=os3 %>>SUN Solaris</option>
																						<option value='linux' <%=os4 %>>Linux</option>
																						<option value='as400' <%=os5 %>>AS400</option>
																					</select>
																					<font color="red">*&nbsp;</font>
																				</TD>
																			</tr>
																					...
																					<select   name="clusterId"  class="formStyle">
																					 <% 
													                  					if(machine.getClusterId()==0){
													                  					 %>
													                  					    <option value='<%=machine.getClusterId() %>'selected> </option>
													                  					    <%
													                  					   }else{
													                  					   %>
													                  					   
													                  					    <option value='<%=machine.getClusterId() %>'> </option>
													                  					    <%
													                  					   }
																			...
																			<tr>
																				...
																				<td nowrap>
																					&nbsp;
																					<input name="powerOnAfter" type="text" size="22"
																						class="formStyle" id="powerOnAfter" value="<%=machine.getPowerOnAfter()==null?"":machine.getPowerOnAfter() %>">
																					<input type="button" value="选择脚本"
																						onclick="loadFile('powerOnAfter')">
																				</td>
																				<td nowrap align="right" height="24">
																					关机前脚本&nbsp;
																				</td>
																				<td nowrap>
																					&nbsp;
																					<input name="powerOffBefore" type="text" size="22"
																						class="formStyle" id="powerOffBefore" value="<%=machine.getPowerOffBefore()==null?"":machine.getPowerOffBefore() %>">
																					<input type="button" value="选择脚本"
																						onclick="loadFile('powerOffBefore')">
																				</td>
																			</tr>
																			<tr style="background-color: #ECECEC;">
