	...
	if(type==1){
		...
		//公共变量
			...
			*/
			...
			function toAdd(){
				mainForm.action = "<%=rootPath%>/vpntelnetconf.do?action=addFileBackupTelnetConfig";
			    mainForm.submit();
			}
			
			function toDelete(){  
     				mainForm.action = "<%=rootPath%>/vpntelnetconf.do?action=deleteFileBackupTelnetConfig";
     				mainForm.submit();
	  		}
	  		
	  		function edit()
			{
				mainForm.action="<%=rootPath%>/vpntelnetconf.do?action=ready_editFileBackupTelnetConfig&id="+node;
				mainForm.submit();
			}
	  		function addBackup()
			{
				mainForm.action="<%=rootPath%>/vpntelnetconf.do?action=addFileBackup&id="+node;
				mainForm.submit();
			}
	  		function disBackup()
			{
				mainForm.action="<%=rootPath%>/vpntelnetconf.do?action=disFileBackup&id="+node;
				mainForm.submit();
			}
		</script>
