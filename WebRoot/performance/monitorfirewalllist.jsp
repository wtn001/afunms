...
<script language="javascript">	
  ...
  var listAction = "<%=rootPath%>/perform.do?action=monitorfirewalllist";
	...
	<tr>
		...
		<td bgcolor="#cedefa" align="center" valign=top>
			<table width="98%" style="BORDER-COLLAPSE: collapse" borderColor=#397DBD cellPadding=0 rules=none align=center border=1 algin="center">
				<tr>
					<td height="28" align="left" bordercolor="#397DBD" bgcolor="#397DBD" class="txtGlobalBold" colspan=3>&nbsp;&nbsp;<font color=#ffffff>&nbsp;设备维护 >> 防火墙列表</font></td>
				</tr>
				<!--<tr bgcolor=#ffffff>
...
%>
   										<tr bgcolor="DEEBF7" <%=onmouseoverstyle%> class="microsoftLook">  
    											<td width='5%'>&nbsp;&nbsp;<INPUT type="checkbox" class=noborder name=checkbox value="<%=vo.getId()%>" border="0">
    												<font color='blue'><%=startRow + i%></font></td>
    												
    											<td  align='center' >
    											<%
    												if(host.isAlarm()){
    													if(host.getAlarmlevel()==1){
    														//普通告警
    											%>
    												<img src="<%=rootPath%>/resource/image/topo/alarm_level_1.gif" >&nbsp;告警
    												<%
    													}else if(host.getAlarmlevel() == 2){
    														//严重告警
    												%>
    												<img src="<%=rootPath%>/resource/image/topo/alarm_level_2.gif" >&nbsp;告警
    												<%
    													}else if(host.getAlarmlevel() == 3){
    														//紧急告警
    												%>
												...
												<img src="<%=rootPath%>/resource/image/status.gif" border="0" width=15 oncontextmenu=showMenu('2','<%=vo.getId()%>','<%=vo.getIpAddress()%>') alt="右键操作"></td>
  										</tr>			
<% }%>
									</table>
								</td>
