...
<script language="javascript">	
  ...
  function toAdd()
  {
      mainForm.action = "<%=rootPath%>/ggsci.do?action=ready_add";
      mainForm.submit();
  }
  ...
  {
     if(confirm('是否删除设备？')){
     	mainForm.action = "<%=rootPath%>/ggsci.do?action=delete";
   	    mainForm.submit();
     }
  }
...
<script language="JavaScript">
	...
	*/
	...
	function detail()
	{
	    mainForm.action ="<%=rootPath%>/ggsci.do?action=detail&id="+node;
	    mainForm.submit();
	}
	function edit()
	{
		mainForm.action ="<%=rootPath%>/ggsci.do?action=ready_edit&id="+node;
		mainForm.submit();
	}
	function cancelmanage()
	{
		mainForm.action ="<%=rootPath%>/ggsci.do?action=cancelalert&id="+node;
		mainForm.submit();
	}
	function addmanage()
	{
		mainForm.action ="<%=rootPath%>/ggsci.do?action=addalert&id="+node;
		mainForm.submit();
	}	
	function clickMenu()
	{
	...
	<tr>
		...
		<td align="center" valign=top>
			...
			<tr>
				<td background="<%=rootPath%>/common/images/right_t_02.jpg" width="100%"><table width="100%" cellspacing="0" cellpadding="0">
                  <tr>
                    ...
                    <td class="add-content-title">应用 >> 服务管理 >> Ggsci监视列表</td>
			...
			<%
    			...
    			{
       				vo = (GgsciConfig)list.get(i);
       				int alarmlevel = 0;
       				Node node = (Node)PollingEngine.getInstance().getGgsciByID(vo.getId());
       				String alarmmessage = "";
       				if(node != null){
