...
<%
	...
	int status = 0;
	Node node = (Node) PollingEngine.getInstance().getTftpByID(
			queryconf.getId());
	String alarmmessage = "";
	if (node != null) {
...
function setClass(){
	document.getElementById('tftpDetailTitle-1').className='detail-data-title';
	document.getElementById('tftpDetailTitle-1').onmouseover="this.className='detail-data-title'";
	document.getElementById('tftpDetailTitle-1').onmouseout="this.className='detail-data-title'";
}
...
function query()
  {  
     mainForm.action = "<%=rootPath%>/tftp.do?action=alarm";
     mainForm.submit();
  }
</script>
																								...
																								<!--<table width="100%" cellspacing="0" cellpadding="0" align="center">
																					            	<tr> 
																					         		<td width="100%" align="center"> 
																										...
																										<script type="text/javascript">
																											var so = new SWFObject("<%=rootPath%>/flex/Tftp_Info.swf?names=<%=queryconf.getName()%>&filename=<%=queryconf.getFilename()%>&ip=<%=queryconf.getIpaddress()%>&lasttime=<%=request.getAttribute("lasttime")%>&nexttime=<%=request.getAttribute("nexttime")%>", "Ftp_Info", "400", "250", "8", "#ffffff");
