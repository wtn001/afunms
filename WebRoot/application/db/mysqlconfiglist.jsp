...
<script language="JavaScript" type="text/javascript">
  ...
  var delAction = "<%=rootPath%>/mysqlconfig.do?action=delete";
  var listAction = "<%=rootPath%>/mysqlconfig.do?action=list";
  ...
  {
     mainForm.action = "<%=rootPath%>/mysqlconfig.do?action=createspaceconfig";
     mainForm.submit();
  }
  function search()
  {
     mainForm.action = "<%=rootPath%>/mysqlconfig.do?action=search";
     mainForm.submit();
  }  
</script>
<script language="JavaScript">
	...
	*/
	...
	function edit()
	{
		location.href="<%=rootPath%>/mysqlconfig.do?action=ready_edit&id="+node;
	}	
	function clickMenu()
	{
	}
			...
			<tr>
				<td background="<%=rootPath%>/common/images/right_t_02.jpg" width="100%"><table width="100%" cellspacing="0" cellpadding="0">
                  <tr>
                    ...
                    <td class="add-content-title">应用 >> 数据库管理 >> MYSQL告警设置</td>
...
if(rc > 0){
    ...
    for(int i=0;i<rc;i++)
    {
       MySqlSpaceConfig vo = (MySqlSpaceConfig)list.get(i);
       String flag = "告警";
...
%>
       <tr bgcolor="#FFFFFF" <%=onmouseoverstyle%> class="microsoftLook">
    		<td height=25>&nbsp;<font color='blue'><%=i + startRow%></font></td>
		...
		<td height=25>&nbsp;
			<%
				if(vo.getSms()==0){
			%>
			<a href="<%=rootPath%>/mysqlconfig.do?action=addalert&id=<%=vo.getId()%>"><font color=#397DBD>告警</font></a>
			<%
				}else{
			%>
			<a href="<%=rootPath%>/mysqlconfig.do?action=cancelalert&id=<%=vo.getId()%>"><font color=#397DBD>取消告警</font></a>
			<%	}
			%>
