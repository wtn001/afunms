...
<script language="JavaScript" type="text/javascript">
	...
	*/
	...
	function edit()
	{
	    mainForm.action = "<%=rootPath%>/oraspace.do?action=ready_edit&id="+node+"&sid="+sid;
        mainForm.submit();
	}
	function cancelalert(id)
	{
        mainForm.action = "<%=rootPath%>/oraspace.do?action=cancelalert&id=" + id;
        mainForm.submit();
	}
	function addalert(id)
	{
        mainForm.action = "<%=rootPath%>/oraspace.do?action=addalert&id=" + id;
        mainForm.submit();
	}	
	function clickMenu()
	{
			...
			<tr>
				<td background="<%=rootPath%>/common/images/right_t_02.jpg" width="100%"><table width="100%" cellspacing="0" cellpadding="0">
                  <tr>
                    ...
                    <td class="layout_title">应用 >> 数据库管理 >> ORACLE告警设置</td>
...
%>
       <tr bgcolor="#FFFFFF" <%=onmouseoverstyle%> class="microsoftLook">
		...
		<td height=25>&nbsp;<%=ip3[0]+"."+ip3[1]+"."+ip3[2]+"."+ip3[3]+":"+dbvo.getId()%></td> 
...
%>				
		...
		<tr>
              <td>
              	...
              	<center><input type="button" value="关 闭" style="width: 50" onclick="window.close()"></center></br>
              </td>
            </tr>
	</table>
</td>
