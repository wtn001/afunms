...
<%
  ...
  if(list == null)list = new ArrayList();
    ...
    String showNodeList = (String)request.getAttribute("showNodeList");
%>
...
<script language="JavaScript" type="text/javascript">
  ...
  {
     mainForm.action = "<%=rootPath%>/sqldbconfig.do?action=createspaceconfig";
     mainForm.submit();
  }
  function search()
  {
     mainForm.action = "<%=rootPath%>/sqldbconfig.do?action=search";
     mainForm.submit();
  }  
</script>
<script language="JavaScript">
	...
	*/
	...
	function edit()
	{
		mainForm.action = "<%=rootPath%>/sqldbconfig.do?action=ready_edit&id="+node;
        mainForm.submit();
	}
    function addalert(node)
    {
        mainForm.action = "<%=rootPath%>/sqldbconfig.do?action=addalert&id="+node;
        mainForm.submit();
    }
    function cancelalert(node)
    {
        mainForm.action = "<%=rootPath%>/sqldbconfig.do?action=cancelalert&id="+node;
        mainForm.submit();
    }
	function clickMenu()
	{
...
<form method="post" name="mainForm">
    <input type=hidden name="showNodeList" value="<%=showNodeList%>">
			...
			<tr>
				<td background="<%=rootPath%>/common/images/right_t_02.jpg" width="100%"><table width="100%" cellspacing="0" cellpadding="0">
                  <tr>
                    ...
                    <td class="layout_title"><b>应用 >> 数据库管理 >> SQLSERVER告警设置</b></td>
                    <td align="right"><img src="<%=rootPath%>/common/images/right_t_03.jpg" width="5" height="29" /></td>
								...
								<td bgcolor="#ECECEC" width="50%" align='right'>
									...
									<a href="#" onclick="createSpaceConfig()">刷新数据库阀值</a>&nbsp;&nbsp;&nbsp;
		  						</td>
										</tr>
								</table>
		  						</td>
...
if(list.size() > 0){
    ...
    {
       Sqldbconfig vo = (Sqldbconfig)list.get(i);
       String flag = "告警";
       if(vo.getSms() == 0)flag = "未告警";
       dbdao = new DBDao();
	List shareList = dbdao.getDbByTypeAndIpaddress(typevo.getId(), vo.getIpaddress());
	DBVo dbvo = (DBVo)shareList.get(0);
	String logflag = "库文件";
	if(vo.getLogflag() == 1)logflag = "日志文件";
...
%>
       <tr bgcolor="#FFFFFF" <%=onmouseoverstyle%> class="microsoftLook">
		...
		<td height=25>&nbsp;<%=logflag%></td> 
		<td height=25>&nbsp;<%=vo.getAlarmvalue()%></td> 
		<td height=25>&nbsp;
			<%
				if(vo.getSms()==0){
			%>
			<a href="#" onclick="addalert('<%=vo.getId()%>');"><font color=#397DBD>告警</font></a>
			<%
				}else{
			%>
			<a href="#" onclick="cancelalert('<%=vo.getId()%>');"><font color=#397DBD>取消告警</font></a>
			<%	}
			%>
