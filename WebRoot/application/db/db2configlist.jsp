...
<script language="JavaScript">
	...
	*/
	...
	function edit()
	{
		location.href="<%=rootPath%>/db2config.do?action=ready_edit&id="+node;
	}	
	function clickMenu()
	{
	}
       ...
       <tr bgcolor="#FFFFFF" <%=onmouseoverstyle%> class="microsoftLook">
		...
		<td height=25>&nbsp;
			<%
				if(vo.getSms()==0){
			%>
			<a href="<%=rootPath%>/db2config.do?action=addalert&id=<%=vo.getId()%>"><font color=#397DBD>告警</font></a>
			<%
				}else{
			%>
			<a href="<%=rootPath%>/db2config.do?action=cancelalert&id=<%=vo.getId()%>"><font color=#397DBD>取消告警</font></a>
			<%	}
			%>
