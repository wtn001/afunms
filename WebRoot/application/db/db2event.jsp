...
if(commonValue.containsKey("configured_cpu")){
	...
	String[] sysDbApplStatus = { "INIT", "CONNECTPEND", "CONNECTED", "UOWEXEC", "UOWWAIT", "LOCKWAIT", "COMMIT_ACT",
			"ROLLBACK_ACT", "RECOMP", "COMP", "INTR", "DISCONNECTPEND", "TPREP", "THCOMT", "THABRT", "TEND", "CREATE_DB",
			"RESTART", "RESTORE", "BACKUP", "LOAD", "UNLOAD", "IOERROR_WAIT", "QUIESCE_TABLESPACE", "WAITFOR_REMOTE",
			"REMOTE_RQST", "DECOUPLED", "ROLLBACK_TO_SAVEPOINT" };
	String[] sysDbPlatform = { "UNKNOWN", "OS2", "DOS", "WINDOWS", "AIX", "NT", "HP", "SUN", "MVS_DRDA", "AS400_DRDA",
			"VM_DRDA", "VSE_DRDA", "UNKNOWN_DRDA", "SNI", "MAC", "WINDOWS95", "SCO", "SGI", "LINUX", "DYNIX", "AIX64",
			"SUN64", "HP64", "NT64", "LINUX390", "LINUXZ64", "LINUXIA64", "LINUXPPC", "LINUXPPC64", "OS390",
			"LINUXX8664", "HPIA", "HPIA64", "SUNX86", "SUNX8664" };
	String[] sysDbProtocols = { "appc", "netbios", "appn", "tcp/ip", "APPC using CPIC", "ipx/spx", "local client",
			"Named pipe" };
	...
	String _flag = (String)request.getAttribute("flag");
     ...
     int level1=0;
     int _status=0;
     String level1strg=String.valueOf(request.getAttribute("level1")==null?"0":request.getAttribute("level1"));
     String statusstrg=String.valueOf(request.getAttribute("status")==null?"0":request.getAttribute("status"));
     if(null!=level1strg)
		{ level1 = Integer.parseInt(level1strg);}
	 if(null!=statusstrg)
	 	{_status = Integer.parseInt(statusstrg);}
...
setInterval(gzmajax,60000);
  ...
  function doQuery()
  {  
     mainForm.action = "<%=rootPath%>/db2.do?action=db2event&id=<%=vo.getId()%>";
     mainForm.submit();
  }
...
function setClass(){
	document.getElementById('db2DetailTitle-8').className='detail-data-title';
	document.getElementById('db2DetailTitle-8').onmouseover="this.className='detail-data-title'";
	document.getElementById('db2DetailTitle-8').onmouseout="this.className='detail-data-title'";
}
																	...
																	<td>
																		<table width="100%">
																			...
																			<%
																				...
																				List list = (List) request.getAttribute("list")==null?(new ArrayList()):(List) request.getAttribute("list");
																			...
																			<tr bgcolor="#FFFFFF" <%=onmouseoverstyle%>
																				...
																				<td align=center
																					...
																					<%=rtime2%></td>
																				<td align=center
																					class="application-detail-data-body-list" bgcolor=<%=bgcolorstr%>>
																					<%=status%></td>
																				<td align=center
																					class="application-detail-data-body-list">
																					<%
