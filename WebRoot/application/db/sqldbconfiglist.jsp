...
<script language="JavaScript" type="text/javascript">
  ...
  var delAction = "<%=rootPath%>/sqldbconfig.do?action=delete";
  var listAction = "<%=rootPath%>/sqldbconfig.do?action=list";
...
<script language="JavaScript">
	...
	*/
	...
	function edit()
	{
		location.href="<%=rootPath%>/sqldbconfig.do?action=ready_edit&id="+node;
	}	
	function clickMenu()
	{
	}
			...
			<tr>
				<td background="<%=rootPath%>/common/images/right_t_02.jpg" width="100%"><table width="100%" cellspacing="0" cellpadding="0">
                  <tr>
                    ...
                    <td class="add-content-title"><b>应用 >> 数据库管理 >> SQLSERVER告警设置</b></td>
       ...
       <tr bgcolor="#FFFFFF" <%=onmouseoverstyle%> class="microsoftLook">
		...
		<td height=25>&nbsp;
			<%
				if(vo.getSms()==0){
			%>
			<a href="<%=rootPath%>/sqldbconfig.do?action=addalert&id=<%=vo.getId()%>"><font color=#397DBD>告警</font></a>
			<%
				}else{
			%>
			<a href="<%=rootPath%>/sqldbconfig.do?action=cancelalert&id=<%=vo.getId()%>"><font color=#397DBD>取消告警</font></a>
			<%	}
			%>
