...
<script language="JavaScript" type="text/javascript">
  ...
  var delAction = "<%=rootPath%>/sybaseconfig.do?action=delete";
  var listAction = "<%=rootPath%>/sybaseconfig.do?action=list";
...
<script language="JavaScript">
	...
	*/
	...
	function edit()
	{
		location.href="<%=rootPath%>/sybaseconfig.do?action=ready_edit&id="+node;
	}	
	function clickMenu()
	{
	}
			...
			<tr>
				<td background="<%=rootPath%>/common/images/right_t_02.jpg" width="100%"><table width="100%" cellspacing="0" cellpadding="0">
                  <tr>
                    ...
                    <td class="add-content-title">应用 >> 数据库管理 >> SYBASE告警设置</td>
...
%>
       <tr bgcolor="#FFFFFF" <%=onmouseoverstyle%> class="microsoftLook">
    		<td height=25>&nbsp;<font color='blue'><%=startRow + i%></font></td>
		...
		<td height=25>&nbsp;
			<%
				if(vo.getSms()==0){
			%>
			<a href="<%=rootPath%>/sybaseconfig.do?action=addalert&id=<%=vo.getId()%>"><font color=#397DBD>告警</font></a>
			<%
				}else{
			%>
			<a href="<%=rootPath%>/sybaseconfig.do?action=cancelalert&id=<%=vo.getId()%>"><font color=#397DBD>取消告警</font></a>
			<%	}
			%>
