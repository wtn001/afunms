...
<%
	...
	if("0".equals(runmodel)){
		  ...
		  if(wasIpaddressData != null){
			  ...
			  hm = CommonUtil.converHashTableToHashMap(threadHashtable); 
		  }
  	}else{
  		 //采集与访问分离模式
  		 GetWasInfo wasconf = new GetWasInfo();
  		 try{
		 	hm = wasconf.executeQueryHashMap(vo.getIpaddress(),"wasthread");
	 	 }catch(Exception e){
		 	e.printStackTrace();
		 }finally{
		 	 wasconf.close();
...
function setClass(){
	document.getElementById('was5DetailTitle-5').className='detail-data-title';
	document.getElementById('was5DetailTitle-5').onmouseover="this.className='detail-data-title'";
	document.getElementById('was5DetailTitle-5').onmouseout="this.className='detail-data-title'";
}
																...
																<td>
																	<table width="96%" align="center">
																		...
																		<tr>
																			...
																			<%
																				if (hm.get("destroyCount") != null) {
																			%>
																			<td width="35%"><%=hm.get("destroyCount")%>
																			</td>
																			<%
																				} else {
																			%>
