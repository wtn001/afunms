...
<script language="JavaScript" type="text/javascript">
  ...
  function toAdd()
  {
     mainForm.action = "<%=rootPath%>/mqchannel.do?action=ready_add";
     mainForm.submit();
  }
  function toDelete()
  {
     mainForm.action = "<%=rootPath%>/mqchannel.do?action=delete";
     mainForm.submit();
  } 
  function createMQConfig(){
 
  	mainForm.action = "<%=rootPath%>/mqchannel.do?action=create_mqconfig";
	mainForm.submit();
  }   
</script>
<script language="JavaScript">
	...
	*/
	...
	function detail()
	{
	    location.href="<%=rootPath%>/mqchannel.do?action=detail&id="+node;
	}
	function edit()
	{
		location.href="<%=rootPath%>/mqchannel.do?action=ready_edit&id="+node;
	}
	function cancelalert()
	{
		location.href="<%=rootPath%>/mqchannel.do?action=cancelalert&id="+node;
	}
	function addalert()
	{
		location.href="<%=rootPath%>/mqchannel.do?action=addalert&id="+node;
	}	
	function clickMenu()
	{
	}
...
<!-- 这里用来定义需要显示的右键菜单 -->
	...
	<table border="1" width="100%" height="100%" bgcolor="#F1F1F1"
		...
		<tr>
			<td style="cursor: default; border: outset 1;" align="center"
				onclick="parent.cancelalert()">取消告警</td>
		</tr>
		<tr>
			<td style="cursor: default; border: outset 1;" align="center"
				onclick="parent.addalert()">添加告警</td>
		</tr>		
	</table>
	</div>
	<!-- 右键菜单结束-->
	...
	<tr>
		...
		<td align="center" valign=top >
			...
			<tr>
				<td background="<%=rootPath%>/common/images/right_t_02.jpg" width="100%"><table width="100%" cellspacing="0" cellpadding="0">
                  <tr>
                    ...
                    <td class="add-content-title">应用 >> 中间件管理 >> MQ通道监视列表</td>
        	...
        	<tr>           
						...
						&nbsp;<input type="button" value="刷新MQ告警配置" class="button" onclick="createMQConfig(operate,'create_mqconfig')">
          							</td>            
...
if(list.size() > 0){
    ...
    {
       MQchannelConfig vo = (MQchannelConfig)list.get(i);
