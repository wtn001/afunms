...
{
	mainForm.action = "<%=rootPath%>/mq.do?action=downloadAllReport&type=queue&str=1&id=<%=tmp%>";
	mainForm.submit();
}
function ping_excel_report()
{
	mainForm.action = "<%=rootPath%>/mq.do?action=downloadAllReport&type=queue&str=0&id=<%=tmp%>";
	mainForm.submit();
}
function ping_pdf_report()
{
	mainForm.action = "<%=rootPath%>/mq.do?action=downloadAllReport&type=queue&str=2&id=<%=tmp%>";
	mainForm.submit();
}
function ping_ok()
{
									...
									<td>
										<table id="win-content-header" class="win-content-header">
				                			<tr>
							                	...
							                	<td class="win-content-title" style="align:center">&nbsp;MQ(<%=vo.getIpaddress() %>)队列报表</td>
