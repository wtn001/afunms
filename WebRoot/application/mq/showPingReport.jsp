...
{
	mainForm.action = "<%=rootPath%>/servicecapreport.do?action=PublicdownloadPingReport&ipaddress=<%=ipaddress%>&str=1&type=mq";
	mainForm.submit();
}
function ping_excel_report()
{
	mainForm.action = "<%=rootPath%>/servicecapreport.do?action=PublicdownloadPingReport&ipaddress=<%=ipaddress%>&str=0&type=mq";
	mainForm.submit();
}
function ping_pdf_report()
{
	mainForm.action = "<%=rootPath%>/servicecapreport.do?action=PublicdownloadPingReport&ipaddress=<%=ipaddress%>&str=2&type=mq";
	mainForm.submit();
}
function ping_ok()
{
	...
	mainForm.action = "<%=rootPath%>/mq.do?action=showPingReport&ipaddress=<%=ipaddress%>&id=<%=id%>&startdate="+starttime+"&todate="+endtime;
	mainForm.submit();
}
				...
				<table id="container-main-win" class="container-main-win">
									...
									<td>
										<table id="win-content-header" class="win-content-header">
				                			<tr>
							                	...
							                	<td class="win-content-title" style="align:center">&nbsp;MQ(<%=newip %>)连通率报表</td>
					...
					<tr>
						<td colspan="4" align="center">
							<img src="<%=rootPath%>/resource/image/jfreechart/<%=newip %>mqpingConnect.png"/><br><br>
