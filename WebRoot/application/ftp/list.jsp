...
<%
    ...
    List<FTPConfig> userFTPConfigList = (List<FTPConfig>) request.getAttribute("userFTPConfigList");
%>
<%
...
<html>
  ...
  function toAdd()
  {
     mainForm.action = "<%=rootPath%>/FTP.do?action=ready_add";
     mainForm.submit();
  }
  function toDelete()
  {
     if(confirm('是否确定删除这条记录?')) {
     mainForm.action = "<%=rootPath%>/FTP.do?action=delete";
     mainForm.submit();
     }
  }  
</script>
