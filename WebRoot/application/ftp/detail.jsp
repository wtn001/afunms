...
<%
  ...
  FTPConfig queryconf = (FTPConfig)request.getAttribute("initconf");
	...
	int status = 0;
	Node node = (Node) PollingEngine.getInstance().getFtpByID(queryconf.getId());
	String alarmmessage = "";
	if (node != null) {
...
//    ResourceCenter.getInstance().setManagerMap(managerMap);
  ...
  int alarmlevel = SystemSnap.getNodeStatus(node);
...
function setClass(){
	document.getElementById('ftpDetailTitle-0').className='detail-data-title';
	document.getElementById('ftpDetailTitle-0').onmouseover="this.className='detail-data-title'";
	document.getElementById('ftpDetailTitle-0').onmouseout="this.className='detail-data-title'";
}
...
function show_graph(){
      ...
      mainForm.action = "<%=rootPath%>/FTP.do?action=detail";
		...
		<table id="body-container" class="body-container">
			<tr>
													...
													<td>
														<table id="application-detail-content-header" class="application-detail-content-header">
										                	<tr>
											                	...
											                	<td class="application-detail-content-title">FTP 详细信息</td>
				        											...
				        											<table>
										               					<tr>
										     								<td>
																				...
																				<input type="button" id = "1" onclick="show_graph()" class=button value="查询">&nbsp;&nbsp;&nbsp; <br>
																			 			...
																			 			<td width="60%" align="center"> 
																			 				<table style="BORDER-COLLAPSE: collapse"
																									...
																									<tr bgcolor="#F1F1F1">
																										...
																										<td width="70%">
																											<img src="<%=rootPath%>/resource/<%=NodeHelper.getCurrentStatusImage(alarmlevel)%>" border="0" >&nbsp;&nbsp;<%=NodeHelper.getStatusDescr(alarmlevel)%>
																										</td>
																									</tr>
																									<tr>
																			 				...
																			 				<!--<table width="100%" cellspacing="0" cellpadding="0" align="center">
																					            	<tr> 
																					         		<td width="100%" align="center"> 
																										...
																										<script type="text/javascript">
																											var so = new SWFObject("<%=rootPath%>/flex/Ftp_Info.swf?names=<%=queryconf.getName()%>&filename=<%=queryconf.getFilename()%>&ip=<%=queryconf.getIpaddress()%>&lasttime=<%=request.getAttribute("lasttime")%>&nexttime=<%=request.getAttribute("nexttime")%>", "Ftp_Info", "400", "250", "8", "#ffffff");
																				        	...
																				        	<table width="100%" cellspacing="0" cellpadding="0" align="center">
																					            <tr> 
																					         		<td width="100%" align="center"> 
																										...
																										<script type="text/javascript">
																											var so = new SWFObject("<%=rootPath%>/flex/Ftp_Ping_Line.swf?id=<%=queryconf.getId()%>", "Ftp_Ping_Line", "400", "250", "8", "#ffffff");
				...
				<td width=15% valign=top>
															<jsp:include page="/include/ftptoolbar.jsp">
...
{  
  ...
  mainForm.action = "<%=rootPath%>/FTP.do?action=sychronizeData&id=<%=queryconf.getId()%>&flag=<%=flag_1%>";
  mainForm.submit();
 });    
});
</script>
