...
{
	mainForm.action = "<%=rootPath%>/servicecapreport.do?action=publicdownloadPingReport&ipaddress=<%=ipaddress%>&str=1&type=ftp";
	mainForm.submit();
}
function ping_excel_report()
{
	mainForm.action = "<%=rootPath%>/servicecapreport.do?action=publicdownloadPingReport&ipaddress=<%=ipaddress%>&str=0&type=ftp";
	mainForm.submit();
}
function ping_pdf_report()
{
	mainForm.action = "<%=rootPath%>/servicecapreport.do?action=publicdownloadPingReport&ipaddress=<%=ipaddress%>&str=2&type=ftp";
	mainForm.submit();
}
function ping_ok()
{
	...
	mainForm.action = "<%=rootPath%>/ftp.do?action=showPingReport&ipaddress=<%=ipaddress%>&id=<%=id%>&startdate="+starttime+"&todate="+endtime;
	mainForm.submit();
}
				...
				<table id="container-main-win" class="container-main-win">
					...
					<tr>
						<td colspan="4" align="center">
							<img src="<%=rootPath%>/resource/image/jfreechart/<%=newip %>ftppingConnect.png"/>
						</td>
					</tr>
				</table>
