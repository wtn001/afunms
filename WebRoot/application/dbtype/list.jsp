...
<script language="JavaScript" type="text/javascript">
  ...
  function toAdd()
  {
     mainForm.action = "<%=rootPath%>/dbtype.do?action=ready_add";
     mainForm.submit();
  }
  function toDelete()
  {
     mainForm.action = "<%=rootPath%>/dbtype.do?action=delete";
     mainForm.submit();
  }  
...
<script language="JavaScript">
	...
	*/
	...
	function toEdit(){
  		mainForm.action = "<%=rootPath%>/dbtype.do?action=ready_edit&id="+node;
  		mainForm.submit();
  }
</script>
			...
			<tr>
				<td background="<%=rootPath%>/common/images/right_t_02.jpg" width="100%"><table width="100%" cellspacing="0" cellpadding="0">
                  <tr>
                    ...
                    <td class="add-content-title">应用 >> 数据库管理 >> 数据库类型列表</td>
		...
		<tr >
			<td colspan="2">
				<table cellspacing="1" cellpadding="0" width="100%" >
					...
					<th width='5%'></th>
    					<th width='5%'>序号</th>
    					<th width='30%'>数据库类型</th>
    					<th width='30%'>数据库类型描述</th>
    					<th width='30%'>操作</th>
					</tr>
<%
if(rc > 0){
    for(int i=0;i<rc;i++)
    {
       DBTypeVo vo = (DBTypeVo)list.get(i);
%>
       <tr bgcolor="#FFFFFF" <%=onmouseoverstyle%> class="microsoftLook">
		...
		<td height=25><%=vo.getDbtype()%></td> 
		<td height=25><%=vo.getDbdesc()%></td> 		
		<td height=25>
