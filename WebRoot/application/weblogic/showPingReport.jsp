...
{
	mainForm.action = "<%=rootPath%>/servicecapreport.do?action=PublicdownloadPingReport&ipaddress=<%=ipaddress%>&str=1&type=weblogic";
	mainForm.submit();
}
function ping_excel_report()
{
	mainForm.action = "<%=rootPath%>/servicecapreport.do?action=PublicdownloadPingReport&ipaddress=<%=ipaddress%>&str=0&type=weblogic";
	mainForm.submit();
}
function ping_pdf_report()
{
	mainForm.action = "<%=rootPath%>/servicecapreport.do?action=PublicdownloadPingReport&ipaddress=<%=ipaddress%>&str=2&type=weblogic";
	mainForm.submit();
}
function ping_ok()
{
	...
	mainForm.action = "<%=rootPath%>/weblogic.do?action=showPingReport&ipaddress=<%=ipaddress%>&id=<%=id%>&startdate="+starttime+"&todate="+endtime;
	mainForm.submit();
}
				...
				<table id="container-main-win" class="container-main-win">
					...
					<tr>
						<td colspan="4" align="center">
							<img src="<%=rootPath%>/resource/image/jfreechart/<%=newip %>weblogicpingConnect.png"/>
						</td>
					</tr>
				</table>
