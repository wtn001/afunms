...
{
	mainForm.action = "<%=rootPath%>/servicecapreport.do?action=PublicdownloadPingReport&ipaddress=<%=ipaddress%>&str=1&type=jboss";
	mainForm.submit();
}
function ping_excel_report()
{
	mainForm.action = "<%=rootPath%>/servicecapreport.do?action=PublicdownloadPingReport&ipaddress=<%=ipaddress%>&str=0&type=jboss";
	mainForm.submit();
}
function ping_pdf_report()
{
	mainForm.action = "<%=rootPath%>/servicecapreport.do?action=PublicdownloadPingReport&ipaddress=<%=ipaddress%>&str=2&type=jboss";
	mainForm.submit();
}
function ping_ok()
{
	...
	mainForm.action = "<%=rootPath%>/jboss.do?action=showPingReport&ipaddress=<%=ipaddress%>&id=<%=id%>&startdate="+starttime+"&todate="+endtime;
	mainForm.submit();
}
				...
				<table id="container-main-win" class="container-main-win">
					...
					<tr>
						<td colspan="4" align="center">
						    <%System.out.println("/resource/image/jfreechart/"+newip+"jbosspingConnect.png"); %>
							<img src="<%=rootPath%>/resource/image/jfreechart/<%=newip %>jbosspingConnect.png"/><br><br>
