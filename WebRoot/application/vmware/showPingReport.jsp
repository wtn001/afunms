...
{
	mainForm.action = "<%=rootPath%>/servicecapreport.do?action=PublicdownloadPingReport&ipaddress=<%=ipaddress%>&str=1&type=VMWare";
	mainForm.submit();
}
function ping_excel_report()
{
	mainForm.action = "<%=rootPath%>/servicecapreport.do?action=PublicdownloadPingReport&ipaddress=<%=ipaddress%>&str=0&type=VMWare";
	mainForm.submit();
}
function ping_pdf_report()
{
	mainForm.action = "<%=rootPath%>/servicecapreport.do?action=PublicdownloadPingReport&ipaddress=<%=ipaddress%>&str=2&type=VMWare";
	mainForm.submit();
}
function ping_ok()
{
	...
	mainForm.action = "<%=rootPath%>/vmware.do?action=showPingReport&ipaddress=<%=ipaddress%>&id=<%=id%>&startdate="+starttime+"&todate="+endtime;
	mainForm.submit();
}
				...
				<table id="container-main-win" class="container-main-win">
									...
									<td>
										<table id="win-content-header" class="win-content-header">
				                			<tr>
							                	...
							                	<td class="win-content-title" style="align:center">&nbsp;VMWare(<%=newip %>)连通率报表</td>
					...
					<tr>
						<td colspan="4" align="center">
							<img src="<%=rootPath%>/resource/image/jfreechart/<%=newip %>vmwarePingConnect.png"/>
						</td>
					</tr>
				</table>
