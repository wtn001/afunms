...
<html>
  ...
  function toAdd()
  {
     mainForm.action = "<%=rootPath%>/dhcp.do?action=ready_add";
     mainForm.submit();
  }
  function toDelete()
  {
   if(confirm('是否确定删除这条记录?')) {
     mainForm.action = "<%=rootPath%>/dhcp.do?action=delete";
     mainForm.submit();
    }
  }  
</script>
...
function initmenu()
{
										...
										<%
										    if (list.size() > 0) {
										        for (int i = 0; i < list.size(); i++) {
										            ...
										            Node node = PollingEngine.getInstance().getDHCPByID(vo.getId());
										            String alarmmessage = "";
										            int status = 0;
										            if (node != null) {
	...
	{
	    location.href="<%=rootPath%>/dhcp.do?action=detail&id="+node;
	}
	function edit(node)
	{
		location.href="<%=rootPath%>/dhcp.do?action=ready_edit&id="+node;
	}
	function cancelalert(node)
	{
		location.href="<%=rootPath%>/dhcp.do?action=cancelalert&id="+node;
	}
	function addalert(node)
	{
		location.href="<%=rootPath%>/dhcp.do?action=addalert&id="+node;
	}	
	//右键菜单方法结束
	</script>
</HTML>
