...
{
	mainForm.action = "<%=rootPath%>/servicecapreport.do?action=publicdownloadPingReport&ipaddress=<%=ipaddress%>&str=1&type=dhcp";
	mainForm.submit();
}
function ping_excel_report()
{
	mainForm.action = "<%=rootPath%>/servicecapreport.do?action=publicdownloadPingReport&ipaddress=<%=ipaddress%>&str=0&type=dhcp";
	mainForm.submit();
}
function ping_pdf_report()
{
	mainForm.action = "<%=rootPath%>/servicecapreport.do?action=publicdownloadPingReport&ipaddress=<%=ipaddress%>&str=2&type=dhcp";
	mainForm.submit();
}
function ping_ok()
{
	...
	mainForm.action = "<%=rootPath%>/dhcp.do?action=showPingReport&ipaddress=<%=ipaddress%>&id=<%=id%>&startdate="+starttime+"&todate="+endtime;
	mainForm.submit();
}
				...
				<table id="container-main-win" class="container-main-win">
									...
									<td>
										<table id="win-content-header" class="win-content-header">
				                			<tr>
							                	...
							                	<td class="win-content-title" style="align:center">&nbsp;DHCP(<%=newip %>)连通率报表</td>
					...
					<tr>
						<td colspan="4" align="center">
							<img src="<%=rootPath%>/resource/image/jfreechart/<%=newip %>dhcppingConnect.png"/>
						</td>
					</tr>
				</table>
