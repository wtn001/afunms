...
<script language="javascript">	
  ...
  function toAdd()
  {
      mainForm.action = "<%=rootPath%>/nas.do?action=ready_add";
      mainForm.submit();
  }
  ...
  {
     if(confirm('是否删除设备？')){
     	mainForm.action = "<%=rootPath%>/nas.do?action=delete";
   	    mainForm.submit();
     }
  }
...
<script language="JavaScript">
	...
	*/
	...
	function detail()
	{
	    mainForm.action ="<%=rootPath%>/nas.do?action=detail&id="+node;
	    mainForm.submit();
	}
	function edit()
	{
		mainForm.action ="<%=rootPath%>/nas.do?action=ready_edit&id="+node;
		mainForm.submit();
	}
	function cancelmanage()
	{
		mainForm.action ="<%=rootPath%>/nas.do?action=cancelalert&id="+node;
		mainForm.submit();
	}
	function addmanage()
	{
		mainForm.action ="<%=rootPath%>/nas.do?action=addalert&id="+node;
		mainForm.submit();
	}	
	function clickMenu()
	{
			...
			<tr>
				<td background="<%=rootPath%>/common/images/right_t_02.jpg" width="100%"><table width="100%" cellspacing="0" cellpadding="0">
                  <tr>
                    ...
                    <td class="add-content-title">应用 >> 服务管理 >> Nas监视列表</td>
