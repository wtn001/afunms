...
<%
  ...
  List webloginlist = (List)request.getAttribute("webloginlist");//web虚拟登陆
%>
	...
	*/
	...
	"</table>";
		...
		document.getElementById('itemMenu').innerHTML="<table border='1' width='100%' height='100%' bgcolor='#F1F1F1'"+
			...
			"</tr>"+
		"</table>";
		}else if(ser == "weblogin"){
			document.getElementById('itemMenu').innerHTML="<table border='1' width='100%' height='100%' bgcolor='#F1F1F1'"+
			"style='border: thin;font-size: 12px' cellspacing='0'>"+
			"<tr>"+
				"<td style='cursor: default; border: outset 1;' align='center'"+
					"onclick='parent.webloginedit()'>修改信息</td>"+
			"</tr>"+
			"<tr>"+
				"<td style='cursor: default; border: outset 1;' align='center'"+
					"onclick='parent.weblogindetail();'>监视信息</td>"+
			"</tr>"+
			"<tr>"+
				"<td style='cursor: default; border: outset 1;' align='center'"+
					"onclick='parent.weblogincancelmanage()'>取消监视</td>"+
			"</tr>"+
			"<tr>"+
				"<td style='cursor: default; border: outset 1;' align='center'"+
					"onclick='parent.webloginaddmanage()'>添加监视</td>"+
			"</tr>"+
		"</table>";
		}
		...
		<td align="center" valign=top bgcolor="#ababab">
					...
					<td>
						<table cellspacing="1" cellpadding="0" width="100%">
							<tr>
								...
								<tr class="microsoftLook0" height=28 align="center">
      								<th width='5%' align="center" class="report-data-body-title"><font color="#397DBD">序号</font></th>				
      								<th width='10%' align="center" class="report-data-body-title"><font color="#397DBD">名称</font></th>
      								<th width='35%' align="center" class="report-data-body-title"><font color="#397DBD">URL</font></th>
      								<th width='15%' align="center" class="report-data-body-title"><font color="#397DBD">关键字</font></th>
      								<th width='10%' align="center" class="report-data-body-title"><font color="#397DBD">当前状态</font></th>      
      								<th width='10%' align="center" class="report-data-body-title"><font color="#397DBD">是否监控</font></th>
      								<th width='10%' align="center" class="report-data-body-title"><font color="#397DBD">操作</font></th>
							</tr>
        	    	<%
    			webloginConfig weblvo = null;
   				 for(int i=0;i<webloginlist.size();i++)
    			{
       				weblvo = (webloginConfig)webloginlist.get(i);
       				int status = 0;
       				Node node = (Node)PollingEngine.getInstance().getWebLoginByID(weblvo.getId());
       				String alarmmessage = "";
       				if(node != null){
			...
			%>
   										<tr  bgcolor="FFFFFF" class="microsoftLook" height="25">
    											<td   height=25  class="report-data-body-list">
    												<font color='blue'><%=1 + i%></font></td>
    											<td  align='center'  height=25  class="report-data-body-list"" ><%=weblvo.getAlias()%></td>
    											<td  align='center'  height=25  class="report-data-body-list"><font color="#397DBD"><%= weblvo.getUrl()%></font></td>
    											<td  align='center'  height=25  class="report-data-body-list"><%=weblvo.getKeyword()%></td>
    											<td align='center'  height=25  class="report-data-body-list"><img src="<%=rootPath%>/resource/<%=NodeHelper.getCurrentStatusImage(status)%>" border="0" alt=<%=alarmmessage%>></td>
											<%
												if(weblvo.getFlag().equals("0")){
											%>
											<td  align='center'  height=25  class="report-data-body-list">否</td>
											<% 
												//if(vo.getFlag()==1);
											...
											%>
											 ...
											 %>
											<td height=25  height=25  class="report-data-body-list">&nbsp;
												<img src="<%=rootPath%>/resource/image/status.gif" border="0" width=15 oncontextmenu=showMenu('weblogin','2','<%=weblvo.getId()%>','')>
											</td>											 
  										</tr>			
								<%;} %>	
