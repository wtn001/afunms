				...
				<td class="td-container-main">
													...
													<td>
														<table id="add-content-header" class="add-content-header">
										                	<tr>
											                	...
											                	<td class="add-content-title">设置LoopBack地址</td>
					...
					<TBODY>
							...
							<INPUT type="button" value="关闭窗口" id=button1 name=button1 onclick="window.close()" class="button"> 
							    ...
							    <td class="report-data-body-title"><strong>LoopBack地址</strong></td>
							    </tr>
							   <%
  	                                               int index = 0;
                                 %>
                                    <%
                                       	if(list != null && list.size() > 0) {
	                                         	for(int i = 0; i < list.size(); i++){
	                                         	 loopbackstorage vo = (loopbackstorage)list.get(i);
  		                      	%>
  		                                 	<tr  class="othertr" <%=onmouseoverstyle%> ondblclick="submitCheck1('<%=vo.getId()%>','dbclicksubmit')">
  			
							    				<td class="report-data-body-list" height="20">
							    					<input type=checkbox name=checkbox value='<%=vo.getLoopback()%>' class=noborder onclick="submitCheck1('<%=vo.getId()%>','checkthisbox')" >
							    				</td>  	
							    				<td class="report-data-body-list" onclick="submitCheck1('<%=vo.getId()%>','checkthis')">&nbsp;<%=i+1%></td>
							                    <td class="report-data-body-list" onclick="submitCheck1('<%=vo.getId()%>','checkthis')">       
							                     <%=vo.getLoopback()%></td>			
							  			<%	
							  		}
							  	}
