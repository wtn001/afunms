...
//	},
	...
	initComponent:function() {
		...
		this.btn_today ={//@@@今天
			...
			cls: 'x-btn-text-icon', 
			text: e2cs.cal_locale.todayLabel,//@@@todayLabel: '今天',
			icon: this.iconToday, 
			tooltip: e2cs.cal_locale.todayToolTip//@@@todayToolTip: '设置今天日期...',
		};
		
		this.btn_monthviewchange = { //@@@查看月视图 这个可以取消
			id: this.id + '-btn_monthview', 
			cls: 'x-btn-icon',	
			text: '', 
			icon: this.iconMonthView, 
			tooltip: e2cs.cal_locale.tooltipMonthView //@@@tooltipMonthView:	'查看月视图...',
		};
		//zys
		this.btn_print = { //@@@打印
			id: this.id + '-btn_print', 
			cls: 'x-btn-icon',	
			text: '', 
			icon: this.iconMonthView, 
			handler : this.print,
			tooltip: e2cs.cal_locale.tooltipPrint //@@@tooltipPrint:	'打印...',
		};
		
		this.btn_weekviewchange =  { //@@@查看周视图 这个根本没有
			id: this.id + '-btn_weekview',	 
			cls: 'x-btn-icon',	
			text: '', 
			icon: this.iconWeekView,	
			tooltip: e2cs.cal_locale.tooltipWeekView  //@@@tooltipWeekView:	'查看周视图...',
		};
		
		this.btn_dayviewchange =   { //@@@查看日视图 这个根本没有
			id: this.id + '-btn_dayview',	 
			cls: 'x-btn-icon',	
			text: '', 
			icon: this.iconDayView, 	
			tooltip: e2cs.cal_locale.tooltipDayView //@@@tooltipDayView:		'查看日视图...',	
		};
		
		this.btn_schedviewchange = { //@@@查看调度视图 这个根本没有
			id: this.id + '-btn_sched_view',	 
			cls: 'x-btn-icon',	
			text: '', 
			icon: this.iconSchedView, 	
			tooltip: e2cs.cal_locale.tooltipSchedView  //@@@ tooltipSchedView:   '查看调度视图...',		//0.0.10 new view  
		};
		
		this.btn_listviewchange = { //@@@查看列表视图 本来显示是extjs，现在为jsp
			id: this.id + '-btn_listview', 
			cls: 'x-btn-icon',	
			text: '', 
			icon: this.iconListView, 
			tooltip: e2cs.cal_locale.tooltipListView //@@@tooltipListView:    '查看列表视图...', 
		};
		
		if (this.showCal_tbar){ //@@@ 工具条 显示今天等几个按钮
			this.tbar_calendar= new Ext.Toolbar({	
				...
				items:[ this.btn_today ,'-',this.btn_monthviewchange,this.btn_listviewchange, '-', this.btn_print]  
			});	
		} else { 
	...
	print : function(t) {
			window.open("/afunms/schedule/scheduling/calendarPrint.jsp","打印");
		},
...
//				});	
		...
		if (this.dateSelector && this.showCal_tbar){
			...
			this.btn_selector  = {
					...
					cls: 'x-btn-text-icon',	
					text: e2cs.cal_locale.dateSelectorText,//@@@dateSelectorText:    '选择..',
					icon: this.dateSelectorIcon,
					tooltip: e2cs.cal_locale.dateSelectorTooltip,//@@@dateSelectorTooltip: 'e2cs_cn.js..',
...
//						modify tyl 
						daystoreturn+=' ext:qtitle="' + storex.getAt(itask).data[this.calx.fieldsRefer.username] + '" ';
						var datatip = {
						...
						var mylist_itemTip =  this.calx.tplTaskTip.apply(datatip);
						daystoreturn+='ext:qtip="' + mylist_itemTip + '">';
						daystoreturn+= storex.getAt(itask).data[this.calx.fieldsRefer.username] + '</div>';
