...
double avgpingcon = (Double)request.getAttribute("pingconavg");
  ...
  //else
     ...
     List trustList = (List)request.getAttribute("trustList"); 	
     System.out.println("===="+request.getAttribute("trustflag"));
     int trustflag = 0;
     try{
     	trustflag = java.lang.Integer.parseInt((String)request.getAttribute("trustflag")); 
     }catch(Exception e){
     	e.printStackTrace();
     }	
					...
					<table width="100%" style="BORDER-COLLAPSE: collapse" borderColor=#cedefa cellPadding=0 rules=none align=center border=1 algin="center">
                  				...
                  				<tr>
										...
										<td width="80" height="22" background="<%=rootPath%>/resource/image/anjian.gif">
											<div align="center"><a href="<%=rootPath%>/monitor.do?action=firewallpolicy&id=<%=tmp%>&ipaddress=<%=host.getIpAddress()%>&trustflag=0"><font color="#FFFFFF">策略信息</font></a></div></td>	
                								<td width="80" height="22" background="<%=rootPath%>/resource/image/anjian_1.gif">
										...
										<td width="80" height="22" background="<%=rootPath%>/resource/image/anjian_1.gif">
											<div align="center"><a href="<%=rootPath%>/monitor.do?action=firewalliplist&id=<%=tmp%>&ipaddress=<%=host.getIpAddress()%>"><font color="#397dbd">IP列表</font></a></div></td>	
                								<td width="80" height="22" background="<%=rootPath%>/resource/image/anjian_1.gif">
              ...
              <tr> 
                ...
                <table width="100%" height="1" border="1" cellpadding="0" cellspacing="0" bordercolor="397DBD">
                    <tr align="left" bgcolor="397dbd"> 
                      <td width="50%" height="23">&nbsp;<b><font color="#FFFFFF">>> 策略信息</b></td> 
                      <% if (trustflag == 0){
                      %>  
                      <td width="50%" height="23" align=right><b><font color="#FFFFFF"><a href="<%=rootPath%>/monitor.do?action=firewallpolicy&id=<%=tmp%>&ipaddress=<%=host.getIpAddress()%>&trustflag=1">查看 From trust to untrust </a></b>&nbsp;&nbsp;</td>                    
                    <%
                    	}else{
                    %>
                    <td width="50%" height="23" align=right><b><font color="#FFFFFF"><a href="<%=rootPath%>/monitor.do?action=firewallpolicy&id=<%=tmp%>&ipaddress=<%=host.getIpAddress()%>&trustflag=0">查看 From untrust to trust </a></b>&nbsp;&nbsp;</td>
                    <%
                    	}
                    %>
                    </tr>
                    ...
                    <%
                    	if(trustList != null && trustList.size()>0){
                    		for(int k=0;k<trustList.size();k++){
                    			Policydata policy = (Policydata)trustList.get(k);
                    %>                                           										                        								
                    		<tr height=25 align="left" valign="center" class="othertr"  <%=onmouseoverstyle%>> 
                    			<td><%=k+1%></td>
                      			<td width=10% valign="center">&nbsp;&nbsp;<%=(String)policy.getPid()%></td>
                      			<td width=30% valign="center">&nbsp;&nbsp;<%=(String)policy.getSource()%></td>
                      			<td width=30% valign="center">&nbsp;&nbsp;<%=(String)policy.getDestination()%></td>
                      			<td width=30% valign="center">&nbsp;&nbsp;<%=(String)policy.getServices()%></td>
                   <%
                   		}
                   	}
                   %>                     		                   										                 										                      								
