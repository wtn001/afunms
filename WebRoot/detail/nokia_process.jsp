...
<%
	...
	if ("0".equals(runmodel)) {
		...
		if (ipAllData != null) {
			...
			processHash = (Hashtable) ipAllData.get("process");
			//得到系统启动时间
																...
																<tr bgcolor="#ECECEC">
																	<td>
																		...
																		<tr style="background-color: #ECECEC;" height=28>	
											          					    <td align=center width=10%>进程ID</td>
											            					<td align=center width=10%>父进程ID</td>
											            					<td align=center width=15%>进程名称</td>
											            					<td align=center width=25%>用户</td>
											            					<td align=center width=10%>内存利用率</td>
											            					<td align=center width=15%>内存占有量</td>
											            					<td align=center width=15%>CPU占用率</td>
											            				</tr>  
																			<%
																			    System.out.println(processHash);
																				if (processHash != null && processHash.size() > 0) {
																				    Enumeration enkey=processHash.keys();
																				    while(enkey.hasMoreElements()){ 
																				        ...
																				        Vector imageVector=(Vector)processHash.get(aa);
																				        for (int j = 0; j < imageVector.size(); j++){
																				            Processcollectdata imagedata = (Processcollectdata) imageVector.get(j);
																				            processID = imagedata.getSubentity();
																				            processName  = imagedata.getChname();
																				            if("MemoryUtilization".equals(imagedata.getEntity())){
																				                MemoryUtilization = imagedata.getThevalue()+imagedata.getUnit();
																				            } 
																				            if("Memory".equals(imagedata.getEntity())){
																				                Memory = imagedata.getThevalue()+imagedata.getUnit();
																				            }
																				            if("PPID".equals(imagedata.getEntity())){
																				                PPID = imagedata.getThevalue()+imagedata.getUnit();
																				            }
																				            if("User".equals(imagedata.getEntity())){
																				                User = imagedata.getThevalue()+imagedata.getUnit();
																				            }
																				            if("CpuUtilization".equals(imagedata.getEntity())){
																				                CpuUtilization = imagedata.getThevalue()+imagedata.getUnit();
																				            }
																				        }
																			%>
																			<tr height=25 align="center" valign="center"  bgcolor="#FFFFFF" <%=onmouseoverstyle%>> 
												                    			<td align="center">&nbsp;&nbsp;<%=processID%></td>
												                      			<td align="center">&nbsp;&nbsp;<%=PPID%></td>
												                      			<td align="center">&nbsp;&nbsp;<%=processName%></td>
												                      			<td align="center">&nbsp;&nbsp;<%=User%></td>
												                      			<td align="center">&nbsp;&nbsp;<%=MemoryUtilization%></td>
												                      			<td align="center">&nbsp;&nbsp;<%=Memory%></td>
												                      			<td align="center">&nbsp;&nbsp;<%=CpuUtilization%></td>
												                      		</tr>
																			<%
																					}
																				}
