...
double avgpingcon = (Double)request.getAttribute("pingconavg");
  ...
  System.out.println(avgpingcon+"===================================1");
...
setTimeout(function(){
 ...
 Ext.get("delallprocess").on("click",function(){
  
  Ext.MessageBox.wait('数据加载中，请稍后.. ');   
  mainForm.action = "<%=rootPath%>/macmanager.do?action=deleteall&id=<%=tmp%>";
  mainForm.submit();
 });
  ...
  Ext.MessageBox.wait('数据加载中，请稍后.. ');   
  mainForm.action = "<%=rootPath%>/macmanager.do?action=refresh&id=<%=tmp%>";
  mainForm.submit();
 });
 
   Ext.get("process2").on("click",function(){
					...
					<table width="100%" style="BORDER-COLLAPSE: collapse" borderColor=#cedefa cellPadding=0 rules=none align=center border=1 algin="center">
								...
								<table width="100%" style="BORDER-COLLAPSE: collapse" borderColor=#cedefa cellPadding=0 rules=none w align=center border=1 algin="center">
											...
											<table width="100%" style="BORDER-COLLAPSE: collapse" borderColor=#cedefa cellPadding=0 rules=none w align=center border=1 algin="center">
                        								<tbody>
                          									<tr>
                											<td width="400" align="left" valign="top" class=dashLeft>
					...
					<table width="100%" style="BORDER-COLLAPSE: collapse" borderColor=#cedefa cellPadding=0 rules=none align=center border=1 algin="center">
                  				...
                  				<tr>
										...
										<td width="80" height="22" background="<%=rootPath%>/resource/image/anjian_1.gif">
											...
											<div align="center"><a href="<%=rootPath%>/monitor.do?action=firewallarp&id=<%=tmp%>&ipaddress=<%=host.getIpAddress()%>"><font color="#FFFFFF">ARP信息</font></a></div></td>
                								<td width="80" height="22" background="<%=rootPath%>/resource/image/anjian_1.gif">
                						...
                						<table width="100%" border="0" cellpadding="0" cellspacing="1">
                    							<tr align="left" bgcolor="397dbd"> 
                      								<td height="23" width=70%><b><font color="#FFFFFF">&nbsp;ARP表信息&nbsp;&nbsp;</font></b></td>
                      								<td height="23">&nbsp;<a href="#" id="process1" onclick="#"><b><font color="#FFFFFF">刷新</font></b></a>&nbsp;&nbsp;&nbsp;<a href="#" id="delallprocess" onclick="#"><b><font color="#FFFFFF">从设备上清除所有MAC</font></b></a></td>
                    							</tr>                  							
                    							<tr bgcolor="DEEBF7"> 
                      								<td colspan=2>
  												...
  												<tr align="center"> 
  													<td  width="5%" align="left"></td>
    													<td  width="10%" align="left">端口索引</td>
    													<td  width="20%" align="left">IP</td>
    													...
    													<td  width="10%" align=center>从设备上清除</td>
             											...
             											<%
             												for(int i=0 ;i<ipmacvector.size(); i++){
    															...
    															if(macbaseHash.containsKey(ipmac.getMac())){
    																baseflag = 1;
    														}
    												}
             											%>
            											<tr bgcolor="DEEBF7" <%=onmouseoverstyle%>>
            												<td ><%=i+1%></td>
            												<td ><%=ipmac.getIfindex()%></td>
            												<td ><%=ipmac.getIpaddress()%></td>
            												<td ><%=ipmac.getMac()%></td>
            												<td ><%=sdf.format(cc)%></td>
            												<%
            												if(baseflag == 0){
            												%>
            												<td  align='left' bgcolor=#99CC00><a href="<%=rootPath%>/ipmac.do?action=setmacbase&relateip=<%=host.getIpAddress()%>&ifindex=<%=ipmac.getIfindex()%>&macip=<%=ipmac.getIpaddress()%>&mac=<%=ipmac.getMac()%>&flag=1"><font color=blue>设为基线</font></a></td>
            												<%
            												}else{
            												%>
            												<td  align='left'><a href="<%=rootPath%>/ipmac.do?action=cancelmacbase&relateip=<%=host.getIpAddress()%>&index=<%=ipmac.getIfindex()%>&ip=<%=ipmac.getIpaddress()%>&mac=<%=ipmac.getMac()%>&flag=1"><font color=blue>取消基线</font></a></td>
            												<%}%>
            												<td ><a href="<%=rootPath%>/macmanager.do?action=delete&id=<%=tmp%>&macip=<%=ipmac.getIpaddress()%>&index=<%=ipmac.getIfindex()%>">删除</a></td>            
            											</tr>
            											<%}%>            
                        								</table>
                        							</td>
