...
<%
	...
	Vector QueueStatueVector = new Vector(); 
	Vector QueueMessageVector = new Vector(); 
	...
	if ("0".equals(runmodel)) {
		...
		if (ipAllData != null) {
			...
			queueVector = (Vector) ipAllData.get("queue");
			QueueStatueVector = (Vector) ipAllData.get("queuestatue");
			QueueMessageVector = (Vector) ipAllData.get("queuemessage");
			//得到系统启动时间
		...
		if (memoryList != null && memoryList.size() > 0) {
			for (int i = 0; i < memoryList.size(); i++) {
				...
				queueVector.add(memorycollectdata);
			}
		}
																...
																<tr bgcolor="#ECECEC">
																	<td>
																		...
																		<tr style="background-color: #ECECEC;" height=28>	
											          					    <td align=center width=20%>队列空间利用率</td>
											            					<td align=center width=10%>队列状态</td>
											            					<td align=center width=60%>队列信息数</td>
											            				</tr>  
																			<%
																			    String utillize = "0%";
																			    String statue = "";
																				...
																				if (queueVector != null && queueVector.size() > 0) {
																					for (int i = 0; i < queueVector.size(); i++) {
																						Memorycollectdata queuedata = (Memorycollectdata) queueVector.get(0);
																						utillize = queuedata.getThevalue()+queuedata.getUnit();
																					}
																				}
																				if (QueueStatueVector != null && QueueStatueVector.size() > 0) {
																					for (int i = 0; i < QueueStatueVector.size(); i++) {
																						Memorycollectdata queuedata = (Memorycollectdata) QueueStatueVector.get(0);
																						if(queuedata.getThevalue().equals("1")){
																						    statue = "队列有足够的空间";
																						}else if(queuedata.getThevalue().equals("2")){
																						    statue = "队列接近满";
																						}else if(queuedata.getThevalue().equals("3")){
																						    statue = "队列已满";
																						}
																					}
																				}
																				if (QueueMessageVector != null && QueueMessageVector.size() > 0) {
																					for (int i = 0; i < QueueMessageVector.size(); i++) {
																						Memorycollectdata queuedata = (Memorycollectdata) QueueMessageVector.get(0);
																						number = queuedata.getThevalue()+queuedata.getUnit();
																					}
																				}
																			%>
																			<tr height=25 align="center" valign="center"  bgcolor="#FFFFFF" <%=onmouseoverstyle%>> 
												                    			<td align="center">&nbsp;&nbsp;<%=utillize%></td>
