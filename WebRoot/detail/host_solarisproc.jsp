...
<%
	...
	String[] processItem={"process_id","Name","Type","CpuTime","MemoryUtilization","Memory","Status"};
...
function order(para){
	...
	mainForm.action = "<%=rootPath%>/monitor.do?action=hostproc&id=<%=host.getId()%>&flag=<%=_flag%>&orderflag=" + para;
	mainForm.submit();
...
function setClass(){
	document.getElementById('sunDetailTitle-4').className='detail-data-title';
	document.getElementById('sunDetailTitle-4').onmouseover="this.className='detail-data-title'";
	document.getElementById('sunDetailTitle-4').onmouseout="this.className='detail-data-title'";
}
...
<div id="itemMenu" style="display: none";>
	...
	<form id="mainForm" method="post" name="mainForm">
		...
		<table id="body-container" class="body-container">
            ...
            if(i>=3 && i<=5){
            %>
         <td  align="center" height="28"><input type="submit" onClick="order('<%=processItem[i]%>')" Class="button" value="<%=processItemch[i]%>"></td>
            <%}
            else{%>
              <td   align="center" height="28"><%=title%></td>
              <%}
              }
            %>
              <td align="center" height="28">查看进程历史记录</td>
            </tr>
            ...
            %>
              <td   class="detail-data-body-list" align="center"><%=(String)phash.get(processItem[j])%></td>
              <%}%>
            <td  class="detail-data-body-list" align="center"><input type="button"  Class="button"
			onclick='window.open("<%=rootPath%>/monitor.do?action=look_prohis&ipaddress=<%=ipaddress%>&pid=<%=(String)phash.get("process_id")%>&pname=<%=(String)phash.get("Name")%>","newWindow", "toolbar=no,height=300, width= 800, top=300, left= 200")'
			 value="查看历史">
			 </td>
            </tr>
            <%
