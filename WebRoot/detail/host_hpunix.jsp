...
<%
    	...
    	if("0".equals(runmodel)){
	  	...
	  	if (ipAllData != null) {
			...
			try{
				...
				ifvector = interfaceInfoService.getInterfaceInfo(netInterfaceItem); 
			}catch(Exception e){  
				e.printStackTrace();  
			}
	  		//cpu
  		...
  		//排序
		ifvector = CommonUtil.sortInterface(ifvector, orderflag, sorttype, netInterfaceItem);	 
		//if(ipAllData != null){ 
	...
	double avgpingcon = (Double)request.getAttribute("pingconavg")==null?0:(Double)request.getAttribute("pingconavg");
...
function changeOrder(para){ 
  	location.href="<%=rootPath%>/monitor.do?action=hostutilhdx&id=<%=tmp%>&ipaddress=<%=host.getIpAddress()%>&flag=<%=_flag%>&orderflag="+para;
} 
...
function setClass(){
	document.getElementById('hpunixDetailTitle-10').className='detail-data-title';
	document.getElementById('hpunixDetailTitle-10').onmouseover="this.className='detail-data-title'";
	document.getElementById('hpunixDetailTitle-10').onmouseout="this.className='detail-data-title'";
}
