	...
	if("0".equals(runmodel)){
		...
		if(ipAllData != null){
			...
			if(systemV != null && systemV.size()>0){
	          ...
	          vpnlist=(Hashtable)ipAllData.get("hh3comvpn");
	          if(vpnlist==null)
	          {
	          	System.out.println("5555555555555555555555");
	          	vpnlist = new Hashtable();
	          }
	          System.out.println("=ss============================"+vpnlist.toString());
		}
...
{	
        window.open ("<%=rootPath%>/vpntelnetconf.do?action="+operate+"&id=<%=tmp%>&ipaddress=<%=host.getIpAddress()%>&ifindex="+index+"&ifname="+ifname, "newwindow", "height=200, width=800, toolbar= no, menubar=no, scrollbars=yes, resizable=yes, location=yes, status=yes");
} 
function openwin4(operate,index,ifname) 
{	
        window.open ("<%=rootPath%>/vpntelnetconf.do?action="+operate+"&id=<%=tmp%>&ipaddress=<%=host.getIpAddress()%>&ifindex="+index+"&ifname="+ifname, "newwindow", "height=300, width=800, toolbar= no, menubar=no, scrollbars=yes, resizable=yes, location=yes, status=yes");
} 
...
function setClass(){
	document.getElementById('vpnDetailTitle-8').className='detail-data-title';
	document.getElementById('vpnDetailTitle-8').onmouseover="this.className='detail-data-title'";
	document.getElementById('netDetailTitle-8').onmouseout="this.className='detail-data-title'";
}
...
<!-- 右键菜单结束-->
  ...
  <tr align="center"> 
    ...
    <td rowspan="2" width="15%" align="center">描述</td>
  ...
  <tr> 
    ...
    <td   width="15%" align="center"><input type=button  name="button3" styleClass="button" value="出口流速"></td>
    <td   width="15%" align="center"><input type=button  name="button4" styleClass="button" value="入口流速"></td>
  </tr>
             <%
...
//mainForm.action = "<%=rootPath%>/vpntelnetconf.do?action=bkpCfg&ipaddress=<%=host.getIpAddress()%>&page=netevent&id="+<%=tmp %>;
//mainForm.submit();
