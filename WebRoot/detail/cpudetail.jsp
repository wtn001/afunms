...
<html>
	<head>
		...
		<%
			...
			User user=(User)session.getAttribute(SessionConstant.CURRENT_USER);
		 	//String fileName=user.getUserid()+"_"+user.getId()+"_cpu.xml";   
			...
			String ifindex=id; 
			String fileName = user.getUserid() + "_" + ip + "_" + ifindex+ "_cpu.xml"; 
		%>
		...
		<script type="text/javascript" src="<%=rootPath%>/dwr/interface/CPUControler.js"></script>
		<script type="text/javascript" src="<%=rootPath%>/js/MKDateTime.js"></script>
		<script type="text/javascript"> 
		 	// <![CDATA[ 
			//暂停
			function suspendBtn(){
				//window.open('<%=rootPath%>/amChart.do?action=cpuDetailImage&ip=<%=ip%>&ifindex=<%=ifindex%>','amchart','top=200,left=300,height=430,width=660, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no');
				if(oTimer!=null){
					...
					//var flashMovie = document.getElementById('amline');   
		 			//if (flashMovie) {   
	                //	flashMovie.exportImage('<%=rootPath%>/amChart.do?action=createImgAmChart&ip=<%=ip%>&ifindex=<%=ifindex%>&type=cpudetail'); 
	               //		flashMovie.exportImage('<%=rootPath%>/test2.jsp'); 
	              // }   
			}
			//继续
			...
			function excelBtn(){
				var flashMovie = document.getElementById('amline');   
		 		if (flashMovie) {   
	                flashMovie.exportImage('armChartServlet?ip=<%=ip%>&ifindex=<%=ifindex%>&imageName=CPU利用率'); 
	               	//flashMovie.exportImage('test2.jsp?ip=<%=ip%>&ifindex=<%=ifindex%>&imageName=CPU利用率''); 
	            }   
				//window.open ('<%=rootPath%>/amChart.do?action=cpuDetailDoc&ip=<%=ip%>&ifindex=<%=ifindex%>','amchart','top=200,left=300,height=430,width=660, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no');
			}
			//打印
			function printBtn(){ 
				window.print();
	...
	<body onload="generateData();" id="body" class="body flowratebg"><!--   bgcolor="#0c2869" -->
	 	<table align=center id="table1" background="#c1c8d0">
											...
											<script type="text/javascript">
											 // <![CDATA[ 
												...
												function generateData(){   
											      ...
											      CPUControler.generateData(datafileName,blackFlag,nodeID,{
												...
												function printChart(){
													...
													so.addVariable("settings_file", escape("<%=rootPath%>/amcharts_settings/cpu_setting.xml"));
