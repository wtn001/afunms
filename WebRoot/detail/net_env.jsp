...
function setClass(){
	document.getElementById('netDetailTitle-6').className='detail-data-title';
	document.getElementById('netDetailTitle-6').onmouseover="this.className='detail-data-title'";
	document.getElementById('netDetailTitle-6').onmouseout="this.className='detail-data-title'";
}
			...
			<tr>
				...
				<td width="20%" height="26" align="left" nowrap  class=txtGlobal>&nbsp;设备标签:
                         ...
                         <td width="20%" height="26" align=left valign=middle nowrap class=txtGlobal>&nbsp;IP地址:
											<%
												 IpAliasDao ipdao1 = new IpAliasDao();
												 List iplist1 = ipdao1.loadByIpaddress(host.getIpAddress());
												 ipdao1.close();
												 ipdao1 = new IpAliasDao();
												 IpAlias ipalias1 = ipdao1.getByIpAndUsedFlag(host.getIpAddress(),"1");
												 ipdao1.close();
												 if(iplist1 == null)iplist1 = new ArrayList(); %>
											
												<select name="ipalias<%=host.getIpAddress() %>">
													<option selected><%=host.getIpAddress() %></option>
													<% 
													   for(int j=0 ;j<iplist.size() ; j++){
																IpAlias voTemp1 = (IpAlias)iplist1.get(j); 
															%>
															<option <%if(ipalias1 != null && ipalias1.getAliasip().equals(voTemp1.getAliasip())){ %>selected<%} %>><%=voTemp1.getAliasip() %></option>
													<%} %>
												</select>
												[
																				...
																				if(powervector != null && powervector.size()>0){
																					for(int i=0;i<powervector.size();i++){
																						...
																						if("1".equals(interfacedata.getThevalue())){
																							statusstr = "正常";
																						}else if("3".equals(interfacedata.getThevalue())){
																							statusstr = "未安装";
																						}else if("4".equals(interfacedata.getThevalue())){
																							statusstr = "未支持";
																						}
												                      ...
												                      <table cellpadding="0" cellspacing="0" width=48% align=center>
									              							<tr> 
									                							...
									                							<!-- 电压 -->
																					...
																					<script type="text/javascript">
																						var so = new SWFObject("<%=rootPath%>/flex/Envoriment_component.swf?ipadress=<%=host.getIpAddress()%>&key=voltage&table=vol&type=<%=systype%>&title=电压", "Envoriment_component_2", "346", "250", "8", "#ffffff");
												                      ...
												                      <table cellpadding="0" cellspacing="0" width=48%>
												              							<tr> 
												                							...
												                							<!-- 温度 -->
																								...
																								<script type="text/javascript">
																									var so = new SWFObject("<%=rootPath%>/flex/Envoriment_component.swf?ipadress=<%=host.getIpAddress()%>&key=temperature&table=temper&type=<%=systype%>&title=温度", "Envoriment_component_4", "346", "250", "8", "#ffffff");
