...
function changeOrder(para){
  	location.href="<%=rootPath%>/detail/host_linuxutilhdx.jsp?id=<%=tmp%>&ipaddress=<%=host.getIpAddress()%>&flag=<%=_flag%>&orderflag="+para;
} 
...
function setClass(){
	document.getElementById('linuxDetailTitle-1').className='detail-data-title';
	document.getElementById('linuxDetailTitle-1').onmouseover="this.className='detail-data-title'";
	document.getElementById('linuxDetailTitle-1').onmouseout="this.className='detail-data-title'";
}
...
<div id="itemMenu" style="display: none";>
  ...
  <tr>
    <td   width="15%" align="center"><input type=button  name="button3" styleClass="button" value="出口流速" onclick="changeOrder('OutBandwidthUtilHdx')"></td>
    <td   width="15%" align="center"><input type=button  name="button4" styleClass="button" value="入口流速" onclick="changeOrder('InBandwidthUtilHdx')"></td>
  </tr>
             <%
            ...
            <%
            	...
            	if (hash != null && hash.size()>0){
            		...
            		%>
              			<td width="<%=width[j]/2%>%" class="detail-data-body-list"><%=strs[j] == null?"":strs[j]%></td>
              			<td width="<%=width[j]/2%>%" class="detail-data-body-list"><%=linkuse==null?"":linkuse%></td>            		
              		<%            		
            	}else{
            	%>
              			<td width="<%=width[j]/2%>%" class="detail-data-body-list"><%=strs[j]==null?"":strs[j]%></td>
              ...
              %>
              <td width="<%=width[j]%>%" class="detail-data-body-list"><%=strs[j]==null?"":strs[j]%></td>
              <%
              }
              }
            }%>
    <td  width="9%"align="center" class="detail-data-body-list">
