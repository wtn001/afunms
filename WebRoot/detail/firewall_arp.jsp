...
function setClass(){
	document.getElementById('firewallDetailTitle-2').className='detail-data-title';
	document.getElementById('firewallDetailTitle-2').onmouseover="this.className='detail-data-title'";
	document.getElementById('firewallDetailTitle-2').onmouseout="this.className='detail-data-title'";
	

 
  Ext.get("processmac").on("click",function(){
  Ext.MessageBox.wait('数据加载中，请稍后.. ');   
  mainForm.action = "<%=rootPath%>/macmanager.do?action=refresh&id=<%=tmp%>&flag=<%=flag%>&type=<%=type%>";
  mainForm.submit();
 });
  ...
  Ext.MessageBox.wait('数据加载中，请稍后.. ');   
  mainForm.action = "<%=rootPath%>/ipmac.do?action=setmultimacbase&id=<%=tmp%>&flag=1&type=<%=type%>";
  mainForm.submit();
 });
											...
											<table id="detail-data" class="detail-data">
											  ...
											  <tr>
                    							...
                    							<tr align="left" bgcolor="#ECECEC"> 
                      								<td height="28" align="right">&nbsp;<a href="#" id="processmac" onclick="#"><b>刷新<b></a>&nbsp;&nbsp;&nbsp;<a href="#" id="process3" onclick="#"><b>设为基线<b></a>&nbsp;&nbsp;&nbsp;<a href="#" id="delallprocess" onclick="#"><b>从设备上清除所有MAC<b></a>&nbsp;&nbsp;&nbsp;</td>
                    							</tr>                  							
                    							<tr bgcolor="DEEBF7"> 
                      								<td colspan=2>
  												...
  												<tr bgcolor="#ECECEC"> 
  													<td width='10%' border="0"><INPUT type="checkbox" name="checkall" onclick="javascript:chkall()"></td>
    													...
    													<td  width="10%" align="center">基线</td>
            											...
            											<tr bgcolor="#FFFFFF" <%=onmouseoverstyle%>>
            												...
            												if(baseflag == 0){
            												%>
            												<td  align='center' bgcolor=#99CC00><a href="<%=rootPath%>/ipmac.do?action=setmacbase&relateip=<%=host.getIpAddress()%>&ifindex=<%=ipmac.getIfindex()%>&macip=<%=ipmac.getIpaddress()%>&mac=<%=ipmac.getMac()%>&flag=1&type=<%=type %>"><font color=blue>设为基线</font></a></td>
            												<%
            												}else{
            												%>
            												<td  class="detail-data-body-list" align='center'><a href="<%=rootPath%>/ipmac.do?action=cancelmacbase&relateip=<%=host.getIpAddress()%>&index=<%=ipmac.getIfindex()%>&ip=<%=ipmac.getIpaddress()%>&mac=<%=ipmac.getMac()%>&flag=1"><font color=blue>取消基线</font></a></td>
            												<%}%>
            												<td  class="detail-data-body-list" align='center'><a href="<%=rootPath%>/macmanager.do?action=delete&id=<%=tmp%>&macip=<%=ipmac.getIpaddress()%>&index=<%=ipmac.getIfindex()%>&flag=<%=flag1%>&type=<%=type%>">删除</a></td>            
            											</tr>
            											<%}%>            
                        								</table>
                        							</td>
