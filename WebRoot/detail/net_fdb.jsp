...
<%@page import="com.afunms.polling.snmp.Hostlastcollectdata"%>
...
<%
	...
	boolean isCollectedData = true;//是否采集到数据
    ...
    if("0".equals(runmodel)){
		...
		if(ipAllData != null){
			...
			if (ipmacvector == null || ipmacvector.size() == 0 || ((IpMac)ipmacvector.get(0)).getIfindex().equals("unknown")) {
				isCollectedData = false;
				ipmacvector = new Vector();
				HostLastCollectDataManager hostlastmanager = new HostLastCollectDataManager();
		        try{
		        	ipmacvector = hostlastmanager.getFDB(host.getIpAddress()); 
  												...
  												<tr align="center" bgcolor="#ECECEC"> 
  													<td  width="8%" align="left">&nbsp;<INPUT type="checkbox" class=noborder name="checkall" onclick="javascript:chkall()"></td>
    													...
    													<td  width="10%" align="center">IP-端口-MAC</td>
             											...
             											<%
             												...
             												while (it.hasNext()) {
             													...
             													if(ipmac.getIpaddress() !=null && !"null".equals(ipmac.getIpaddress())){
             														ipaddress_formac = ipmac.getIpaddress();
             													}else {
             														ipaddress_formac = "--";}
             											%>
            											<tr bgcolor="#FFFFFF" <%=onmouseoverstyle%>>
            												...
            												<td height=28 class="detail-data-body-list"><%=ipaddress_formac%></td>
            												<td height=28 class="detail-data-body-list"><%=ipmac.getMac()%></td>
            												<td height=28 class="detail-data-body-list"><%=sdf.format(cc)%></td>
           												<%
           												if(isCollectedData == true){
           												%>
            												<% if(ifband ==0){%>
            												<td height=28 class="detail-data-body-list"><a href="<%=rootPath%>/ipmac.do?action=setipmacbase&id=<%=tmp%>&ipaddress=<%=host.getIpAddress()%>&mac=<%=ipmac.getMac()%>&ifindex=<%=ipmac.getIfindex()%>&macip=<%=ipmac.getIpaddress()%>&flag=<%=flag1%>&doflag=-1"><font color='green'>取消基线</font></a></td>
            												...
            												%>  
            												<td height=28 class="detail-data-body-list" ><a href="<%=rootPath%>/ipmac.do?action=cancelipmacbase&id=<%=tmp%>&ipaddress=<%=host.getIpAddress()%>&mac=<%=ipmac.getMac()%>&ifindex=<%=ipmac.getIfindex()%>&macip=<%=ipmac.getIpaddress()%>&flag=<%=flag1%>&doflag=-1" ><font color='green'>取消基线</font></a></td>
            												<td height=28 class="detail-data-body-list"><a href="<%=rootPath%>/ipmac.do?action=cancelipmacbase&id=<%=tmp%>&ipaddress=<%=host.getIpAddress()%>&mac=<%=ipmac.getMac()%>&ifindex=<%=ipmac.getIfindex()%>&macip=<%=ipmac.getIpaddress()%>&flag=<%=flag1%>&doflag=0"><font color='green'>取消</font></a></td>
            												<td height=28 class="detail-data-body-list"><a href="<%=rootPath%>/ipmac.do?action=setipmacbase&id=<%=tmp%>&ipaddress=<%=host.getIpAddress()%>&mac=<%=ipmac.getMac()%>&ifindex=<%=ipmac.getIfindex()%>&macip=<%=ipmac.getIpaddress()%>&flag=<%=flag1%>&doflag=2">绑定</a></td>
            												<td height=28 class="detail-data-body-list"><a href="<%=rootPath%>/ipmac.do?action=setipmacbase&id=<%=tmp%>&ipaddress=<%=host.getIpAddress()%>&mac=<%=ipmac.getMac()%>&ifindex=<%=ipmac.getIfindex()%>&macip=<%=ipmac.getIpaddress()%>&flag=<%=flag1%>&doflag=3">绑定</a></td>
            												<%
            												    	}else if(ifband == 2){
