...
<html>
	<head>
		...
		<%
			...
			String fileName = user.getUserid() + "_" + ip + "_" + ifindex+ "_multicastpacket_settings.xml";  
		%>
		...
		<script type="text/javascript"
			src="<%=rootPath%>/dwr/interface/MulticastPacketControler.js"></script>
		...
		<script type="text/javascript"> 
			...
			function excelBtn(){
				var flashMovie = document.getElementById('amline');   
		 		if (flashMovie) {   
	                flashMovie.exportImage('armChartServlet?ip=<%=ip%>&ifindex=<%=ifindex%>&imageName=多播数据包'); 
	               	//flashMovie.exportImage('test2.jsp?ip=<%=ip%>&ifindex=<%=ifindex%>&imageName=多播数据包''); 
	            }   
	...
	<body onload="generateData();" id="body" class="body flowratebg">
						...
						<tr style="background-color: #FFFFFF;">
							<td> 
								<jsp:include page="/topology/includejsp/detail_content_top.jsp">
									<jsp:param name="contentTitle" value="多播数据包" />
								</jsp:include> 
							</td>
						</tr>
										...
										<td  align="center">  
										 ...
										 // <![CDATA[ 
											...
											function generateData(){   
										      	...
										      	MulticastPacketControler.generateData(datafileName,blackFlag,nodeID,ifindex,{
											...
											function printChart(){
												...
												so.addVariable("settings_file", escape("<%=rootPath%>/amcharts_settings/multicastpacket_settings.xml"));
