...
function openwin(str,operate,ip) 
{
  ...
  window.open ("iiscatreport.do?action="+operate+"&startdate="+startdate+"&todate="+todate+"&ipaddress="+ip+"&str="+str, "newwindow", "height=500, width=600, toolbar= no, menubar=no, scrollbars=yes, resizable=yes, location=yes, status=yes") 
}
  ...
  function query(){
 	...
 	window.open ("<%=rootPath%>/dbreport.do?action=downloadmultiorareport&startdate="+startdate+"&todate="+todate+"&ids="+oids, "newwindow", "height=500, width=600, toolbar= no, menubar=no, scrollbars=yes, resizable=yes, location=yes, status=yes")    	      
...
setTimeout(function(){
 ...
 Ext.get("process").on("click",function(){
	...
	//mainForm.action="<%=rootPath%>/iiscatreport.do?action=downloadselfiisreport&str=0";
	//mainForm.submit();        
	exeReport();
        }
 });	
...
function exeReport(){
 	...
 	window.open("<%=rootPath%>/iiscatreport.do?action=downloadselfiisreportAll&str=allxls&startdate="+startdate+"&todate="+todate+"&ids="+oids, "newwindow", "height=500, width=600, toolbar= no, menubar=no, scrollbars=yes, resizable=yes, location=yes, status=yes")
}
</script>
...
function setClass(){
	document.getElementById('middlewareReportTitle-2').className='detail-data-title';
	document.getElementById('middlewareReportTitle-2').onmouseover="this.className='detail-data-title'";
	document.getElementById('middlewareReportTitle-2').onmouseout="this.className='detail-data-title'";
}
...
function query()
  {  
     mainForm.action = "<%=rootPath%>/midcapreport.do?action=find&dbflag=3";
     mainForm.submit();
  }
</script>
				        											...
				        											<table id="report-data-body" class="report-data-body">
																		...
																		<%
																	    	for(int i=0;i<rc;i++){
																	       		...
																	       		Node node = PollingEngine.getInstance().getIisByID(vo.getId());
																	       		String alarmmessage = "";
																	       		int status = 0;
																	       		if(node != null){
																		...
																		%>
																		 ...
																		 <tr <%=onmouseoverstyle%> class="microsoftLook" height=25>
																			...
																			<td align=center class="report-data-body-list">      
																				<img  name="submitss" src="<%=rootPath%>/resource/image/export_excel.gif" width=18 border="0" onclick=openwin(0,"downloadselfiisreport","<%=node.getIpAddress()%>") target="_blank">      
																		   	</td>
																		   	<td align=center class="report-data-body-list">     
																				<img  name="submitss" src="<%=rootPath%>/resource/image/export_word.gif" width=18 border="0" onclick=openwin(1,"downloadselfiisreport","<%=node.getIpAddress()%>") target="_blank">      
																		 	</td>
																		  	<td align=center class="report-data-body-list">       
																				<img  name="submitss" src="<%=rootPath%>/resource/image/export_pdf.gif" width=18 border="0" onclick=openwin(4,"downloadselfiisreport","<%=node.getIpAddress()%>") target="_blank">      
																		  	</td>
																		   	<td align=center class="report-data-body-list">       
																				<img  name="submitss" src="<%=rootPath%>/resource/image/export_word.gif" width=18 border="0" onclick=openwin(2,"downloadselfiisreport","<%=node.getIpAddress()%>") target="_blank">      
																		  	</td>
																		  	<td align=center class="report-data-body-list">      
																				<img  name="submitss" src="<%=rootPath%>/resource/image/export_pdf.gif" width=18 border="0" onclick=openwin(3,"downloadselfiisreport","<%=node.getIpAddress()%>") target="_blank">      
																		  	</td>
																		</tr>
																		 <%
																		 	}
