...
function exeReport(){
 	...
 	window.open ("<%=rootPath%>/tomcatreport.do?action=downloadselftomcatreportAll&startdate="+startdate+"&todate="+todate+"&ids="+oids, "newwindow", "height=500, width=600, toolbar= no, menubar=no, scrollbars=yes, resizable=yes, location=yes, status=yes")    	      
}
...
function openwin(str,operate,ip) 
{
  var startdate = mainForm.startdate.value;
  var todate = mainForm.todate.value;
  window.open ("tomcatreport.do?action="+operate+"&startdate="+startdate+"&todate="+todate+"&ipaddress="+ip+"&str="+str, "newwindow", "height=500, width=600, toolbar= no, menubar=no, scrollbars=yes, resizable=yes, location=yes, status=yes") 
}
...
function setClass(){
	document.getElementById('middlewareReportTitle-1').className='detail-data-title';
	document.getElementById('middlewareReportTitle-1').onmouseover="this.className='detail-data-title'";
	document.getElementById('middlewareReportTitle-1').onmouseout="this.className='detail-data-title'";
}
...
function query()
  {  
     mainForm.action = "<%=rootPath%>/midcapreport.do?action=find&dbflag=2";
     mainForm.submit();
  }
</script>
																						...
																						<tr>
																							<td class="report-data-body-title"
																								...
																								<input type=text readonly="readonly"
																									...
																									id="bidtext" name="bidtext" size="25"
																									maxlength="25" value="">
																								<input type="hidden" id="bid" name="bid"
																									value="">
																								<input type="button" name="submitss"
																									value="查  询" onclick="query()">
																								&nbsp;&nbsp;
																								<input type="button" name="process" value="生成报表"
																									onclick="#">
																							</td>
																						</tr>
																					</table>
																		...
																		<table id="report-data-body" class="report-data-body">
																			...
																			<%
																				for (int i = 0; i < rc; i++) {
																					...
																					Node node = PollingEngine.getInstance().getTomcatByID(vo.getId());
																					String alarmmessage = "";
																					int status = 0;
																					if (node != null) {
																			...
																			<tr <%=onmouseoverstyle%> height=25>
																				...
																				<td class="report-data-body-list" align="center">
																					<img name="submitss"
																						...
																						width=18 border="0" onclick=openwin(0,"downloadselftomcatreport","<%=node.getIpAddress()%>") target="_blank">
																				</td>
																				<td class="report-data-body-list" align="center">
																					<img name="submitss"
																						src="<%=rootPath%>/resource/image/export_word.gif"
																						width=18 border="0" onclick=openwin(1,"downloadselftomcatreport","<%=node.getIpAddress()%>") target="_blank">
																				</td>
																				<td class="report-data-body-list" align="center">
																					<img name="submitss"
																						src="<%=rootPath%>/resource/image/export_pdf.gif"
																						width=18 border="0" onclick=openwin(4,"downloadselftomcatreport","<%=node.getIpAddress()%>") target="_blank">
																				</td>
																				<td class="report-data-body-list" align="center">
																					<img name="submitss"
																						src="<%=rootPath%>/resource/image/export_word.gif"
																						width=18 border="0" onclick=openwin(2,"downloadselftomcatreport","<%=node.getIpAddress()%>") target="_blank">
																				</td>
																				<td class="report-data-body-list" align="center">
																					<img name="submitss"
																						src="<%=rootPath%>/resource/image/export_pdf.gif"
																						width=18 border="0" onclick=openwin(3,"downloadselftomcatreport","<%=node.getIpAddress()%>") target="_blank">
																				</td>
																			</tr>
																			<%
																				}
