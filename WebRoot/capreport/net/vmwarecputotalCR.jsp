...
List l = (ArrayList)((HashMap<String, Object>)vmData.get("cr")).get(vid);
...
function query(){
	...
	mainForm.action="<%=rootPath%>/netreport.do?action=vmwarecputotalCR";
	subforms.submit();
}
...
function createChart(){
    ...
    pingSo.addVariable("settings_file", escape("<%=rootPath%>/amcharts_settings/vmwarecputotalCR_settings.xml"));
                						...
                						<table width="100%" border="0" cellpadding="0" cellspacing="1">
                						 <tr>
                      							...
                      							<td align=right valign=top>&nbsp;&nbsp;
                      								<a href="<%=rootPath%>/netreport.do?action=createdocVM&flag=cputotalCR&nowvalue=<%=l.get(1) %>&avgvalue=<%=avgvalue %>&maxvalue=<%=maxvalue %>"><img name="selDay1" alt='导出word' style="CURSOR:hand" src="<%=rootPath%>/resource/image/export_word.gif" width=18  border="0">导出WORD</a>&nbsp;&nbsp;&nbsp;&nbsp;
											        <a href="<%=rootPath%>/netreport.do?action=creatXlsVM&flag=cputotalCR&nowvalue=<%=l.get(1) %>&avgvalue=<%=avgvalue %>&maxvalue=<%=maxvalue %>&startdate=<%=startdate%>&todate=<%=todate%>" target="_blank"><img name="selDay1" alt='导出EXCEL' style="CURSOR:hand" src="<%=rootPath%>/resource/image/export_excel.gif" width=18  border="0">导出EXCEL</a>&nbsp;&nbsp;&nbsp;&nbsp;
                      										&nbsp;&nbsp;
                      								<a href="<%=rootPath%>/netreport.do?action=createpdfVM&flag=cputotalCR&nowvalue=<%=l.get(1) %>&avgvalue=<%=avgvalue %>&maxvalue=<%=maxvalue %>"><img name="selDay1" alt='导出word' style="CURSOR:hand" src="<%=rootPath%>/resource/image/export_pdf.gif" width=18  border="0">导出PDF</a>&nbsp;&nbsp;&nbsp;&nbsp;
                      							</td> 
                      						</tr>
                      					</table>
                    							<tr> 
									...
									<tr valign="middle" bgcolor="#FFFFFF" height="30">
									     <td  width=100% align="right">
									        <table>
									           ...
									           <tr>
									              <td align=center width=30%>CPU总计</td>
									              <td align=center width=20%><%=l.get(1)%></td>
