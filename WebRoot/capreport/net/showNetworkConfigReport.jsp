...
function network_config_report()
{
	mainForm.action = "<%=rootPath%>/netreport.do?action=downloadNetworkConfigReport&ipaddress=<%=ipaddress%>&str=1";
	mainForm.submit();
}
function network_config_ok()
{
	var prewindow = window.open("<%=rootPath%>/netreport.do?action=showNetworkConfigReport&ipaddress=<%=ipaddress%>","hello","width=600,height=400");
	prewindow.opener.close();
}
function network_config_cancel()
{
window.close();
}
function query(){
		...
		<table id="report-data-header-title" class="report-data-header-title">
			<tr>
				<td>
					...
					&nbsp;&nbsp;<input type="button" name="doprocess" value="导出" onclick="network_config_report()">
					<input type="button" name="doprocess" value="取消" onclick="network_config_cancel()">
				</td>
			</tr>
			<tr>
				<td>
					<table>
						<tr>
							<td colspan="8" align="center">主机服务器连通率报表</td>
						</tr>
						<tr>
							<td></td><td>IP地址</td><td>设备名称</td><td>操作系统</td><td>平均连通率</td><td>宕机次数</td><td>平均响应时间(ms)</td><td>最大响应时间(ms)</td>
						</tr>
						<%
			...
			for (int i = 0; i < pinglist.size(); i++) {
				...
				%>
				<tr>
					<td><%=cell5 %></td><td><%=cell6 %></td><td><%=cell7 %></td><td><%=cell8 %></td><td><%=cell9 %></td><td><%=cell10 %></td><td><%=cell15 %></td><td><%=cell16 %></td>
				</tr>
				<%
			}
		}
