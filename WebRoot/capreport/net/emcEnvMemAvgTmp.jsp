...
Environment l = (Environment) ipAllData.get("environment");
String value = "0";
if(l != null){
       List<MemModel> lt = l.getMemList();
       if(lt != null && lt.size()>0){
        for(int i=0;i<lt.size();i++){
         MemModel vo = (MemModel)lt.get(i);
         if(name.equals(vo.getName())){
           value = vo.getAverageDegree();
         }
        }
       }
}
...
function query(){
	...
	mainForm.action="<%=rootPath%>/netreport.do?action=emcEnvMemAvgTmp";
	subforms.submit();
}
...
function createChart(){
    ...
    pingSo.addVariable("settings_file", escape("<%=rootPath%>/amcharts_settings/emcenvmemavgtmp_settings.xml"));
                						...
                						<table width="100%" border="0" cellpadding="0" cellspacing="1">
                						 <tr>
                      							...
                      							<td align=right valign=top>&nbsp;&nbsp;
                      								<a href="<%=rootPath%>/netreport.do?action=createdocEMC&flag=memavgtmp&nowvalue=<%=value %>&avgvalue=<%=avgvalue %>&maxvalue=<%=maxvalue %>"><img name="selDay1" alt='导出word' style="CURSOR:hand" src="<%=rootPath%>/resource/image/export_word.gif" width=18  border="0">导出WORD</a>&nbsp;&nbsp;&nbsp;&nbsp;
											        <a href="<%=rootPath%>/netreport.do?action=creatXlsEMC&flag=memavgtmp&nowvalue=<%=value %>&avgvalue=<%=avgvalue %>&maxvalue=<%=maxvalue %>&startdate=<%=startdate%>&todate=<%=todate%>" target="_blank"><img name="selDay1" alt='导出EXCEL' style="CURSOR:hand" src="<%=rootPath%>/resource/image/export_excel.gif" width=18  border="0">导出EXCEL</a>&nbsp;&nbsp;&nbsp;&nbsp;
                      										&nbsp;&nbsp;
                      								<a href="<%=rootPath%>/netreport.do?action=createpdfEMC&flag=memavgtmp&nowvalue=<%=value %>&avgvalue=<%=avgvalue %>&maxvalue=<%=maxvalue %>"><img name="selDay1" alt='导出word' style="CURSOR:hand" src="<%=rootPath%>/resource/image/export_pdf.gif" width=18  border="0">导出PDF</a>&nbsp;&nbsp;&nbsp;&nbsp;
                      							</td> 
                      						</tr>
                      					</table>
                    							<tr> 
									...
									<tr valign="middle" bgcolor="#FFFFFF" height="30">
									     <td  width=100% align="right">
									        <table>
									           ...
									           <tr>
									              <td align=center width=30%>平均温度(摄氏度)</td>
