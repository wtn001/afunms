...
var starttime = document.getElementById("mystartdate").value;
	...
	mainForm.action = "<%=rootPath%>/hostreport.do?action=downlaodDiskReport&ipaddress=<%=ipaddress%>&str=2";
	mainForm.submit();
}
function disk_ok()
{
	...
	var starttime = document.getElementById("mystartdate").value;
	var endtime = document.getElementById("mytodate").value;
	var prewindow = window.open("<%=rootPath%>/hostreport.do?action=showDiskReport&ipaddress=<%=ipaddress%>&startdate="+starttime+"&todate="+endtime,"hello","width=800,height=400,scrollbars=yes,resizable=yes");
	prewindow.opener.close();
}
function disk_cancel()
{
window.close();
}
function query(){
		...
		<div id="loading">
			<div class="loading-indicator">
				<img src="<%=rootPath%>/js/ext/lib/resources/extanim64.gif" width="32" height="32" style="margin-right: 8px;" align="middle" />Loading...</div>
					...
					<input type="button" name="doprocess" value="取消" onclick="disk_cancel()">
</form>
</div>
</BODY>
</HTML>