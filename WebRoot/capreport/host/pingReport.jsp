...
function ping_ok()
{
	//var prewindow = window.open("<%=rootPath%>/hostreport.do?action=downloadPingReport&ipaddress=<%=ipaddress%>&str=1","hello","width=600,height=400");
	var starttime = document.getElementById("mystartdate").value;
	var endtime = document.getElementById("mytodate").value;
	var prewindow = window.open("<%=rootPath%>/hostreport.do?action=showPingReport&ipaddress=<%=ipaddress%>&startdate="+starttime+"&todate="+endtime,"hello","width=800,height=400,scrollbars=yes,resizable=yes");
	//alert("<%=rootPath%>/hostreport.do?action=showPingReport&ipaddress=<%=ipaddress%>&startdate=<%=startdate%>&todate=<%=todate%>");
		...
		<div id="loading">
			<div class="loading-indicator">
				<img src="<%=rootPath%>/js/ext/lib/resources/extanim64.gif" width="32" height="32" style="margin-right: 8px;" align="middle" />Loading...</div>
					...
					<input type="button" name="doprocess" value="取消" onclick="ping_cancel()">
