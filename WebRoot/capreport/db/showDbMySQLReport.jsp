...
{
	mainForm.action = "<%=rootPath%>/dbreport.do?action=createMySQLPingReport&startdate=<%=startdate%>&todate=<%=todate%>&str=3";
	mainForm.submit();
}
function ping_word_report()
{
	mainForm.action = "<%=rootPath%>/dbreport.do?action=createMySQLPingReport&avgpingcon=<%=avgpingcon%>&newip=<%=newip%>&ip=<%=ip%>&str=1";
	mainForm.submit();
}
function ping_pdf_report()
{
	mainForm.action = "<%=rootPath%>/dbreport.do?action=createMySQLPingReport&avgpingcon=<%=avgpingcon%>&newip=<%=newip%>&ip=<%=ip%>&str=2";
	mainForm.submit();
}
function ping_ok()
{
	...
	mainForm.action = "<%=rootPath%>/mysql.do?action=mysqlManagerPingReportQuery&ipaddress= &startdate="+starttime+"&todate="+endtime;
	mainForm.submit();
}
...
function setClass(){
	document.getElementById('dbMysqlReportPerformTitle-0').className='detail-data-title';
	document.getElementById('dbMysqlReportPerformTitle-0').onmouseover="this.className='detail-data-title'";
	document.getElementById('dbMysqlReportPerformTitle-0').onmouseout="this.className='detail-data-title'";
}
				       			...
				       			<tr>
									<td>
										<table id="report-content-header" class="report-content-header">
						                	<tr>
							    				<td>
										    		<%=dbMysqlReportPerformTitleTable%>
										    	</td>
										  	</tr>
							        	</table>
        							</td>
