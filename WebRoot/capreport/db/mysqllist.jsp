...
function exeReport(){
 	...
 	window.open ("<%=rootPath%>/dbreport.do?action=downloadmultimysqlreport&startdate="+startdate+"&todate="+todate+"&ids="+oids, "newwindow", "height=500, width=600, toolbar= no, menubar=no, scrollbars=yes, resizable=yes, location=yes, status=yes")    	      
...
setTimeout(function(){
 ...
 Ext.get("process").on("click",function(){
     ...
     if(k==0){
		...
		mainForm.action="<%=rootPath%>/dbreport.do?action=downloadmultisqlreport";
		mainForm.submit();        
      }
 });	
...
function query()
  {  
     mainForm.action = "<%=rootPath%>/dbreport.do?action=find&dbflag=4";
     mainForm.submit();
  }
</script>
...
function setClass(){
	document.getElementById('dbReportTitle-7').className='detail-data-title';
	document.getElementById('dbReportTitle-7').onmouseover="this.className='detail-data-title'";
	document.getElementById('dbReportTitle-7').onmouseout="this.className='detail-data-title'";
}
...
<body id="body" class="body" onload="initmenu();">
	...
	<input type=hidden name="eventid">
    	...
    	<td class="report-data-body-title" width="60" colspan="5" align="center"><strong>生成报表</strong></td>
   </tr>
  <tr class="microsoftLook0" height=28>
...
%>
 ...
 <tr bgcolor="#ffffff" <%=onmouseoverstyle%> class="microsoftLook" height=25>
    ...
    <td class="report-data-body-list">&nbsp;&nbsp;<INPUT type="checkbox" class=noborder id="<%=i+1%>" name=checkbox value="<%=node.getId()%>"><%=i+1%></td>
	...
	&nbsp;&nbsp;<img  name="submitss" src="<%=rootPath%>/resource/image/export_excel.gif" width=18 border="0" onclick="document.getElementById('<%=i+1%>').checked = true;exeReport();" target="_blank">      
      </td>
        <td class="report-data-body-list" align=center>     
	&nbsp;&nbsp;<img  name="submitss" src="<%=rootPath%>/resource/image/export_word.gif" width=18 border="0" onclick=openwin(0,"createMySQLCldReport","<%=node.getIpAddress()%>") target="_blank">      
      </td>
        <td class="report-data-body-list" align=center>       
	&nbsp;&nbsp;<img  name="submitss" src="<%=rootPath%>/resource/image/export_pdf.gif" width=18 border="0" onclick=openwin(1,"createMySQLCldReport","<%=node.getIpAddress()%>") target="_blank">      
      </td>
        <td class="report-data-body-list" align=center>       
	&nbsp;&nbsp;<img  name="submitss" src="<%=rootPath%>/resource/image/export_word.gif" width=18 border="0" onclick=openwin(3,"downloadmysqlselfreport","<%=node.getIpAddress()%>") target="_blank">      
      </td>
       <td class="report-data-body-list" align=center>      
	&nbsp;&nbsp;<img  name="submitss" src="<%=rootPath%>/resource/image/export_pdf.gif" width=18 border="0" onclick=openwin(4,"downloadmysqlselfreport","<%=node.getIpAddress()%>") target="_blank">      
      </td>
 </tr>
 <%}
