...
function ping_word_report()
{
	...
	//HONGLI MODIFFY
	mainForm.action = "<%=rootPath%>/dbreport.do?action=createOracleEventReport&str=0&ipaddress=<%=ip%>&typevo=<%=typevo%>&dbname=<%=dbname%>&pingvalue=<%=pingvalue%>&startdate=<%=startdate%>&todate=<%=todate%>";
	mainForm.submit();
}
function ping_excel_report()
{
	mainForm.action = "<%=rootPath%>/dbreport.do?action=createOracleEventReport&str=2&startdate=<%=startdate%>&todate=<%=todate%>&ipaddress=<%=ip%>&typevo=<%=typevo%>&dbname=<%=dbname%>&pingvalue=<%=pingvalue%>";
	mainForm.submit();
}
function ping_pdf_report()
{
	mainForm.action = "<%=rootPath%>/dbreport.do?action=createOracleEventReport&str=1&ipaddress=<%=ip%>&typevo=<%=typevo%>&dbname=<%=dbname%>&pingvalue=<%=pingvalue%>&startdate=<%=startdate%>&todate=<%=todate%>";
	mainForm.submit();
}
function ping_ok()
{
	...
	mainForm.action = "<%=rootPath%>/oracle.do?action=oracleManagerEventReportQuery&ipaddress=<%=ip%>&startdate="+starttime+"&todate="+endtime+"&id="+<%=id%>+"&status="+status+"&level1="+level1;
	mainForm.submit();
}
...
function setClass(){
	document.getElementById('dbReportPerformTitle-3').className='detail-data-title';
	document.getElementById('dbReportPerformTitle-3').onmouseover="this.className='detail-data-title'";
	document.getElementById('dbReportPerformTitle-3').onmouseout="this.className='detail-data-title'";
}
										...
										<td>
											<table id="win-content-header" class="win-content-header">
					                			<tr>
								                	...
								                	<td class="win-content-title" style="align:center">&nbsp;报表管理</td>
																	...
																	<td>
																		<table width="100%" border="0" cellpadding="3"
																			...
																			<%}
 																			...
 																			%>
																		</table><br><br><center><input type=button value="关闭窗口" onclick="window.close()"></center></br>
																	</td>
																</tr>
															</table>
							       				</tr> 
