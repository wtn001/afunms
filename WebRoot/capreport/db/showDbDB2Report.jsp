...
{
	mainForm.action = "<%=rootPath%>/dbreport.do?action=createDB2PingReport&startdate=<%=startdate%>&todate=<%=todate%>&str=3";
	mainForm.submit();
}
function ping_word_report()
{
	mainForm.action = "<%=rootPath%>/dbreport.do?action=createDB2PingReport&avgpingcon=<%=avgpingcon%>&newip=<%=newip%>&ip=<%=ip%>&str=1";
	mainForm.submit();
}
function ping_pdf_report()
{
	mainForm.action = "<%=rootPath%>/dbreport.do?action=createDB2PingReport&avgpingcon=<%=avgpingcon%>&newip=<%=newip%>&ip=<%=ip%>&str=2";
	mainForm.submit();
}
function ping_ok()
{
	...
	mainForm.action = "<%=rootPath%>/db2.do?action=db2ManagerPingReportQuery&ipaddress=<%=ip%>&id=<%=session.getAttribute("id")%>&startdate="+starttime+"&todate="+endtime;
	mainForm.submit();
}
...
function setClass(){
	document.getElementById('db2ReportPerformTitle-0').className='detail-data-title';
	document.getElementById('db2ReportPerformTitle-0').onmouseover="this.className='detail-data-title'";
	document.getElementById('db2ReportPerformTitle-0').onmouseout="this.className='detail-data-title'";
}
