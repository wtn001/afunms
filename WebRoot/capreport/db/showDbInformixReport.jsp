...
{
	mainForm.action = "<%=rootPath%>/dbreport.do?action=createInformixPingReport&startdate=<%=startdate%>&todate=<%=todate%>&str=3";
	mainForm.submit();
}
function ping_word_report()
{
	mainForm.action = "<%=rootPath%>/dbreport.do?action=createInformixPingReport&avgpingcon=<%=avgpingcon%>&newip=<%=newip%>&ip=<%=ip%>&str=1";
	mainForm.submit();
}
function ping_pdf_report()
{
	mainForm.action = "<%=rootPath%>/dbreport.do?action=createInformixPingReport&avgpingcon=<%=avgpingcon%>&newip=<%=newip%>&ip=<%=ip%>&str=2";
	mainForm.submit();
}
function ping_ok()
{
	...
	mainForm.action = "<%=rootPath%>/informix.do?action=informixManagerPingReportQuery&ipaddress=<%=ip%>&id=<%=(String)session.getAttribute("id")%>&startdate="+starttime+"&todate="+endtime;
	mainForm.submit();
}
				...
				<table id="container-main-win" class="container-main-win">
					...
					<tr>
						<td colspan="4" align="center">
							<img src="<%=rootPath%>/resource/image/jfreechart/<%=newip %>ConnectUtilization.png"/><br><br>
