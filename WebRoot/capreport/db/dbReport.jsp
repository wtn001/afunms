...
function parseDataForTree()
{
	...
	ddtree.insertNewItem("","root","数据库", 0, "", "","", "");
...
msgWindow=window.open(url,"protypeWindow","toolbar=no,width=400,height=400,directories=no,status=no,scrollbars=no,menubar=no")
  ...
  function loadIds(id,ids,startdate,todate,beginHour,endHour){
   $.ajax({
			type:"POST",
			dataType:"json",
			data:"id="+id+"&ids="+ids+"&startdate="+startdate+"&todate="+todate+"&beginHour="+beginHour+"&endHour="+endHour+"&nowtime="+(new Date()),
...
if((ids.length<=0|| ids.length == "")&&(id.length<=0|| id.length == "")){
 ...
 window.open('<%=rootPath%>/netreport.do?action=downloadReport&type=db&exportType='+exportType+'&id='+id+'&ids='+ids+'&startdate='+startdate+'&todate='+todate+'&beginHour='+beginHour+'&endHour='+endHour+'&nowtime='+(new Date()),"_blank","toolbar=no,width=1,height=1,top=2000,left=3000,directories=no,status=no,menubar=no,alwaysLowered=yes");
