...
<%
	...
	Double avgtime = (Double) request.getAttribute("avgpingcon");
  ...
  Double Ping = (Double)request.getAttribute("Ping");
  Double pingmax = (Double)request.getAttribute("pingmax");
...
String sid = (String)request.getAttribute("sid");//HONGLI ADD
...
{
	mainForm.action = "<%=rootPath%>/oracle.do?action=dboraReportdownusableload&ipaddress=<%=ipaddress%>&str=1";
	mainForm.submit();
}
function ping_excel_report()
{
	mainForm.action = "<%=rootPath%>/oracle.do?action=dboraReportdownusableload&ipaddress=<%=ipaddress%>&str=0";
	mainForm.submit();
}
function ping_pdf_report()
{
	mainForm.action = "<%=rootPath%>/oracle.do?action=dboraReportdownusableload&ipaddress=<%=ipaddress%>&str=2";
	mainForm.submit();
}
function ping_ok()
{
	...
	//HONGLI MODIFY
	mainForm.action = "<%=rootPath%>/hostreport.do?action=showPingReportByDate&ipaddress=<%=ipaddress%>&startdate="+starttime+"&todate="+endtime+"&sid="+<%=sid%>;
	mainForm.submit();
}
...
function setClass(){
	document.getElementById('dbReportPerformTitle-0').className='detail-data-title';
	document.getElementById('dbReportPerformTitle-0').onmouseover="this.className='detail-data-title'";
	document.getElementById('dbReportPerformTitle-0').onmouseout="this.className='detail-data-title'";
}
									...
									<td>
										<table id="win-content-header" class="win-content-header">
				                			<tr>
							                	...
							                	<td class="win-content-title" style="align:center">&nbsp;可用性报表</td>
						       					...
						       					<td>
													<table bgcolor="#ECECEC">
														...
														<td height="28" align="left">
															开始日期
															<input type="text" id="mystartdate" name="startdate" value="<%=starttime%>" size="10">
															<a onclick="event.cancelBubble=true;" href="javascript:ShowCalendar(document.forms[0].imageCalendar1,document.forms[0].startdate,null,0,330)">
															<img id=imageCalendar1 align=absmiddle width=34 height=21 src="<%=rootPath%>/include/calendar/button.gif" border=0></a>
																							
															截止日期
															<input type="text" id="mytodate" name="todate" value="<%=totime%>" size="10"/>
							       			...
							       			<tr align="left" valign="center"> 
			             						<td height="28" align="left" border="0">
													...
													<table border="1" width="90%">
														...
														<td><%=pingmax%></td>
														--%>
														<td><%=pingnow%>%</td><!-- HONGLI MODIFY -->
														<td><%=pingmin%>%</td><!-- HONGLI MODIFY -->
														<td><%=avgpingcon %>%</td>
														</tr>
					<tr>
