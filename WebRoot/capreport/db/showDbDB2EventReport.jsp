...
function ping_word_report()
{
	...
	//HONGLI MODIFFY
	mainForm.action = "<%=rootPath%>/dbreport.do?action=createDB2EventReport&str=0&ipaddress=<%=ip%>&typevo=<%=typevo%>&dbname=<%=dbname%>&pingvalue=<%=pingvalue%>&startdate=<%=startdate%>&todate=<%=todate%>";
	mainForm.submit();
}
function ping_excel_report()
{
	mainForm.action = "<%=rootPath%>/dbreport.do?action=createDB2EventReport&str=2&startdate=<%=startdate%>&todate=<%=todate%>&ipaddress=<%=ip%>&typevo=<%=typevo%>&dbname=<%=dbname%>&pingvalue=<%=pingvalue%>";
	mainForm.submit();
}
function ping_pdf_report()
{
	mainForm.action = "<%=rootPath%>/dbreport.do?action=createDB2EventReport&str=1&ipaddress=<%=ip%>&typevo=<%=typevo%>&dbname=<%=dbname%>&pingvalue=<%=pingvalue%>&startdate=<%=startdate%>&todate=<%=todate%>";
	mainForm.submit();
}
function ping_ok()
{
	...
	mainForm.action = "<%=rootPath%>/db2.do?action=db2ManagerEventReportQuery&ipaddress=<%=ip%>&startdate="+starttime+"&todate="+endtime+"&id="+<%=session.getAttribute("id")%>+"&status="+status+"&level1="+level1;
	mainForm.submit();
}
...
function setClass(){
	document.getElementById('db2ReportPerformTitle-3').className='detail-data-title';
	document.getElementById('db2ReportPerformTitle-3').onmouseover="this.className='detail-data-title'";
	document.getElementById('db2ReportPerformTitle-3').onmouseout="this.className='detail-data-title'";
}
