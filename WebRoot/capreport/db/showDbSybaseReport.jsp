...
{
	mainForm.action = "<%=rootPath%>/dbreport.do?action=createSybasePingReport&startdate=<%=startdate%>&todate=<%=todate%>&str=3";
	mainForm.submit();
}
function ping_word_report()
{
	mainForm.action = "<%=rootPath%>/dbreport.do?action=createSybasePingReport&avgpingcon=<%=avgpingcon%>&newip=<%=newip%>&ip=<%=ip%>&str=1";
	mainForm.submit();
}
function ping_pdf_report()
{
	mainForm.action = "<%=rootPath%>/dbreport.do?action=createSybasePingReport&avgpingcon=<%=avgpingcon%>&newip=<%=newip%>&ip=<%=ip%>&str=2";
	mainForm.submit();
}
function ping_ok()
{
	...
	mainForm.action = "<%=rootPath%>/sybase.do?action=sybaseManagerPingReportQuery&ipaddress=<%=ip%>&id=<%=session.getAttribute("id")%>&startdate="+starttime+"&todate="+endtime;
	mainForm.submit();
}
