...
function opennewwin()
  {  
   window.open("<%=rootPath%>/monitor.do?action=hosteventlist&nodetype=db&id=<%=String.valueOf(request.getAttribute("ids"))%>");
  									...
  									<tr>
  										<td>数据库事件报表&nbsp;&nbsp;时间段:&nbsp;<%=(String)request.getAttribute("starttime")%>&nbsp;至&nbsp;<%=(String)request.getAttribute("totime")%>
											...
											<a href="<%=rootPath%>/dbreport.do?action=createeventpdf"><img name="selDay1" alt='导出pdf' style="CURSOR:hand" src="<%=rootPath%>/resource/image/export_pdf.gif" width=18  border="0">导出PDF</a>&nbsp;&nbsp;&nbsp;&nbsp;
                      								<a href="<%=rootPath%>/dbreport.do?action=createeventdoc"><img name="selDay1" alt='导出word' style="CURSOR:hand" src="<%=rootPath%>/resource/image/export_word.gif" width=18  border="0">导出WORD</a>&nbsp;&nbsp;&nbsp;&nbsp;
