...
Map<String,String> map = (Map<String,String>)request.getAttribute("taskUsers");
...
var listAction = "<%=rootPath%>/controller/taskClaimList.action";
</script>
<%} %>
<script language="javascript">	
  ...
  {  
     ...
     mainForm.action = "<%=rootPath%>/controller/taskClaimList.action";
     mainForm.submit();
  }
</script>
...
window.showModalDialog('<%=rootPath%>/system/codedetail/user.jsp?userid='+userid,window,'dialogWidth=400px;dialogHeight=200px;center=yes')
}
											...
											<td>
												<table id="content-header" class="content-header">
								                	<tr>
									                	...
									                	<td class="content-title"> 流程 >> 流程任务 >> 待办任务 </td>
		...
		<%
	 	...
	 	if (jp.getList() != null)
	 	{
	 		...
	 		task=(Task)jp.getList().get(i);
	 			%><tr>
	 			<!--<td align="center" class="body-data-list"><%=task.getId() %></td>  -->
				...
				<td align="center" class="body-data-list"><%=contentList.get(i) %></td>
				<td align="center" class="body-data-list"><%=DateFormat.TIME_FORMAT.format(task.getCreateTime()) %></td>
				<td align="center" class="body-data-list">
				<a href="javascript:ajaxuser('<%=map.get(task.getId())%>')" style="cursor: pointer;">
				<%=StringUtil.isBlank(map.get(task.getId()))?"":map.get(task.getId())%></a></td>
				...
				<td align="center" class="body-data-list">
				<a href="../controller/taskFormKey.action?taskId=<%=task.getId() %>&taskName=<%=task.getName() %>&taskExtId=<%=task.getExecutionId() %>&processInstanceId=<%=task.getProcessInstanceId() %>">处理
				</a>
				</td>
			</tr><%
	 		}
