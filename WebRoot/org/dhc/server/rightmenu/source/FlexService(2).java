...
//		System.out.println("==================="+ipAddress);
	...
	public String getNodebyId(String id,String category,String time){
		...
		return fnode.getAlias()+":"+fnode.getIpAddress();
		...
		try {
			for(int i=0 ;i<vector.size() ; i++){
                ...
                if(strs!=null&&strs.length>0){
                    ...
                    Vo.setStatue(url);
                    Vo.setOutBandwidthUtilHdx(strs[6]);
                    Vo.setInBandwidthUtilHdx(strs[7]);
                    Vo.setInBandwidthUtilHdxPerc(strs[5]);
                    Vo.setOutBandwidthUtilHdxPerc(strs[4]);
                    flexDataList.add(Vo);
                }
			}
		} catch (Exception e) {
