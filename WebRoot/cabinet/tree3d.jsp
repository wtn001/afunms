...
<script type="text/javascript"><!--
		...
		d = new dTree('d');
//		d.add(0,-1,' 机房列表');
		d.add(0,-1,' 机房列表','<%=rootPath+ "/cabinetshow.do?action=list&jp=1"%>','','','rightFrame');
		
		parent.document.getElementById("rightFrame").src='<%=rootPath+ "/cabinetshow.do?action=list&jp=1"%>';//rooms
		<%
			for(int i = 0;i<list.size();i++){
				EqpRoom room = (EqpRoom)list.get(i);
				String url = rootPath + "/cabinetshow.do?action=list3d&roomId="+room.getId()+"&jp=1";
				%>
