...
function Node(id, pid, name, url, onclick, title, target, icon, iconOpen, open) {
...
dTree.prototype.add = function(id, pid, name, url, onclick, title, target, icon, iconOpen, open) {

	this.aNodes[this.aNodes.length] = new Node(id, pid, name, url, onclick, title, target, icon, iconOpen, open);
...
dTree.prototype.node = function(node, nodeId) {
    ...
    if (node.onclick) {
		...
		str += '<a id="s' + this.obj + nodeId + '" class="' + ((this.config.useSelection) ? ((node._is ? 'nodeSel' : 'node')) : 'node') + '" onclick=SearchNode("'+node.onclick+'") oncontextmenu=showMenu("'+node.title+'","'+node.onclick+'")';
