// Node object
function Node(id, name, pid , isSelected,cabinetid) {
...
Tree.prototype = {
	...
	init:function(dataArray){
		...
		for(var i = 0 ; i < this.dataArray.length ; i++){
			...
			var node = new Node(data[0] , data[1] , data[2] , data[3],data[4]);
	...
	createNodeCheckboxIcon:function(node){
		...
		if(node.isSelected){
			img.src = "/afunms/config/business/img/radio_on.gif";
		}else{
			img.src = "/afunms/config/business/img/radio_off.gif";
		}
		img.onclick = function(){
			if(node.isSelected){
				// 如'选中' 则变为 '未选中'
	...
	choose:function(node){
		...
		checkimage.src = "/afunms/config/business/img/radio_on.gif";
		node.isSelected = true;
	},
	
	noChoose:function(node){
		var checkimage = document.getElementById("nodeCheckboxIcon-" + node.id);
		checkimage.src = "/afunms/config/business/img/radio_off.gif";
		node.isSelected = false;
	},
