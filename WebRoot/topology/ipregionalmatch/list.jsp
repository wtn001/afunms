...
<html>
	<head>
		...
		<script>
			...
			function refreshIpOnline(){
				if(window.confirm("同步刷新所需的时间比较长! 并会将此现有信息全部清空! 是否继续?")){
					Ext.MessageBox.wait('数据加载中，请稍后.. '); 
					mainForm.action = "<%=rootPath%>/ipDistrictMatch.do?action=list&refresh=refreshIpOnline"; 
					mainForm.submit();
				}
		        								...
		        								<table id="content-body" class="content-body">
		        									...
		        									<tr>
														<td colspan="7" style="text-align: right;" class="body-data-title">
															<input  type="button" onclick="refreshIpOnline()" value="IP同步刷新">&nbsp;&nbsp;&nbsp;
															<input  type="button" onclick="refresh()" value="MAC同步刷新">&nbsp;&nbsp;&nbsp;
					        							</td>
					        						</tr>
					        						<tr>
		        									...
		        									<%
		        									    if(list!=null&& list.size()>0){
		        									        ...
		        									        for(int i = 0 ; i < list.size() ; i++){
		        									            ...
		        									            %>
		        									            <tr <%=onmouseoverstyle%>>
		        													<td align="center" class="body-data-list"><INPUT type="checkbox" value="<%=ipDistrictMatchConfig.getId()%>" name="check" onclick="javascript:chkall()"><%=jp.getStartRow()+i%></td>
