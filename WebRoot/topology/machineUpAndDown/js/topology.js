...
{	
	...
	for (var i = 0; i < nodes.length; i += 1)
	{
		...
		var state = node.getElementsByTagName("state")[0].text;
...
//		document.all.divLayer.appendChild(divBackGround);
		...
		divText.style.background = "#CEDFF6";
	...
	for (var j = 0; j < lines.length; j += 1)
	{
		...
		var dash = lineObj.getElementsByTagName("dash")[0].text;
		var lineWidth = lineObj.getElementsByTagName("lineWidth")[0].text;
		var lineInfo = "策略";//lineObj.getElementsByTagName("lineInfo")[0].text;
		var lineMenu = lineObj.getElementsByTagName("lineMenu")[0].text;
		...
		line.from = (parseInt(document.all("node_" + a).style.left) + parseInt(document.all("node_" + a).style.width)/2 - 3) + "," + (parseInt(document.all("node_" + a).style.top) + parseInt(document.all("node_" + a).style.height)/2 - 3);
		line.to = (parseInt(document.all("node_" + b).style.left)-2) + "," + (parseInt(document.all("node_" + b).style.top) + parseInt(document.all("node_" + b).style.height)/2 - 3);
		line.strokecolor = color;   
		line.strokeweight = lineWidth;
...
function showLineInfo(id)
{
	...
	lastLine = id;   
	//alert(lastLine);
	//window.lineInfoFrame.location = "showStatic.jsp";
	document.all.linkedline.style.border = "solid #000066 1px";
	document.all.linkedline.style.backgroundColor = "#F5F5F5";
	document.all.linkedline.innerHTML = "";
	clearTimer();
...
//单个节点关机
function printedOne(id){
    //alert(id);
    if(parent.topFrame.goon==1){
        //var id = "node_" + node.getElementsByTagName("id")[0].text;
...
//单个节点关机结束
function endprintedOne(id){
    parent.topFrame.stopDownOne(id);
    document.all(id.replace("node_","text_")).style.background = "green";
}
//终止单个节点关机
function clearprintedOne(id){
    if(document.all(id).style.visibility != "visible"){
        document.all(id).style.visibility = "visible";   
    }
}
//
function printedAll(url){
//alert(parent.topFrame.goon);
//alert(url);
	if(parent.topFrame.goon==1){
        xmldoc = initXML(url);
	    var nodes = xmldoc.getElementsByTagName("node");
		...
		{
			var node = nodes[j];
			var name = node.getElementsByTagName("alias")[0].text;
			var id = "node_" + node.getElementsByTagName("id")[0].text;
			var ids = "text_" + node.getElementsByTagName("id")[0].text;
			var state = node.getElementsByTagName("state")[0].text;
			//alert(document.all(id).style.visibility);
			//alert(state);
			//0无操作，1正在关机，2正常关机，3关机失败，4连接异常，5手动关机
			if(state==0){
			    document.all(ids).style.background = "#CEDFF6";
			} else if(state==1){
			    document.all(ids).style.background = "yellow";
			    if(document.all.linkedline.innerHTML.indexOf("正在关闭设备 "+name+" step......")<0){
			        document.all.linkedline.innerHTML +=  "正在关闭设备 "+name+" step......<br>";
			    }
			    if(document.all(id).style.visibility == "visible")
				    document.all(id).style.visibility = "hidden";
			    else
			    ...
			    document.all(ids).style.background = "green";
			    if(document.all.linkedline.innerHTML.indexOf("设备 "+name+" 正常关机......")<0){
			        document.all.linkedline.innerHTML +=  "设备 "+name+" 正常关机......<br>";
			    }
			    if(document.all(id).style.visibility != "visible"){
		            document.all(id).style.visibility = "visible";   
		        }   
			} else if(state==3){
			    document.all(ids).style.background = "red";
			    if(document.all.linkedline.innerHTML.indexOf("设备 "+name+" 关机失败......")<0){
			        document.all.linkedline.innerHTML +=  "设备 "+name+" 关机失败......<br>";
			    }
			    if(document.all(id).style.visibility != "visible"){
		            document.all(id).style.visibility = "visible";   
		        } 
			} else if(state==4){
			    document.all(ids).style.background = "#8E8E8E";
			    if(document.all.linkedline.innerHTML.indexOf("设备 "+name+" 连接异常......")<0){
			        document.all.linkedline.innerHTML +=  "设备 "+name+" 连接异常......<br>";
			    }
			    if(document.all(id).style.visibility != "visible"){
		            document.all(id).style.visibility = "visible";   
		        }   
			} else if(state==5){
			    document.all(ids).style.background = "blue";
			    if(document.all.linkedline.innerHTML.indexOf("设备 "+name+" 已关机......")<0){
			        document.all.linkedline.innerHTML +=  "设备 "+name+" 已关机......<br>";
			    }
			    if(document.all(id).style.visibility != "visible"){
		            document.all(id).style.visibility = "visible";   
		        }  
...
//并行关机结束
function endprinted(){
    parent.topFrame.stopDown();
    var nodes = xmldoc.getElementsByTagName("node");
    for (var j = 0; j < nodes.length; j += 1)
	{
		var node = nodes[j];
		var id = "text_" + node.getElementsByTagName("id")[0].text;
		//alert(document.all(id).style.visibility);background = "#9BFFAC";
        //if(document.all(id).style.visibility == "visible")
		    //document.all(id).style.visibility = "hidden";
	   //else
		    document.all(id).style.background = "green";   
	}
}
function loadInfo()
{
	...
	if (fullscreen == "0")
	{
		x = 200;//(parseInt(viewWidth) - 800) / 2;
	...
	document.write('<div id="linkedline" style="position:absolute;left:'+ x +'px;top:'+ y +'px;float:center;width:70%;height:100px;z-index:999;visibility:hidden;overflow:auto">');
	//document.write('<iframe id="lineInfoFrame" frameborder="0" marginheight="0" marginwidth="0" scrolling="no" align="center" width="100%" height="'+frameHeight+'" src="showStatic.jsp"></iframe>');
	document.write('</div>');
}
...
if (fullscreen == "0")
{
	...
	defaultY = parseInt(window.parent.parent.document.body.clientHeight) - frameHeight  - 30;//- 35 + 15- 90
}
else
{
	window.parent.document.body.onscroll = screenScroll;
	defaultY = parseInt(window.parent.document.body.clientHeight) - frameHeight;//- 30  + 15
}
