...
<%
	...
	String collecttime =request.getParameter("collecttime")==null?"":request.getParameter("collecttime"); //数据采集时间
	String sysuptime =request.getParameter("sysuptime")==null?"":request.getParameter("sysuptime");//设备启动时间 
	String sysdescr =request.getParameter("sysdescr")==null?"":request.getParameter("sysdescr");// 系统描述 
	String percent1 =request.getParameter("percent1")==null?"":request.getParameter("percent1");//   
	String percent2 =request.getParameter("percent2")==null?"":request.getParameter("percent2");//  
	String cpuper =request.getParameter("cpuper")==null?"":request.getParameter("cpuper");
							...
							<tr>
							             <td width="100%" align="center" valign="top" colspan=2>
							                    <table> 
	                                              ...
	                                              <% if(!cpuData.equals("0")){%>
	                                          var so = new SWFObject("<%=rootPath%>/amchart/amcolumn.swf", "amcolumn","258", "210", "8", "#FFFFFF");
				                                  ...
				                                  so.write("wmiHostCpu");
				                                  <%}else{%>
				                                  var _div=document.getElementById("wmiHostCpu");
	                                          ...
	                                          var so = new SWFObject("<%=rootPath%>/amchart/amcolumn.swf", "amcolumn","242", "210", "8", "#FFFFFF");
				                                  so.addVariable("path", "<%=rootPath%>/amchart/");
				                                  so.addVariable("settings_file", escape("<%=rootPath%>/amcharts_settings/hostdisk_settings.xml"));
				                                  so.addVariable("chart_data","<%=valueStr%>");
				                                  so.write("wmiHostdisk");
				                                  <%}else{%>
				                                  var _div=document.getElementById("wmiHostdisk");
												  var img=document.createElement("img");
													  img.setAttribute("src","<%=rootPath%>/resource/image/nodata.gif");
													 _div.appendChild(img);
				                                  <%}%>
