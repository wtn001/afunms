	...
	<head>
		...
		<script type="text/javascript">
			...
			var curpage= <%=jp.getCurrentPage()%>;
  			...
  			var listAction = "<%=rootPath%>/network.do?action=monitorroutelist";
			...
			function doCancelManage(){  
  			...
  			function doSearch(){  
				mainForm.action = "<%=rootPath%>/network.do?action=monitorroutelist";
     			mainForm.submit();
  			}
											...
											<td>
												<table id="content-header" class="content-header">
								                	<tr>
									                	...
									                	<td class="content-title"> 资源 >> 性能监视 >> 监视对象一览 >> 路由器列表 </td>
		        										...
		        										<td>
		        											<table>
		        												<tr>
		        													...
		        													<td align="center" class="body-data-title" width="10%"><a href="#" onclick="showSort('name')">名称</a><%=nameImg%></td>
		        													...
		        													<td align="center" class="body-data-title" width="10%"><a href="#" onclick="showSort('inutilhdx')">出口流速(KB/S)</a><%=inutilhdxImg%></td>
		        													<td align="center" class="body-data-title" width="10%"><a href="#" onclick="showSort('oututilhdx')">入口流速(KB/S)</a><%=oututilhdxImg%></td>
		        													<%-- <td align="center" class="body-data-title" width="10%">告警</td>--%>
		        													<td align="center" class="body-data-title" width="10%">接口数量</td>
					        									        ...
					        									        for(int i = 0 ; i < list.size() ; i++){
					        									            ...
					        									            <tr <%=onmouseoverstyle%>>
					        													...
					        													<%
																				 ...
																				 List iplist = ipdao.loadByIpaddress(monitorNetDTO.getIpAddress());
																				 ipdao.close();
																				 ipdao = new IpAliasDao();
					                     											 IpAlias ipalias = ipdao.getByIpAndUsedFlag(monitorNetDTO.getIpAddress(),"1"); 
					                     											 ipdao.close();
					        													...
					        													<td align="left" class="body-data-list">
					        														<table>
													                      				<tr>
													                      					<td>
								        														<select style="width:110px;" name="ipalias<%=monitorNetDTO.getIpAddress() %>">
														                  							<option selected><%=monitorNetDTO.getIpAddress() %></option>
									       															<% 
								                    											...
								                    											<img href="#" src="<%= rootPath%>/resource/image/menu/xgmm.gif" style="cursor:hand" onclick="modifyIpAliasajax('<%=monitorNetDTO.getIpAddress()%>');"/>
                    																		</td>
                    																	</tr>
												                      				</table>
                    															</td>
					        													...
					        													--%>
					        													<td align="center" class="body-data-list"><%=monitorNetDTO.getEntityNumber()%></td>
