...
// document.all.divLayer.appendChild(divBackGround);
		...
		img.style.width = inj.offsetWidth;
		img.style.height = inj.offsetHeight;
...
*/
		...
		divText.style.left = parseInt(x, 10) - 10;
		divText.style.top = parseInt(y, 10) + inj.offsetHeight;
		...
		document.all("node_" + id).ondblclick = function() {
		                                         if(this.name.substring(0,this.name.lastIndexOf(","))!=""){
			                                         ...
			                                         //showalert(nodeId);
			                                         window.open("/afunms/detail/dispatcher.jsp?flag=1&id=" + nodeId,"fullScreenWindow", "toolbar=no,height="+window.screen.height + ","+"width=" + (window.screen.width-8)+ ",scrollbars=yes,resizable=yes"+"screenX=0,screenY=0");
...
//		line.appendChild(stroke);
	...
	//	    divLineText.style.filter="Alpha(Opacity=60);";
		...
		line.onmousemove = (function(lineid){
				return function(e) { 
