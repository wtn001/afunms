...
<%
  ...
  String zoomStr = "0.5";//request.getParameter("zoom");
  String viewFile = "network.jsp";//(String)session.getAttribute(SessionConstant.HOME_TOPO_VIEW);  
...
<script type="text/javascript">
	...
	{
	    window.location = "home_showMap.jsp?zoom=<%=zoom%>";
	}
</script>
<style>
...
document.write('<div id="divLayer" style="width:2000px;height:1000px; background:url(<%=rootPath%>/resource/image/bg/<%=bg%>) left top no-repeat; top:0px;left:0px; position:absolute;" onmousedown="divLayerDown()" onclick="javascript:closeLineFrame();"></div>');//#000066
...
divLayer.ondblclick = function() {
	window.parent.fatherXML = '<%=viewFile%>';
...
parent.topFrame.showController(true);
