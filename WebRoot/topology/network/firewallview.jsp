...
<%
 String runmodel = PollingEngine.getCollectwebflag();//采集与访问模式
	...
	if("0".equals(runmodel)){
        ...
        String[] netInterfaceItem={"index","ifDescr","ifSpeed","ifOperStatus","ifOutBroadcastPkts","ifInBroadcastPkts","ifOutMulticastPkts","ifInMulticastPkts","OutBandwidthUtilHdx","InBandwidthUtilHdx"}; try{
...
function showPort(){
	...
	mainForm.action = "<%=rootPath%>/topology/network/firewallview.jsp?id=<%=tmp%>&flag=<%=flag%>&showAllPortFlag="+showAllPortFlag;
    mainForm.submit();
}
...
function changeOrder(para){
  	location.href="<%=rootPath%>/topology/network/firewallview.jsp?id=<%=tmp%>&ipaddress=<%=host.getIpAddress()%>&flag=<%=flag1%>&orderflag="+para+"&showAllPortFlag=<%=showAllPortFlag%>";
} 
...
<script language="JavaScript">
	...
	*/
	...
	{
		window.open ('<%=rootPath%>/monitor.do?action=portdetail&id=<%=tmp%>&ip=<%=host.getIpAddress()%>&ifindex='+node,'端口流速','top=200,left=300,height=380 ,width=720, toolbar=no, menubar=no, scrollbars=yes, resizable=no, location=no, status=no')
	}	
	function bandwidth()
	{
		window.open ('<%=rootPath%>/monitor.do?action=bandwidthdetail&id=<%=tmp%>&ip=<%=host.getIpAddress()%>&ifindex='+node,'带宽','top=200,left=300,height=380 ,width=720, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no')
	}	
	function portstatus()
	{
		window.open ('<%=rootPath%>/portconfig.do?action=showPortStatus&index='+node+'&ip=<%=host.getIpAddress()%>','端口状态','top=200,left=300,height=380 ,width=780, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no')
	}	
</script>
...
function setClass(){
	document.getElementById('firewallDetailTitle-0').className='detail-data-title';
	document.getElementById('firewallDetailTitle-0').onmouseover="this.className='detail-data-title'";
	document.getElementById('firewallDetailTitle-0').onmouseout="this.className='detail-data-title'";
}
					...
					<table id="container-main" class="container-main">
						<tr>
															...
															<table class="detail-data-body">
																<tr>
																  ...
																  <tr align="center"> 
																    ...
																    <td rowspan="2" width="11%">传输速率(kb/s)</td>
																  ...
																  <tr align="center"> 
																    <td  colspan="12" width="100%" align="right" bordercolor="#ECECEC">
																    	<select name="allportflag" id="allportflag">
																    		<option value="0" label="显示所有端口"></option>
																    		<%if("1".equals(showAllPortFlag)){%>
																    		<option value="1" label="显示关键端口" selected></option>
																			<%}else{ %>
																			<option value="1" label="显示关键端口"></option>
																			<%} %>
																    	</select>
																    	<input type="button" onclick="showPort()" value="提交"/>&nbsp;
																    </td>
																  </tr>
																             <%
							...
							<td class="td-container-main-tool">
								<% System.out.println("nodedto.getSubtype():==="+nodedto.getSubtype());%>
