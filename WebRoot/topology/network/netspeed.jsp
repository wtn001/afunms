...
function showPort(){
	...
	mainForm.action = "<%=rootPath%>/monitor.do?action=speed&id=<%=tmp%>&flag=<%=flag%>&showAllPortFlag="+showAllPortFlag;
    mainForm.submit();
}
...
function changeOrder(para){
  	location.href="<%=rootPath%>/monitor.do?action=speed&id=<%=tmp%>&ipaddress=<%=host.getIpAddress()%>&flag=<%=flag1%>&orderflag="+para+"&showAllPortFlag=<%=showAllPortFlag%>";
} 
...
<!-- 右键菜单结束-->
  ...
  <tr align="center"> 
    ...
    <td rowspan="2" width="11%" align="center">传输速率(kb/s)</td>
