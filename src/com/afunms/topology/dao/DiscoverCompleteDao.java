...
public class DiscoverCompleteDao extends BaseDao {
	...
	public void addLinkData(List linkList) {
			...
			for (int k = 0; k < repairlist.size(); k++) {
				...
				if (starthost.getIpAddress().equalsIgnoreCase(
						...
						&& link.getEndIndex().equalsIgnoreCase(
								repairlink.getEndIndex())) {
					// 存在修改过的连接关系
					SysLogger.info("存在修改过的连接关系!" + starthost.getIpAddress()
		...
		for (int i = 0; i < linkList.size(); i++) {
			...
			if (donelinkhashtable.containsKey(link.getStartIp() + "_"
					+ link.getStartIndex() + "/" + link.getEndIp() + "_"
					+ link.getEndIndex())) {
				continue;
			} else {
				donelinkhashtable.put(
						link.getStartIp() + "_" + link.getStartIndex() + "/"
	...
	public void addInterfaceData(List hostList) {
		...
		for (int i = 0; i < hostList.size(); i++) {
			...
			for (int j = 0; j < ifList.size(); j++) {
				...
				String physAddress = ifEntity.getPhysAddress().replace("'", "")
						.replace("<", "").replace(">", "");
		...
		for (int k = 0; k < calhostlist.size(); k++) {
			...
			if (aliaslist != null && aliaslist.size() > 0) {
				for (int m = 0; m < aliaslist.size(); m++) {
					aliasHash.put((String) aliaslist.get(m),
							host.getIpAddress());
				}
			}
		}
					...
					if (DiscoverEngine.getInstance().getDiscovermodel() == 1) {
						...
						if (node.getDiscoverstatus() == -1) {
							...
							ctable.createTable(conn, "allutilhdx", allipstr,
									"allhdx");
							ctable.createTable(conn, "autilhdxh", allipstr,
									"ahdxh");
							ctable.createTable(conn, "autilhdxd", allipstr,
									"ahdxd");
							...
							ctable.createTable(conn, "inpacks", allipstr,
									"inpacks");
							ctable.createTable(conn, "ipacksh", allipstr,
									"ipacksh");
							ctable.createTable(conn, "ipackd", allipstr,
									"ipackd");

							// 出口数据包
							ctable.createTable(conn, "outpacks", allipstr,
									"outpacks");
							ctable.createTable(conn, "opackh", allipstr,
									"opackh");
							ctable.createTable(conn, "opacksd", allipstr,
									"opacksd");

							// 温度
							ctable.createTable(conn, "temper", allipstr,
									"temper");
							ctable.createTable(conn, "temperh", allipstr,
									"temperh");
							ctable.createTable(conn, "temperd", allipstr,
									"temperd");
						}
					} else {
						// 重新发现
					...
					if (DiscoverEngine.getInstance().getDiscovermodel() == 1) {
						...
						if (node.getDiscoverstatus() == -1) {
							...
							ctable.createTable(conn, "diskincre", allipstr,
									"diskincre");// 磁盘增长率yangjun
							ctable.createTable(conn, "diskinch", allipstr,
									"diskinch");// 磁盘增长率小时
							ctable.createTable(conn, "diskincd", allipstr,
									"diskincd");// 磁盘增长率天
							 ...
							 * ctable
							 * .createTable("diskhour",allipstr,"diskhour");
						...
						ctable.createTable(conn, "cpudtl", allipstr, "cpudtl");// CPU
																				// detail
						ctable.createTable(conn, "cpudtlhour", allipstr,
								"cpudtlhour");// CPU detail
						ctable.createTable(conn, "cpudtlday", allipstr,
								"cpudtlday");// CPU detail
		...
		for (int i = 0; i < hostList.size(); i++) {
			try {
				...
				try {
					...
					if (aliasList != null && aliasList.size() > 0) {
						for (int k = 0; k < aliasList.size(); k++) {
							...
							// SysLogger.info("#### put "+vo.getAliasip()
							// +" to ipaliasHash ####");
				...
				try {
					...
					if (hostlist != null && hostlist.size() > 0) {
						for (int k = 0; k < hostlist.size(); k++) {
							...
							nodelistHash.put(_node.getIpAddress(),
									_node.getIpAddress());
						}
					}
				} catch (Exception e) {
				...
				if (ipaliasHash.containsKey(node.getIpAddress())) {
					// 已经存在该设备
					String nodeIp = "";
					if (nodelistHash.containsKey(node.getIpAddress()))
						nodeIp = (String) nodelistHash.get(node.getIpAddress());
					if (nodeIp == null || nodeIp.trim().length() == 0) {
						if (ipaliasHash.containsKey(node.getIpAddress()))
							nodeIp = (String) ipaliasHash.get(node
									.getIpAddress());
					}
					com.afunms.polling.node.Host _host = null;
					if (nodeIp != null && nodeIp.trim().length() > 0) {
						_host = (com.afunms.polling.node.Host) PollingEngine
								.getInstance().getNodeByIp(nodeIp);
						if (_host != null) {
							// 只计算链路关系
							continue;
						}
				...
				if ((node.getCategory() > 0 && node.getCategory() < 4)
						|| node.getCategory() == 7 || node.getCategory() == 8
						|| node.getCategory() == 9 || node.getCategory() == 10
						|| node.getCategory() == 11) {
					if (node.getDiscoverstatus() == -1) {
						// 新发现的设备
						// 生成网络设备表
					...
					if (node.getCollecttype() == 3) {
						...
						ctable.createTable(conn, "statushour", allipstr,
								"statushour");// 通道状态按小时
						ctable.createTable(conn, "statusday", allipstr,
								"statusday");// 通道状态按天

						ctable.createCiscoCMTSTable(conn, "noise", allipstr,
								"noise");// 通道信躁比
						ctable.createCiscoCMTSTable(conn, "noisehour",
								allipstr, "noisehour");// 通道信躁比按小时
						ctable.createCiscoCMTSTable(conn, "noiseday", allipstr,
								"noiseday");// 通道信躁比按天

						ctable.createCiscoCMTSIPMACTable(conn, "ipmac",
								allipstr, "ipmac");// IPMAC信息（在线用户信息）

						ctable.createTable(conn, "utilhdxpercs", allipstr,
								"hdpercs");
						ctable.createTable(conn, "hdxperchours", allipstr,
								"hdperchours");
						ctable.createTable(conn, "hdxpercdays", allipstr,
								"hdpercdays");

						ctable.createTable(conn, "utilhdxs", allipstr, "hdxs");
						ctable.createTable(conn, "utilhdxhours", allipstr,
								"hdxhours");
						ctable.createTable(conn, "utilhdxdays", allipstr,
								"hdxdays");

						ctable.createTable(conn, "allutilhdxs", allipstr,
								"allhdxs");
						ctable.createTable(conn, "autilhdxhs", allipstr,
								"ahdxhs");
						ctable.createTable(conn, "autilhdxds", allipstr,
								"ahdxds");

						ctable.createTable(conn, "discardspercs", allipstr,
								"dcardpercs");
						ctable.createTable(conn, "dcarperhs", allipstr,
								"dcarperhs");
						ctable.createTable(conn, "dcarperds", allipstr,
								"dcarperds");

						ctable.createTable(conn, "errorspercs", allipstr,
								"errpercs");
						ctable.createTable(conn, "errperchs", allipstr,
								"errperchs");
						ctable.createTable(conn, "errpercds", allipstr,
								"errpercds");

						ctable.createTable(conn, "packss", allipstr, "packss");
						ctable.createTable(conn, "packshours", allipstr,
								"packshours");
						ctable.createTable(conn, "packsdays", allipstr,
								"packsdays");

						ctable.createTable(conn, "inpackss", allipstr,
								"inpackss");
						ctable.createTable(conn, "ipackshs", allipstr,
								"ipackshs");
						ctable.createTable(conn, "ipackds", allipstr, "ipackds");

						ctable.createTable(conn, "outpackss", allipstr,
								"outpackss");
						ctable.createTable(conn, "opackhs", allipstr, "opackhs");
						ctable.createTable(conn, "opacksds", allipstr,
								"opacksds");
					}
					...
					if (node.getCollecttype() == 3) {
						...
						ctable.createTable(conn, "pinghours", allipstr,
								"pinghours");// Ping
						ctable.createTable(conn, "pingdays", allipstr,
								"pingdays");// Ping
					} else if (node.getCollecttype() == 7
							|| (node.getSysOid() != null && node.getSysOid()
									.trim()
									.startsWith("1.3.6.1.4.1.11.2.3.7.11"))) {
						// hp storage
						ctable.createTable(conn, "ping", allipstr, "ping");// Ping
						ctable.createTable(conn, "pinghour", allipstr,
								"pinghour");// Ping
						ctable.createTable(conn, "pingday", allipstr, "pingday");// Ping
					} else if (node.getSysOid() != null
							&& node.getSysOid().trim()
									.startsWith("1.3.6.1.4.1.789.")) {
						// NETAPP存储
						ctable.createTable(conn, "pings", allipstr, "pings");// Ping
						ctable.createTable(conn, "pingshour", allipstr,
								"pingshour");// Ping
						ctable.createTable(conn, "pingsday", allipstr,
								"pingsday");// Ping
						...
						ctable.createTable(conn, "rsluncon", allipstr,
								"rsluncon");
						...
						ctable.createEmcTable(conn, "emcdiskper", allipstr,
								"emcdiskper");
						ctable.createEmcTable(conn, "emclunper", allipstr,
								"emclunper");
						ctable.createEmcTable(conn, "emcenvpower", allipstr,
								"emcenvpower");
						ctable.createEmcTable(conn, "emcenvstore", allipstr,
								"emcenvstore");
						ctable.createEmcTable(conn, "emcbakpower", allipstr,
								"emcbakpower");
					...
					if (node.getCollecttype() == 3) {
						...
						ctable.createVMhostTable(conn, "vm_host", allipstr,
								"vm_host");// 创建VMWare 物理机的性能信息表
						ctable.createVMguesthostTable(conn, "vm_guesthost",
								allipstr, "vm_guesthost");// 创建VMWare 虚拟机的性能信息表
						ctable.createVMCRTable(conn, "vm_cluster", allipstr,
								"vm_cluster");// 创建VMWare 集群的性能信息表
						ctable.createVMDSTable(conn, "vm_datastore", allipstr,
								"vm_datastore");// 创建VMWare 存储的性能信息表
						ctable.createVMRPTable(conn, "vm_resourcepool",
								allipstr, "vm_resourcepool");// 创建VMWare
																// 资源池的性能信息表
						// vm_basephysical
						ctable.createVMBaseTable(conn, "vm_basephysical",
								allipstr, "vm_basephysical");// 创建VMWare
																// 物理机的基础信息表
						ctable.createVMBaseTable(conn, "vm_basevmware",
								allipstr, "vm_basevmware");// 创建VMWare 虚拟机的基础信息表
						ctable.createVMBaseTable(conn, "vm_baseyun", allipstr,
								"vm_baseyun");// 创建VMWare 云资源的基础信息表
						ctable.createVMBaseTable(conn, "vm_basedatastore",
								allipstr, "vm_basedatastore");// 创建VMWare
																// 存储的基础信息表
						ctable.createVMBaseTable(conn, "vm_basedatacenter",
								allipstr, "vm_basedatacenter");// 创建VMWare
																// 数据中心的基础信息表
						ctable.createVMBaseTable(conn, "vm_baseresource",
								allipstr, "vm_baseresource");// 创建VMWare
																// 资源池的基础信息表
					...
					if (node.getCollecttype() == 3) {
						...
						ctable.createTable(conn, "software", allipstr,
								"software");
						// System.out.println("com.afunms.topology.dao---->DiscoverCompleteDao.java----1502hang------==>"+node.getSysOid());
						if (node.getSysOid().equalsIgnoreCase(
								"1.3.6.1.4.1.2.3.1.2.1.1")) {
							// 生成换页率
							ctable.createTable(conn, "pgused", allipstr,
									"pgused");
							ctable.createTable(conn, "pgusedhour", allipstr,
									"pgusedhour");
							ctable.createTable(conn, "pgusedday", allipstr,
									"pgusedday");
						}
					}
					...
					ctable.createTable(conn, "outputhour", allipstr,
							"outputhour");// output
				...
				if (bridgeAddress != null
						&& !"".equalsIgnoreCase(bridgeAddress)) {
					bridgeAddress = CommonUtil
							.removeIllegalStr(bridgeAddress.replace("'", "")
									.replace("<", "").replace(">", ""));
				}
	...
	private void addTelnetConfig(int node_id) {
		if (telnetID == 0)
			...
			telnetID++;
		conn.executeUpdate("insert into server_telnet_config(id,node_id)values("
				+ telnetID + "," + node_id + ")");
	}
