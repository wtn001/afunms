...
public class PerformanceManager extends BaseManager implements ManagerInterface {
	...
	private String update() {
				...
				if (host.getCategory() < 4 || host.getCategory() == 7
					...
					try {
						ctable.deleteTable(conn, "discardsperc", allipstr,
								"dcardperc");
						ctable.deleteTable(conn, "dcarperh", allipstr,
								"dcarperh");
						ctable.deleteTable(conn, "dcarperd", allipstr,
								"dcarperd");
					} catch (Exception e) {
						e.printStackTrace();
					}
					...
					try {
						ctable.deleteTable(conn, "errorsperc", allipstr,
								"errperc");
						ctable.deleteTable(conn, "errperch", allipstr,
								"errperch");
						ctable.deleteTable(conn, "errpercd", allipstr,
								"errpercd");
					} catch (Exception e) {
						e.printStackTrace();
					}
					...
					try {
						...
						ctable.createTable(conn, "discardsperc", allipstr,
								"dcardperc");
						ctable.createTable(conn, "dcarperh", allipstr,
								"dcarperh");
						ctable.createTable(conn, "dcarperd", allipstr,
								"dcarperd");

						ctable.createTable(conn, "errorsperc", allipstr,
								"errperc");
						ctable.createTable(conn, "errperch", allipstr,
								"errperch");
						ctable.createTable(conn, "errpercd", allipstr,
								"errpercd");
		...
		try {
			boolean result = timeShareConfigUtil.saveTimeShareConfigList(
					request, String.valueOf(vo.getId()),
					timeShareConfigUtil.getObjectType("0"));
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		try {
			boolean result2 = timeGratherConfigUtil.saveTimeGratherConfigList(
					request, String.valueOf(vo.getId()),
					timeGratherConfigUtil.getObjectType("0"));
		} catch (Exception e) {
			e.printStackTrace();
		}
		/* snow close */
			...
			for (int i = 0; i < ids.length; i++) {
				...
				try {
					if (host.getCategory() < 4 || host.getCategory() == 7
						...
						try {
							ctable.deleteTable(conn, "autilhdxh", allipstr,
									"ahdxh");
						} catch (Exception e) {

						}
						try {
							ctable.deleteTable(conn, "autilhdxd", allipstr,
									"ahdxd");
						} catch (Exception e) {
						...
						try {
							ctable.deleteTable(conn, "ipacksh", allipstr,
									"ipacksh");
						} catch (Exception e) {

						}
						try {
							ctable.deleteTable(conn, "ipackd", allipstr,
									"ipackd");
						} catch (Exception e) {
						...
						try {
							ctable.deleteTable(conn, "opackh", allipstr,
									"opackh");
						} catch (Exception e) {

						}
						try {
							ctable.deleteTable(conn, "opacksd", allipstr,
									"opacksd");
						} catch (Exception e) {
						...
						try {
							ctable.deleteTable(conn, "temperh", allipstr,
									"temperh");
						} catch (Exception e) {

						}
						try {
							ctable.deleteTable(conn, "temperd", allipstr,
									"temperd");
						} catch (Exception e) {
						...
						DiscoverCompleteDao dcDao = new DiscoverCompleteDao();
						try {
							dcDao.deleteMonitor(host.getId(),
									host.getIpAddress());
						} catch (Exception e) {
							e.printStackTrace();
						} finally {
						...
						try {
							ctable.deleteTable(conn, "diskinch", allipstr,
									"diskinch");// 磁盘增长
						} catch (Exception e) {

						}
						try {
							ctable.deleteTable(conn, "diskincd", allipstr,
									"diskincd");// 磁盘增长
						} catch (Exception e) {
				...
				if (list != null && list.size() > 0) {
					for (int j = 0; j < list.size(); j++) {
						...
						if (vo != null) {
							...
							if (nodeDependDao.isNodeExist("net" + id,
									vo.getXmlfile())) {
								nodeDependDao.deleteByIdXml("net" + id,
										vo.getXmlfile());
							} else {
								nodeDependDao.close();
							}
									...
									chartxml = new ChartXml("NetworkMonitor",
											"/"
													+ vo.getXmlfile().replace(
															"jsp", "xml"));
									chartxml.addBussinessXML(
											manageXml.getTopoName(), lists);
	...
	private String add() {
		 ...
		 *         helper.addHost(ipAddress,alias,community
		 *         ,writecommunity,type,ostype, collecttype); //加入一台服务器
		 */
		...
		if (supperid >= 0) {
			if (collecttype == 3) {
				if (PollingEngine.getAddiplist() != null
						&& PollingEngine.getAddiplist().size() > 0) {
					List list = PollingEngine.getAddiplist();
					for (int i = 0; i < list.size(); i++) {
						Hashtable hst = (Hashtable) list.get(i);
						NetworkUtil test = new NetworkUtil();
						List listip = test.parseAllIp(
								(String) hst.get("startip"),
								(String) hst.get("endip"));
						for (int j = 0; j < listip.size(); j++) {
							String iplist = (String) listip.get(j);
							addResult = helper.addHost(assetid, location,
									iplist, iplist, snmpversion, community,
									writecommunity, transfer, type, ostype,
									collecttype, bid, sendmobiles, sendemail,
									sendphone, supperid, managed,
									securityLevel, securityName, v3_ap,
									authPassPhrase, v3_privacy,
									privacyPassPhrase);
							Host node = (Host) PollingEngine.getInstance()
									.getNodeByIP(iplist);
							try {
									...
									if (node.getCategory() == 4) {
										...
										try {
											...
											if (node.getCollecttype() == 1) {// snmp方式采集
												...
												NodeGatherIndicatorsUtil nodeGatherIndicatorsUtil = new NodeGatherIndicatorsUtil();
												nodeGatherIndicatorsUtil
														.addGatherIndicatorsOtherForNode(
																node.getId()
																		+ "",
																AlarmConstant.TYPE_HOST,
																getSutType(node
					...
					addResult = helper.addHost(assetid, location, ipAddress,
							alias, snmpversion, community, writecommunity,
							transfer, type, ostype, collecttype, bid,
							sendmobiles, sendemail, sendphone, supperid,
							managed, securityLevel, securityName, v3_ap,
							authPassPhrase, v3_privacy, privacyPassPhrase);
				} else {
					addResult = helper.addHost(assetid, location, ipAddress,
							alias, snmpversion, community, writecommunity,
							transfer, type, ostype, collecttype, bid,
				...
				addResult = helper.addHost(assetid, location, ipAddress, alias,
						...
						sendphone, supperid, managed, securityLevel,
						securityName, v3_ap, authPassPhrase, v3_privacy,
						privacyPassPhrase);
			}
		}
		...
		if (addResult > 0) {
			...
			try {
				...
				String strSql = "";
				strSql = "insert into nms_netsyslogrule_node(nodeid,facility)values('"
				...
				try {
					boolean resutl = timeShareConfigUtil
							.saveTimeShareConfigList(request,
				...
				try {
					boolean result2 = timeGratherConfigUtil
							.saveTimeGratherConfigList(request,
									String.valueOf(addResult),
									timeGratherConfigUtil.getObjectType("0"));
				} catch (Exception e) {
					e.printStackTrace();
			...
			if (node.getEndpoint() == 2) {
				...
				if (node.getCategory() == 4) {
					...
					try {
						...
						// //snmp方式采集
						NodeGatherIndicatorsUtil nodeGatherIndicatorsUtil = new NodeGatherIndicatorsUtil();
						nodeGatherIndicatorsUtil.addGatherIndicatorsForNode(
								node.getId() + "", AlarmConstant.TYPE_HOST,
					...
					try {
						AlarmIndicatorsUtil alarmIndicatorsUtil = new AlarmIndicatorsUtil();
						alarmIndicatorsUtil.saveAlarmInicatorsThresholdForNode(
								node.getId() + "", AlarmConstant.TYPE_NET,
								getSutType(node.getSysOid()));
					...
					try {
						AlarmIndicatorsUtil alarmIndicatorsUtil = new AlarmIndicatorsUtil();
						alarmIndicatorsUtil.saveAlarmInicatorsThresholdForNode(
								node.getId() + "", AlarmConstant.TYPE_FIREWALL,
								getSutType(node.getSysOid()));
					...
					try {
						AlarmIndicatorsUtil alarmIndicatorsUtil = new AlarmIndicatorsUtil();
						alarmIndicatorsUtil.saveAlarmInicatorsThresholdForNode(
								node.getId() + "", AlarmConstant.TYPE_STORAGE,
								getSutType(node.getSysOid()));
						NodeGatherIndicatorsUtil nodeGatherIndicatorsUtil = new NodeGatherIndicatorsUtil();
						nodeGatherIndicatorsUtil.addGatherIndicatorsForNode(
								node.getId() + "", AlarmConstant.TYPE_STORAGE,
					...
					if (node.getCollecttype() == SystemConstant.COLLECTTYPE_PING) {
						...
						try {
							...
							try {
								...
								vmware.setPwd(EncryptUtil
										.encode(vmwarepassword));
								vmware.setHosturl("");
								vmware.setBak("");
								...
								try {
									SysLogger.info(node.getIpAddress()
									...
									if (vmids != null && vmids.size() > 0) {
										Iterator iterator = vmids.keySet()
												.iterator();
										List vmwarevids = new ArrayList();
										while (iterator.hasNext()) {
											String vmid = (String) iterator
													.next();
											String guestname = (String) vmids
													.get(vmid);
											VMWareVid vmwarevid = new VMWareVid();
											vmwarevid.setNodeid(Long
													.parseLong(node.getId()
															+ ""));
									...
									try {
										String url = "https://"
												+ node.getIpAddress() + "/sdk";
										HashMap<String, Object> summaryresultMap = (HashMap<String, Object>) VIMMgr
												.syncVIMObjs(url,
														vmwareusername,
														vmwarepassword);
										ArrayList<HashMap<String, Object>> dslist = (ArrayList<HashMap<String, Object>>) summaryresultMap
												.get("Datastore");
										ArrayList<HashMap<String, Object>> wulist = (ArrayList<HashMap<String, Object>>) summaryresultMap
												.get("HostSystem");
										ArrayList<HashMap<String, Object>> rplist = (ArrayList<HashMap<String, Object>>) summaryresultMap
												.get("ResourcePool");
										ArrayList<HashMap<String, Object>> vmlist = (ArrayList<HashMap<String, Object>>) summaryresultMap
												.get("VirtualMachine");
										ArrayList<HashMap<String, Object>> dclist = (ArrayList<HashMap<String, Object>>) summaryresultMap
												.get("Datacenter");
										ArrayList<HashMap<String, Object>> crlist = (ArrayList<HashMap<String, Object>>) summaryresultMap
												.get("ComputeResource");// 群集
										PerformancePanelAjaxManager per = new PerformancePanelAjaxManager();
										per.savePhysical(wulist, node.getId()
												+ "", node.getIpAddress());
										per.saveVmware(vmlist, node.getId()
												+ "", node.getIpAddress());
										per.saveDatastore(dslist, node.getId()
												+ "", node.getIpAddress());
										per.saveResourcepool(rplist,
												node.getId() + "",
												node.getIpAddress());
										per.saveDatacenter(dclist, node.getId()
												+ "", node.getIpAddress());
										per.saveYun(crlist, node.getId() + "",
												node.getIpAddress());
										// 初始化告警
										if (wulist != null && wulist.size() > 0) {
											String vid = "";
											for (int i = 0; i < wulist.size(); i++) {
												vid = wulist.get(i).get("vid")
														.toString();
												AlarmIndicatorsUtil alarmIndicatorsUtil = new AlarmIndicatorsUtil();
												alarmIndicatorsUtil
														.VMsaveAlarmInicatorsThresholdForNode(
																node.getId()
																		+ "",
																AlarmConstant.TYPE_VIRTUAL,
																getSutType(node
										...
										SysLogger.info("================采集数据失败"
												+ node.getIpAddress());
										e.printStackTrace();
									}
									SysLogger.info("================结束采集VMWARE"
											+ node.getIpAddress());
								} catch (Exception e) {
									e.printStackTrace();
								}
							...
							// vmwareusername, vmwarepassword);//获取vmware返回的所有数据
							//
							// ArrayList<HashMap<String,Object>> VMlist = new
							// ArrayList<HashMap<String,Object>>();
							// VMlist =
							// (ArrayList<HashMap<String,Object>>)VIMmap.get(VIMConstants.SYNC_VM);//获得虚机数据
							//
							//
							// HashMap<String, Object> VMmap = new
							// HashMap<String, Object>();
							// if(VMlist.size()==0){
							// System.out.println("没有发现虚机！");
							// }else{
							// for(int i=0;i<VMlist.size();i++){
							// String vid =
			...
			if (hostlist != null && hostlist.size() > 0) {
				for (int i = 0; i < hostlist.size(); i++) {
					...
					if (nodehash.containsKey(_node.getCategory() + "")) {
						((List) nodehash.get(_node.getCategory() + ""))
								.add(_node);
					} else {
							...
							if (hostdata != null) {
								if (hostdata.getSubentity() != null) {
									if (hostdata.getSubentity().equals(
											"ConnectUtilization")) {
										reporthash.put("time",
												hostdata.getCollecttime());
										reporthash.put("Ping",
												hostdata.getThevalue());
	...
	public MonitorNodeDTO getMonitorNodeDTOByHostNode(HostNode hostNode) {
		...
		String chexkname = nodeDTO.getId() + ":" + nodeDTO.getType() + ":"
				+ nodeDTO.getSubtype() + ":";
		for (Iterator it = checkEventHashtable.keySet().iterator(); it
				.hasNext();) {
			try {
		...
		if ("0".equals(runmodel)) {
				...
				if (memoryVector != null && memoryVector.size() > 0) {
					for (int si = 0; si < memoryVector.size(); si++) {
						...
						if (memorydata.getEntity().equalsIgnoreCase(
								"Utilization")) {
							if (category == 4
									&& hostNode.getSysOid().startsWith(
											"1.3.6.1.4.1.2.3.1.2.")) {// 服务器的情况
								if (memorydata.getSubentity().equalsIgnoreCase(
										"PhysicalMemory")) {
									memeryValueDouble = Double
											.valueOf(memorydata.getThevalue());
								}
								if (memorydata.getSubentity().equalsIgnoreCase(
										"SwapMemory")) {
									virtualMemeryValueDouble = Double
											.valueOf(memorydata.getThevalue());
								}
							} else if (category == 4) {
								if (memorydata.getSubentity().equalsIgnoreCase(
					...
					if (memoryVector != null && memoryVector.size() > 0) {
						for (int i = 0; i < memoryVector.size(); i++) {
							Memorycollectdata memorycollectdata = (Memorycollectdata) memoryVector
									.get(i);
							allmemoryvalue = allmemoryvalue
									+ Integer.parseInt(memorycollectdata
											.getThevalue());
						}
			...
			if (SystemConstant.COLLECTTYPE_SSH == hostNode.getCollecttype()
					|| SystemConstant.COLLECTTYPE_SHELL == hostNode
							.getCollecttype()) {
				if (ipAllData != null
						&& ipAllData.containsKey("interfaceNumber"))
					interfaceNubmer = (Integer) ipAllData
							.get("interfaceNumber");
		...
		if (memoryValueStr != null) {
			...
			memoryValueDouble = Double
					.parseDouble(df.format(memoryValueDouble));
		}
		// 设定cpu和内存的告警颜色
		if (category == 4) {
			type = "host";
		...
		try {
			nodeMonitorList = nmDao.findByNodeIdAndTypeAndSubType(nodeId + "",
					type, "");
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
		...
		if (nodeMonitorList != null) {
			for (int j = 0; j < nodeMonitorList.size(); j++) {
				AlarmIndicatorsNode nodeMonitor = (AlarmIndicatorsNode) nodeMonitorList
						.get(j);
				if ("cpu".equals(nodeMonitor.getName())) {
					if (Double.parseDouble(cpuValue) > Double
							.parseDouble(nodeMonitor.getLimenvalue2())) {
						cpuValueColor = "red";
					} else if (Double.parseDouble(cpuValue) > Double
							.parseDouble(nodeMonitor.getLimenvalue1())) {
						cpuValueColor = "orange";
					} else if (Double.parseDouble(cpuValue) > Double
							.parseDouble(nodeMonitor.getLimenvalue0())) {
				...
				if ("memory".equals(nodeMonitor.getName())) {
					if (Double.parseDouble(memoryValue) > Double
							.parseDouble(nodeMonitor.getLimenvalue2())) {
						memoryValueColor = "red";
					} else if (Double.parseDouble(memoryValue) > Double
							.parseDouble(nodeMonitor.getLimenvalue1())) {
				...
				if ("SwapMemory".equals(nodeMonitor.getName())) {
					if (Double.parseDouble(virtualMemoryValue) > Double
							.parseDouble(nodeMonitor.getLimenvalue2())) {
						virtualMemoryValueColor = "red";
					} else if (Double.parseDouble(virtualMemoryValue) > Double
							.parseDouble(nodeMonitor.getLimenvalue1())) {
			...
			if (monitornodelist != null) {
				try {
					...
					for (int i = 0; i < monitornodelist.size(); i++) {
						...
						List list_vid = viddao.queryVidFlag("datastore",
								monitorNodeDTO.getId() + "", "");
						List<HashMap<String, Object>> ds_list = vmdao.getbyvid(
								list_vid, "vm_basedatastore",
								monitorNodeDTO.getIpAddress());
						for (int j = 0; j < ds_list.size(); j++) {
							double num = Double.parseDouble(ds_list.get(j)
									.get("capacity").toString()
									.replaceAll(" GB", ""));
							l.add(num);
						}
						...
						hostnum = allHostnum.get(hostNode.getId() + "") != null ? allHostnum
								.get(hostNode.getId() + "") : "0";
						vmnum = allVMnum.get(hostNode.getId() + "") != null ? allVMnum
								.get(hostNode.getId() + "") : "0";
						poolnum = allPoolnum.get(hostNode.getId() + "") != null ? allPoolnum
								.get(hostNode.getId() + "") : "0";
			...
			if (node.getCollecttype() == SystemConstant.COLLECTTYPE_SHELL) {
				...
				try {
					if (node.getOstype() == 6) {
						...
						try {
							windowswmi = new LoadWindowsWMIFile(
									node.getIpAddress());
