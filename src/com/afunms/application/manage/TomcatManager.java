...
public class TomcatManager extends BaseManager implements ManagerInterface {
	...
	private String add() {
		...
		try {
			...
			timeShareConfigUtil.saveTimeShareConfigList(request,
					String.valueOf(vo.getId()),
					timeShareConfigUtil.getObjectType("13"));
			/* 增加采集时间设置 snow add at 2010-5-20 */
			TimeGratherConfigUtil timeGratherConfigUtil = new TimeGratherConfigUtil();
			timeGratherConfigUtil.saveTimeGratherConfigList(request,
					String.valueOf(vo.getId()),
					timeGratherConfigUtil.getObjectType("13"));
			/* snow add end */

			// 初始化采集指标
			try {
				NodeGatherIndicatorsUtil nodeGatherIndicatorsUtil = new NodeGatherIndicatorsUtil();
				nodeGatherIndicatorsUtil.addGatherIndicatorsForNode(vo.getId()
						+ "", "middleware", "tomcat", "1");
			...
			try {
				...
				alarmIndicatorsUtil.saveAlarmInicatorsThresholdForNode(
						String.valueOf(vo.getId()), "middleware", "tomcat");
			} catch (RuntimeException e) {
				e.printStackTrace();
			}
	...
	public String delete() {
		...
		try {
			...
			try {
				gatherdao.deleteByNodeIdAndTypeAndSubtype(id, "middleware",
						"tomcat");
			} catch (RuntimeException e) {
				e.printStackTrace();
			} finally {
			...
			for (int j = 0; j < list.size(); j++) {
				...
				if (vo != null) {
					...
					if (nodeDependDao.isNodeExist("tom" + id, vo.getXmlfile())) {
						nodeDependDao
								.deleteByIdXml("tom" + id, vo.getXmlfile());
					} else {
						nodeDependDao.close();
					}
					...
					ManageXml manageXml = (ManageXml) subMapDao.findByXml(vo
							.getXmlfile());
					if (manageXml != null) {
						NodeDependDao nodeDepenDao = new NodeDependDao();
						try {
	...
	private String ready_edit() {
		...
		try {
			...
			jsp = readyEdit(dao);
			TimeShareConfigUtil timeShareConfigUtil = new TimeShareConfigUtil();
			timeShareConfigList = timeShareConfigUtil
					.getTimeShareConfigList(getParaValue("id"),
							timeShareConfigUtil.getObjectType("13"));
			...
			List<TimeGratherConfig> timeGratherConfigList = tg
					.getTimeGratherConfig(getParaValue("id"),
							tg.getObjectType("13"));
	...
	private String update() {
		...
		if (PollingEngine.getInstance().getTomcatByID(vo.getId()) != null) {
			com.afunms.polling.node.Tomcat tomcat = (com.afunms.polling.node.Tomcat) PollingEngine
					.getInstance().getTomcatByID(vo.getId());
	...
	private String syncconfig() {
		...
		if (hash_data != null && hash_data.size() > 0) {
			if (!"".equals((String) hash_data.get("server"))
					&& !"null".equals((String) hash_data.get("server"))) {
				String server = hash_data.get("server") == null ? ""
						: hash_data.get("server").toString();
				...
				if (server != null) {
					...
					if (PollingEngine.getInstance().getTomcatByID(pvo.getId()) != null) {
						com.afunms.polling.node.Tomcat tomcat = (com.afunms.polling.node.Tomcat) PollingEngine
								.getInstance().getTomcatByID(pvo.getId());
	...
	public Hashtable detail(com.afunms.polling.node.Tomcat node) {
		...
		try {
			...
			for (int list_i = 0; list_i < pos_s.length - 1; list_i++) {
				...
				serverstream.foundData(serverinfo[0], serverinfo[1],
						serverinfo[2], serverinfo[3]);
				serverstream.foundApp(serverinfo[0], serverinfo[1],
						serverinfo[2], serverinfo[3]);
				...
				try {
					...
					if (data_ht != null) {
						...
						double freememory = Double
								.parseDouble(temjvm[0].trim());
						double totalmemory = (double) Double
								.parseDouble(temjvm[1].trim());
						double maxmemory = (double) Double
								.parseDouble(temjvm[2].trim());

						jvm_memoryuiltillize = (int) Math
								.rint((totalmemory - freememory) * 100
										/ totalmemory);
	...
	public double tomcatping(int id) {
		...
		try {
			...
			I_HostCollectData hostmanager = new HostCollectDataManager();
			try {
				ConnectUtilizationhash = getCategory(vo.getIpAddress(),
						"TomcatPing", "ConnectUtilization", starttime1,
						totime1, "");
			} catch (Exception ex) {
				ex.printStackTrace();
			}
	...
	public Hashtable getCollecttime(String ip) throws Exception {
		...
		try {
			...
			if ("mysql".equalsIgnoreCase(SystemConstant.DBType)) {
				sb.append(" select max(DATE_FORMAT(h.collecttime,'%Y-%m-%d %H:%i:%s')) as collecttime from tomcatping"
						+ allipstr + " h  ");
			} else if ("oracle".equalsIgnoreCase(SystemConstant.DBType)) {
				sb.append(" select max(h.collecttime) as collecttime from tomcatping"
						+ allipstr + " h  ");
			}
			sql = sb.toString();
			SysLogger.info(sql);
	...
	public Hashtable getCategory(String ip, String category, String subentity,
			String starttime, String endtime, String time) throws Exception {
		...
		try {
			if (!starttime.equals("") && !endtime.equals("")) {
				...
				if ("mysql".equalsIgnoreCase(SystemConstant.DBType)) {
					...
					if (category.equals("TomcatPing")) {
						sb.append(" select h.thevalue,h.collecttime,h.unit from tomcatping"
								+ time + allipstr + " h where ");
					}
					if (category.equals("tomcat_jvm")) {
						sb.append(" select h.thevalue,h.collecttime,h.unit from tomcat_jvm"
								+ allipstr + " h where ");
					}
					sb.append(" h.category='");
					sb.append(category);
					...
					sb.append(subentity);
					sb.append("' and h.collecttime >= ");
					sb.append("to_date('" + starttime
							+ "','YYYY-MM-DD HH24:MI:SS')");
					sb.append(" and h.collecttime <= ");
					sb.append("to_date('" + endtime
							+ "','YYYY-MM-DD HH24:MI:SS')");
					sb.append(" order by h.collecttime");
				}
				...
				if (category.equals("TomcatPing")
					...
					if (list1 != null && list1.size() > 0) {
						hash.put("avgpingcon",
								CEIString.round(pingcon / list1.size(), 2)
										+ unit);
				...
				if (category.equals("tomcat_jvm")) {
					if (list1 != null && list1.size() > 0) {
						hash.put(
								"avg_tomcat_jvm",
								CEIString.round(tomcat_jvm_con / list1.size(),
										2) + unit);
					} else {
	...
	public String app() {
		...
		com.afunms.polling.node.Tomcat tomcat = (com.afunms.polling.node.Tomcat) PollingEngine
				.getInstance().getTomcatByID(Integer.parseInt(id));
		...
		try {
			...
			try {
				Hashtable hash1 = getCategory(ip, "TomcatPing",
						"ConnectUtilization", starttime1, totime1, "");
				p_draw_line(hash1, "连通率", newip + "TomcatPing", 740, 150);
				Hashtable hash = getCategory(ip, "tomcat_jvm",
						"jvm_utilization", starttime1, totime1, "");
			...
			// imgurlhash
			imgurlhash.put("tomcat_jvm", "resource/image/jfreechart/" + newip
					+ "tomcat_jvm" + ".png");
			imgurlhash.put("TomcatPing", "resource/image/jfreechart/" + newip
					+ "TomcatPing" + ".png");
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String tomcat_jvm() {
		...
		// tomcat性能数据
		Hashtable tom = (Hashtable) ShareData.getTomcatdata()
				.get(ip + ":" + id);
		if (tom == null) {
				...
				if (category.equals("tomcat_jvm")) {
					if (list1 != null && list1.size() > 0) {
						hash.put(
								...
								CEIString.round(tomcat_jvm_con / list1.size(),
										2) + unit);
						hash.put("max_tomcat_jvm",
								CEIString.round(tempfloat1, 2) + unit + "");
					} else {
						hash.put("avg_tomcat_jvm", "0.0%");
	...
	private String isOK() {
		...
		try {
			...
			try {
				...
				com.afunms.polling.node.Tomcat tnode = (com.afunms.polling.node.Tomcat) PollingEngine
						.getInstance().getTomcatByIP(ipaddress);
	...
	private String alarm() {
		...
		try {
			...
			com.afunms.polling.node.Tomcat tomcat = (com.afunms.polling.node.Tomcat) PollingEngine
					.getInstance().getTomcatByID(Integer.parseInt(tmp));
	...
	private String showPingReport() {
		...
		try {
			...
			Hashtable ConnectUtilizationhash = tomdao1.getPingDataById(ip,
	...
	private String allReport() {
		...
		try {
			...
			try {
				...
				p_draw_line(hash1, "连通率", newip + "TomcatPing", 740, 150);
				Hashtable hash = getCategory(ip, "tomcat_jvm",
						"jvm_utilization", starttime, totime, "");
				p_draw_line(hash, "JVM内存利用率", newip + "tomcat_jvm", 740, 150);
				if (hash1 != null)
					request.setAttribute("pingcon", hash1);
				if (hash != null)
		...
		try {
			...
			Hashtable ConnectUtilizationhash = tomdao2.getPingDataById(ip,
					queryid, starttime, totime);
			// 画图----------------------
			String timeType = "minute";
			PollMonitorManager pollMonitorManager = new PollMonitorManager();
	...
	private String downloadReport() {
		...
		if ("per".equalsIgnoreCase(flag)) {
			if ("0".equals(str)) {
				...
				// excel综合报表
				report.createReportxls_tomcat_per(
						"temp/tomcat_PerReportxls.xls", id);
			...
			if ("0".equals(str)) {
				// report.createReport_host("temp/hostnms_report.xls");//
				// excel综合报表
				report.createReportxls_tomcat_all(
						"temp/tomcat_AllReportxls.xls", id);
				request.setAttribute("filename", report.getFileName());
			} else if ("1".equals(str)) {
				ExcelReport1 report1 = new ExcelReport1(new IpResourceReport(),
						reporthash);
			...
			if ("0".equals(str)) {
				// report.createReport_host("temp/hostnms_report.xls");//
				// excel综合报表
				report.createReportxls_tomcat_config(
						"temp/tomcat_reportxls.xls", id);
				request.setAttribute("filename", report.getFileName());
			} else if ("1".equals(str)) {
				ExcelReport1 report1 = new ExcelReport1(new IpResourceReport(),
						reporthash);
	...
	private String downloadEventReport() {
		...
		if ("0".equals(str)) {
			...
			try {
				report1.createReport_midEventDoc(fileName, starttime1, totime1,
						"tomcat");
			} catch (IOException e) {
				e.printStackTrace();
			}// word事件报表分析表
			...
			String fileName = ResourceCenter.getInstance().getSysPath() + file;// 获取系统文件夹路径
			try {
				report1.createReport_TomcatEventExc(file, id, starttime1,
						totime1, "tomcat");
			} catch (IOException e) {
				e.printStackTrace();
			}// xls事件报表分析表
			...
			String fileName = ResourceCenter.getInstance().getSysPath() + file;// 获取系统文件夹路径
			try {
				report1.createReport_midEventPdf(fileName, starttime1, totime1,
						"tomcat");
			} catch (IOException e) {
				e.printStackTrace();
			}// pdf事件报表分析表
