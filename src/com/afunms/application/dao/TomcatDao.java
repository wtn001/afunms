...
public class TomcatDao extends BaseDao implements DaoInterface {
	...
	public boolean delete(String id) {
		...
		try {
			...
			conn.addBatch("delete from topo_node_single_data where node_id="
					+ id);
			conn.addBatch("delete from topo_node_multi_data where node_id="
					+ id);
			conn.executeBatch();
			result = true;
		} catch (Exception e) {
			...
			if (pingdata.getRestype().equals("dynamic")) {
				...
				if ("mysql".equalsIgnoreCase(SystemConstant.DBType)) {
					...
					sql = "insert into "
							...
							+ "','" + pingdata.getBak() + "',"
							+ pingdata.getCount() + ",'"
							+ pingdata.getThevalue() + "',to_date('" + time
	...
	public int getidByIp(String ip) {
		String string = "select id from app_tomcat_node where ip_address ="
				+ "'" + ip + "'";
		int id = 0;
		ResultSet rSet = null;
	...
	public List getListCheckData(Tomcat tomcat) {
		// 验证是否有IP、端口号都相同的数据
		String sql = "select * from app_tomcat_node where ip_address ='"
				+ tomcat.getIpAddress() + "'";
	...
	public Hashtable getPingDataById(String ip, Integer id, String starttime,
			String endtime) {
		...
		if (!starttime.equals("") && !endtime.equals("")) {
			...
			String sql = "";
			if ("mysql".equalsIgnoreCase(SystemConstant.DBType)) {
				sql = "select a.thevalue,a.collecttime from tomcatping"
						+ ip.replace(".", "_") + " a where "
						+ "(a.collecttime >= '" + starttime
						+ "' and  a.collecttime <= '" + endtime
						+ "') order by id";
				;
			} else if ("oracle".equalsIgnoreCase(SystemConstant.DBType)) {
				sql = "select a.thevalue from tomcatping"
						+ ip.replace(".", "_") + " a where "
						+ " (a.collecttime >= " + "to_date('" + starttime
						+ "','YYYY-MM-DD HH24:MI:SS')"
						+ " and  a.collecttime <= " + "to_date('" + endtime
						+ "','YYYY-MM-DD HH24:MI:SS')" + ") order by id";
				;
			}
			// SysLogger.info(sql);
			int i = 0;
