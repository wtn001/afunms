...
public class DiscoverEngine {
	...
	private static final int MAX_THREADS = DiscoverResource.getInstance()
			.getMaxThreads(); // 最大线程数,根据tomcat设置
	private static ExecutorService threadExecutor = Executors
			.newFixedThreadPool(MAX_THREADS);
	private static ExecutorService runnableExecutor = Executors
			.newFixedThreadPool(MAX_THREADS);
	...
	public void unload() {
		 ...
		 * ((Runnable)threads.get(i)); } catch (Exception e) {
		 * 
		 * } } }
		 */
		instance = null;
	 ...
	 * threadPool.runTask(newThread); //threadPool.join();
	 * //threadExecutor.execute(newThread); //futureList.add(future);
	 * //threadList.add(newThread); //newThread.setName("afunms[" + threads +
	 * "]"); threads++; }
	 */
	...
	public void addHost(Host newNode, Link link) {
		 ...
		 * onchange="unionSelect();"> <option value='1' selected>路由器</option>
		 * <option value='2'>路由交换机</option> <option value='3'>交换机</option>
		 * <option value='4'>服务器</option> <option value='8'>防火墙</option>
		 * </select>
		 */
		int flag = 0;
		...
		for (int i = 0; i < hostList.size(); i++) {
			...
			if (tmpNode.getBridgeAddress() != null
					&& tmpNode.getBridgeAddress().equalsIgnoreCase(
							newNode.getBridgeAddress())) {
				// SysLogger.info("&&&&&******"+node.getIpAddress()+"----别名  "+node_ipalias.get(m)+"======="+tmpNode.getIpAddress()+"===temp_ip:"+temp_ip);
				SysLogger.info("类型为" + newNode.getCategory() + "的设备"
						+ newNode.getIpAddress() + "已经存在");
				// exist = true;
				flag = 1;
				break;
			}
			...
			{
				newNode = tmpNode;
				SysLogger.info("已发现的设备列表中已经存在" + newNode.getCategory() + "的设备:"
						+ newNode.getIpAddress() + ",从列表中得到该设备");
				flag = 1;
				break;
			} else {
				...
				{
					newNode = tmpNode;
					SysLogger.info("已发现的设备列表中已经存在" + newNode.getCategory()
							+ "的设备:" + newNode.getIpAddress() + ",从列表中得到该设备");
					flag = 1;
					break;
				}
			}
		...
		SysLogger.info("开始分析类型为" + newNode.getCategory() + "的设备:"
				+ newNode.getIpAddress());
		// 路由交换或二层交换,并且增加了防火墙
		if (newNode.getCategory() == 2 || newNode.getCategory() == 3
				|| newNode.getCategory() == 7 || newNode.getCategory() == 8) {
			newNode.setBridgeAddress(snmp.getBridgeAddress(
					newNode.getIpAddress(), newNode.getCommunity()));// 设置该IP对应的mac地址
			// try{
			...
			// }
			SysLogger.info("&&&&&&&&&&&&&&&&&&");
			SysLogger.info(newNode.getIpAddress() + " "
					+ newNode.getBridgeAddress());
			SysLogger.info("&&&&&&&&&&&&&&&&&&");
			// 得到STP的桥数据(STP只对二层交换起作用)
			if (newNode.getCategory() != 7) {
				if (newNode.getBridgestpList() == null
						|| newNode.getBridgestpList().size() == 0)
					...
					newNode.setBridgestpList(snmp.getBridgeStpList(
							newNode.getIpAddress(), newNode.getCommunity()));
		...
		if (newNode.getCategory() < 5 || newNode.getCategory() == 7) {
			...
			if (newNode.getIfEntityList() == null)
				...
				newNode.setIfEntityList(snmp.getIfEntityList(
						newNode.getIpAddress(), newNode.getCommunity(),
						newNode.getCategory()));
		...
		if (newNode.getSysOid() != null) {
			if (newNode.getSysOid().indexOf("1.3.6.1.4.1.25506") >= 0
					&& (newNode.getCategory() == 2 || newNode.getCategory() == 3)) {
				// H3C的交换设备
				if (newNode.getNdpHash() == null)
					// 返回的hashtable描述： key 保存mac地址，value
					// 保存mac地址对应的端口的名字，如ethernet1/1/1
					newNode.setNdpHash(snmp.getNDPTable(newNode.getIpAddress(),
							newNode.getCommunity()));
				// newNode.setNdpHash(SnmpUtils.getNDPTable(newNode.getIpAddress(),newNode.getCommunity(),3,5000));
			}
		}

		// 若一个设备已经存在,是否可以分析第二次????
		SysLogger.info("开始判断类型为" + newNode.getCategory() + "的设备:"
				+ newNode.getIpAddress() + "=======是否已经存在");
		...
		if (link != null)
			SysLogger.info(newNode.getIpAddress() + " " + newNode.getAdminIp()
					+ "   link:startip: " + link.getStartIp() + " endip:"
					+ link.getEndIp());
		...
		// return;
		// }
		SysLogger.info("结束判断类型为" + newNode.getCategory() + "的设备:"
				+ newNode.getIpAddress() + "======是否已经存在");

		newNode.setId(KeyGenerator.getInstance().getHostKey());
		// 若是网络设备
		if (newNode.getCategory() == 1 || newNode.getCategory() == 2
				|| newNode.getCategory() == 3 || newNode.getCategory() == 7
				|| newNode.getCategory() == 8) {
			if (newNode.getIpNetTable() == null
					|| newNode.getIpNetTable().size() == 0) {
				// 返回com.afunms.discovery.IpAddress类型的List，
				newNode.setIpNetTable(snmp.getIpNetToMediaTable(
						newNode.getIpAddress(), newNode.getCommunity()));// 得到所有IpNetToMedia,即直接与该设备连接的ip
			...
			if (newNode.getPortMacs() == null
					|| newNode.getPortMacs().size() == 0) {
				// 返回hashtable，50=[00:0f:e2:49:fc:47,
				// 00:0f:e2:49:fd:fa]，这是其中一个的键值对
				newNode.setPortMacs(snmp.getDtpFdbTable(newNode.getIpAddress(),
						newNode.getCommunity()));
				// newNode.setPortMacs(SnmpUtils.getDtpFdbTable(newNode.getIpAddress(),newNode.getCommunity(),3,5000));
			}
			SysLogger.info(newNode.getIpAddress()
					+ " ################# PortMacs size "
					+ newNode.getPortMacs().size());
			if (newNode.getAdminIp() == null) {
				newNode.setAtInterfaces(snmp.getAtInterfaceTable(
						newNode.getIpAddress(), newNode.getCommunity()));
			...
			if (newNode.getBridgeIdentifiers() == null) {
				newNode.addBridgeIdentifier(snmp.getBridgeAddress(
						newNode.getIpAddress(), newNode.getCommunity()));
				// try{
			...
			if (newNode.getFdbList() == null) {
				try {
					// 返回值是 String[]类型的List，其中的一个String[]为 [67108897,
					// 00:0f:e2:49:fc:47]
					newNode.setFdbList(snmp.getFdbTable(newNode.getIpAddress(),
							newNode.getCommunity()));
					// newNode.setFdbList(SnmpUtils.getFdbTable(newNode.getIpAddress(),
					// newNode.getCommunity(),-1,3,5000));
				} catch (Exception e) {
					SysLogger.info("获取设备" + newNode.getIpAddress() + "的FDB数据出错"
							+ e.getMessage());
				}
			}
			// 若是CISCO类型的设备,得到CDP数据
			if (newNode.getSysOid().indexOf("1.3.6.1.4.1.9") >= 0) {
				if (newNode.getCdpList() == null
						|| newNode.getCdpList().size() == 0)
					newNode.setCdpList(snmp.getCiscoCDPList(
							newNode.getIpAddress(), newNode.getCommunity()));
		...
		} else if (newNode.getCategory() == 2 || newNode.getCategory() == 3
				|| newNode.getCategory() == 7 || newNode.getCategory() == 8)// 把该交换设备加到它到它父节点的交换列表中
		{
			try {
				getHostByID(newNode.getSuperNode())
						.addSwitchId(newNode.getId());
			} catch (Exception e) {

			}
		}
		SysLogger.info("开始获取设备:" + newNode.getIpAddress() + "的系统名称");
		Hashtable sysGroupProperty = snmp.getSysGroup(newNode.getIpAddress(),
				newNode.getCommunity());
		// Hashtable sysGroupProperty =
		// SnmpUtils.getSysGroup(newNode.getIpAddress(),newNode.getCommunity(),3,5000);
		...
		if (newNode.getCategory() == 4 || newNode.getCategory() == 5
				|| newNode.getCategory() == 6) // 对于服务器和打印机,防火墙,无线接入
		{
			newNode.setDiscovered(true);
		...
		setStatus(newNode);
		SysLogger.info("新增加接点" + newNode.getIpAddress() + " Layer:"
				+ newNode.getLayer());
		...
		if (newNode.getCategory() != 7) {
			if (newNode.getIfEntityList() != null
					&& newNode.getIfEntityList().size() != 0)
				newNode.doDiscover();
		}
	}
	...
	public void addHost_SOLO(Host newNode, Link link) {
		...
		if (newNode.getCategory() < 5 || newNode.getCategory() == 7) {
			...
			SysLogger.info("开始获取设备:" + newNode.getIpAddress() + "的接口表");
			if (newNode.getIfEntityList() == null)
				newNode.setIfEntityList(snmp.getIfEntityList2(
						newNode.getIpAddress(), newNode.getCommunity(),
						newNode.getCategory(), newNode.getSnmpversion()));
	...
	public synchronized void addLinks(List temporaryLinks) {
		...
		for (int i = 0; i < temporaryLinks.size(); i++) {
			...
			IfEntity if1 = startNode.getIfEntityByIndex(templink.getStart()
					.getIfIndex());
			if (if1 == null)
				continue;
			...
			IfEntity if2 = endNode.getIfEntityByIndex(templink.getEnd()
					.getIfIndex());
			if (if2 == null)
				continue;
			...
			for (int j = 0; j < linkList.size(); j++) {
				...
				if (tempLink.getStartId() == link.getStartId()
					...
					System.out.println("Temp双链路:" + link.getStartIp() + "|"
							+ link.getStartIndex() + "<--->" + link.getEndIp()
							+ "|" + link.getEndIndex());
					System.out.println("和" + tempLink.getStartIp() + "|"
							+ tempLink.getStartIndex() + "<--->"
							+ tempLink.getEndIp() + "|"
							+ tempLink.getEndIndex());
					link.setAssistant(1);
					break;
				}
			}
	...
	private synchronized void addHost(Host node) {
		...
		for (int i = 0; i < hostList.size(); i++) {
			...
			if (tmpNode.getBridgeAddress() != null
					&& tmpNode.getBridgeAddress().equalsIgnoreCase(
							node.getBridgeAddress())) {
				SysLogger.info("类型为" + node.getCategory() + "的设备"
						+ node.getIpAddress() + "已经存在");
				// exist = true;
				flag = 1;
				break;
			}
			...
			// if(tmpNode.getCategory()==5 || tmpNode.getCategory()==6 )
			// continue; //打印机或防火墙
		}
		if (flag == 0) {
			hostList.add(node);
	...
	/**
	 * 确定一个设备是否已经存在 当前Link只有起始IP和起始端口连接ID
	 */
	private boolean isHostExist(Host node) {
		...
		for (int i = 0; i < hostList.size(); i++) {
			...
			if (tmpNode.getIpAddress().equalsIgnoreCase(node.getIpAddress())) {
				// existHost = tmpNode;
				SysLogger.info("已发现的设备列表中已经存在" + tmpNode.getCategory() + "的设备:"
						+ node.getIpAddress());
				exist = true;
				break;
			} else {
				...
				if (aliasIPs != null && aliasIPs.size() > 0) {
					if (aliasIPs.contains(node.getIpAddress())) {
						SysLogger.info("已发现的设备列表中已经存在" + tmpNode.getCategory()
								+ "的设备:" + node.getIpAddress());
						exist = true;
						break;
					}
					for (int k = 0; k < aliasIPs.size(); k++) {
						...
						if (node_ipalias != null && node_ipalias.size() > 0) {
							for (int m = 0; m < node_ipalias.size(); m++) {
								if (temp_ip
										.equalsIgnoreCase((String) node_ipalias
												.get(m))) {
		...
		SysLogger.info("类型为" + node.getCategory() + "的设备" + node.getIpAddress()
				+ "目前存在为--" + exist);
	...
	public boolean isDiscovered() {
		...
		// threadPool = null;
		/*
		 * SysLogger.info("############当前有"+threads+"个线程在运行##############"+"已经结束"
		 * +discoverdcount+"个线程"); //Runnable test =
		 * (Runnable)threadList.get(0); if(this.discoverdcount == this.threads){
		 * //threadPool.join(); //threadPool.interrupt();
		 * //threadPool.destroy(); return true; }else return false;
		 */
		...
		for (int i = 0; i < threadList.size(); i++) {
			...
			if (!bt.isCompleted()) {
				SysLogger.info(((BaseThread) threadList.get(i)).getName() + "("
						+ ((BaseThread) threadList.get(i)).getClass().getName()
						+ ") has not complete");
				finish = false;
				break;
			} else {
	...
	public void dealLink(Host node, Link link) {
		...
		if (link.getStartId() == link.getEndId())
			return;
		// 过滤掉端口聚合的情况(逻辑连接关系)
		if (link.getStartDescr().toLowerCase().indexOf("channel") >= 0)
			return;
		...
		// if( !hostidHash.containsKey(node.getId()+"") ||
		// !hostidHash.containsKey(link.getStartIndex()+""))return;
		if (!hostidHash.containsKey(node.getId() + "")
				|| !hostidHash.containsKey(link.getStartId() + ""))
			return;
		// hostList
		...
		if (link.getFindtype() == SystemConstant.ISRouter) {
			...
			if (ipObj == null) {
				SysLogger.info("======================");
				SysLogger.info(node.getIpAddress() + "设备中,找不到"
						+ link.getEndIp() + "和" + link.getStartIp()
						+ "对应的接口,返回");
				SysLogger.info("======================");
				return;
			}
	...
	public synchronized void addLink(List linkList, Link link) {
		if (linkList != null && linkList.size() > 0) {
			...
			if (existFlag == 0) {
				linkList.add(link);
				SysLogger.info("新增连接链路关系 " + link.getStartIp() + "|"
						+ link.getStartIndex() + "<--->" + link.getEndIp()
						+ "|" + link.getEndIndex());
			}
		} else {
	 ...
	 */
	@SuppressWarnings("rawtypes")
	public synchronized void checkAssistantLink(List linkList, Link link,
			int linktype) {
		if (linkList != null && linkList.size() > 0) {
			for (int i = 0; i < linkList.size(); i++) {
				Link tempLink = (Link) linkList.get(i);
				if (linktype != -1) {
					if (tempLink.getLinktype() < 0)
						continue;
				} else {
					if (tempLink.getLinktype() != linktype)
						continue;
				}

				if ((tempLink.getStartId() == link.getStartId() && tempLink
						.getEndId() == link.getEndId())
						|| (tempLink.getStartId() == link.getEndId() && tempLink
								.getEndId() == link.getStartId())) {
					if ((link.getStartIp().equalsIgnoreCase(
							tempLink.getStartIp()) && link.getStartIndex()
							.equalsIgnoreCase(tempLink.getStartIndex()))
							|| (link.getEndIp().equalsIgnoreCase(
									tempLink.getEndIp()) && link.getEndIndex()
									.equalsIgnoreCase(tempLink.getEndIndex())))
						continue;

					SysLogger.info("======================");
					SysLogger.info("dealLink找到双链路:" + link.getStartIp() + "|"
							+ link.getStartIndex() + "<--->" + link.getEndIp()
							+ "|" + link.getEndIndex());
					SysLogger.info("和" + tempLink.getStartIp() + "|"
							+ tempLink.getStartIndex() + "<--->"
							+ tempLink.getEndIp() + "|"
							+ tempLink.getEndIndex());
					link.setAssistant(1);
	...
	public boolean isSameLink(Link sourceLink, Link destLink) {
		if ((destLink.getStartIp().equalsIgnoreCase(sourceLink.getStartIp())
				&& destLink.getStartIndex().equalsIgnoreCase(
						sourceLink.getStartIndex())
				&& destLink.getEndIp().equalsIgnoreCase(sourceLink.getEndIp()) && destLink
				.getEndIndex().equalsIgnoreCase(sourceLink.getEndIndex()))
				|| (destLink.getStartIp().equalsIgnoreCase(
						sourceLink.getEndIp())
						&& destLink.getStartIndex().equalsIgnoreCase(
								sourceLink.getEndIndex())
						&& destLink.getEndIp().equalsIgnoreCase(
								sourceLink.getStartIp()) && destLink
						.getEndIndex().equalsIgnoreCase(
								sourceLink.getStartIndex())))
			return true;
		return false;
	}
	...
	private void findManageAddress(Host node) {
		...
		for (int i = 0; i < ifList.size(); i++) {
			...
			if (ifObj.getType() == 24 && !"".equals(ifObj.getIpAddress())
					&& !"127.0.0.1".equals(ifObj.getIpAddress())) {
				SysLogger.info(node.getIpAddress() + " 的管理地址是 "
						+ ifObj.getIpAddress());
	...
	private void setStatus(Host node) {
		...
		if (formerNodeList != null && formerNodeList.size() > 0) {
			...
			for (int k = 0; k < formerNodeList.size(); k++) {
				...
				// formernode.getBridgeAddress().equals(node.getBridgeAddress())){
				if (formernode.getIpAddress().equals(node.getIpAddress())) {
	...
	public synchronized Host getHostByID(int id) {
		Host host = null;
		for (int i = 0; i < hostList.size(); i++) {
			SysLogger.info("已有设备" + ((Host) hostList.get(i)).getIpAddress()
					+ " ID:" + ((Host) hostList.get(i)).getId() + " id:" + id);
