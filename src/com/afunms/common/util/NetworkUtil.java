...
public class NetworkUtil {
	...
	/**
	 * 3.用于确定路由表中一行中的dest是网络地址，还是路由设备地址
	 */
	public static boolean isNetAddress(String ipAddress, String netMask) {
		...
		for (int i = 0; i < 4; i++) {
			if (result == null) {
				result = "" + (ips[i] & masks[i]); // 按位与运算
			} else {
				result += "." + (ips[i] & masks[i]);
			}
		}
		boolean res = false;
		if (result.equals(ipAddress)) // 逻辑与运算的结果和原IP地址相同，则为网络地址
			res = true;
		return res;
	}

	/**
	 * 4.通过子网掩码取得该段网络中共有多少个IP地址
	 */
	...
	public static String long2ip(long ip) {
		...
		b[2] = (int) (ip >> 8 & 255L);
		b[3] = (int) (ip & 255L);
		String strIP = Integer.toString(b[0]) + "." + Integer.toString(b[1])
				+ "." + Integer.toString(b[2]) + "." + Integer.toString(b[3]);
		return strIP;
	}

	/**
	 * 7.检查一个ip是否是一个有效的主机地址(是否在掩码的范围内) netAddress:网络IP地址 netMask:子网掩码
	 * ipAddress:主机IP地址
	 */
	...
	public static boolean checkService(String ipAddress) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (vo.getPort() == 23)
				continue; // 不检查telnet,因为网络设备也有telnet
	...
	public static boolean isShieldAddress(String address) {
		...
		boolean result = false;
		Iterator iterator = DiscoverResource.getInstance().getShieldSet()
				.iterator();
	...
	public static int ping(String ipAddress) {
		...
		if (pingInfo == null
				|| pingInfo.indexOf("Destination host unreachable") != -1
				|| pingInfo.indexOf("Unknown host") != -1
				|| pingInfo.indexOf("Request timed out.") != -1)
			return 0;
		else
			return 1;
	}
