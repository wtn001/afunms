...
public class MonitorFactory {
	...
	static {
		...
		try {
			Document doc = builder.build(new File(ResourceCenter.getInstance()
					.getSysPath() + "WEB-INF/classes/monitor.xml"));
			...
			while (it.hasNext()) {
				...
				mo.setResultType(Integer.parseInt(element
						.getChildText("result_type")));
				mo.setThreshold(Integer.parseInt(element
						.getChildText("threshold")));
				mo.setUnit(element.getChildText("unit"));
				mo.setCompare(Integer.parseInt(element.getChildText("compare")));
				mo.setCompareType(Integer.parseInt(element.getChild("compare")
						.getAttributeValue("type")));
				mo.setUpperTimes(Integer.parseInt(element
						.getChildText("upper_times")));
				mo.setAlarmInfo(element.getChildText("alarm_info"));
				mo.setAlarmLevel(Integer.parseInt(element
						.getChildText("alarm_level")));
				mo.setPollInterval(Integer.parseInt(element
						.getChildText("poll_interval")));
				mo.setIntervalUnit(element.getChild("poll_interval")
						.getAttributeValue("unit"));
				mo.setDefault(Boolean.parseBoolean(element
						.getChildText("default")));
				mo.setEnabled(Boolean.parseBoolean(element
						.getChildText("enabled")));
				mo.setShowInTopo(Boolean.parseBoolean(element
						.getChildText("show_in_topo")));
				mo.setNodetype(element.getChildText("nodetype"));
				mo.setSubentity(element.getChildText("subentity"));
				mo.setLimenvalue0(Integer.parseInt(element
						.getChildText("limenvalue0")));
				mo.setLimenvalue1(Integer.parseInt(element
						.getChildText("limenvalue1")));
				mo.setLimenvalue2(Integer.parseInt(element
						.getChildText("limenvalue2")));
	...
	public static MonitoredItem createItem(String moid) {
		...
		try {
			item = (MonitoredItem) Class.forName(
					(String) itemClassNameMap.get(moid)).newInstance();
		} catch (Exception e) {
